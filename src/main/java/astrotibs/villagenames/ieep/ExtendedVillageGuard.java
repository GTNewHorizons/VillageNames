...
public class ExtendedVillageGuard implements IExtendedEntityProperties {
    ...
    /*
     * public int getProfession() { return this.profession; } public int getCareer() { return this.career; } public void
     * setProfession(int profession) { this.profession = profession >= 0 ? profession : -1; this.hasValidData = true; }
     * public void setCareer(int career) { this.career = career >= 0 ? career : -1; this.hasValidData = true; } public
     * void pickRandomProfession() { if (this.profession <= -1 && (this.hasValidData == null || !this.hasValidData)) {
     * int p = this.myWorld.rand.nextInt(6); // Uniformly distributed between 0 and 5 this.setProfession(p); } } public
     * void pickRandomProfessionAndCareer() { if (this.profession <= -1 && this.career <= -1 && (this.hasValidData ==
     * null || !this.hasValidData)) { int p = this.myWorld.rand.nextInt(6); // Uniformly distributed between 0 and 5 int
     * c = -1; switch(p) { case 0: // FARMER c = 1 + this.myWorld.rand.nextInt(4); case 1: // LIBRARIAN c = 1 +
     * this.myWorld.rand.nextInt(2); case 2: // PRIEST c = 1;// + this.myWorld.rand.nextInt(1); case 3: // BLACKSMITH c
     * = 1 + this.myWorld.rand.nextInt(3); case 4: // BUTCHER c = 1 + this.myWorld.rand.nextInt(2); case 5: // NITWIT c
     * = 1;// + this.myWorld.rand.nextInt(1); } this.setProfession(p); this.setCareer(c); } }
     */
