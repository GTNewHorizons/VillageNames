...
public class ReputationHandler {
    ...
    /**
     * Return the village reputation for a player player: the EntityPlayerMP entity to check villageTopTag: the village
     * to check as given by the topmost entry in Village.dat (e.g. "[12,10]") Village: the village that will be checked
     * for a reputation value via villageCollectionObj
     */
    public static int getVNReputationForPlayer(EntityPlayerMP player, String villageTopTag, Village village) {
        ...
        try {
            NBTBase nbtbase = nbttagcompound.getTag(villageTopTag); // Retrieve the specific village as indicated by
                                                                    // villageTopTag
        ...
        if (villageTag != null) { // Otherwise, the game crashes when in an artificial village
            ...
            if (villageTag.hasKey(repKey)) {
                playerReps = (NBTTagCompound) (villageTag.getTag(repKey)); // Retrieve the entry that has all the player
                                                                           // reps and cast it as a tag compound
                ...
                if (playerReps.hasKey(player.getUniqueID().toString())) {
                    ...
                    if (village == null) { // There is no village in the collectionObj--presumably, it was annihilated.
                        ...
                        // if (GeneralConfigHandler.debugMessages) LogHelper.info("This village was previously
                        // annihilated. Your reputation is now " + VNPlayerRep);
                        ...
                        if (PrevVNPlayerRep == collectionRep) { // They match.
                            ...
                            VNPlayerRep = PrevVNPlayerRep;
                            // if (GeneralConfigHandler.debugMessages) LogHelper.info("Your collection and VN reputation
                            // match: " + VNPlayerRep);
                        } else { // They do not match.
                            if (villageTag.getBoolean(collectedKey)) { // This village was previously reported as
                                                                       // "collected"
            ...
            } catch (Exception e) {} // Something went wrong. Probably because the village is null. Just leave it at
                                     // zero.
