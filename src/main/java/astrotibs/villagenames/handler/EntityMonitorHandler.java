                ...
                if (GeneralConfig.debugMessages) {
                    LogHelper.info(
                        "EntityMonitorHandler > A zombie just killed villager " + (villager.getCustomNameTag()
                            ...
                            // Vec3.createVectorHelper(villager.posX, villager.posY, villager.posZ) // Changed because
                            // of server crash
    ...
    public void onLivingUpdateEvent(LivingUpdateEvent event) {
        ...
        if (FunctionsVN.isVanillaZombie(event.entity)) {
            ...
            if (!zombie.worldObj.isRemote && zombie.isConverting()) {
                ...
                // Check the spaces around the zombie, and speed up or slow down the conversion process based on keyed
                // blocks
                ...
                if (zombie.worldObj.rand.nextFloat() < (0.01F * checkfactor)) {
                    ...
                    for (int groupi = 0; groupi < GeneralConfig.zombieCureGroups_map.get("Groups")
                        .size(); groupi++) { // Go through all the groups in GeneralConfig.zombieCureGroups_map
                            ...
                            for (int l = (int) zombie.posY - 4; l < (int) zombie.posY + 4
                                ...
                                for (int i1 = (int) zombie.posZ - 4; i1 < (int) zombie.posZ + 4
                                    ...
                                    for (int blocki = 0; blocki < GeneralConfig.zombieCureCatalysts_map.get("Groups")
                                        .size(); blocki++) { // Go through all the custom block entries
                        ...
                        // GeneralConfigHandler.debugMessages) {LogHelper.info("Incrementing conversion as per " +
                        // countedGroupBlocks + " blocks from " + group + " group.");}
                    }
                }
                ...
                try {
                    ...
                    // if (GeneralConfigHandler.debugMessages) LogHelper.warn("EntityMonitorHandler > Could not
                    // reflect/modify conversionTime field");
                }

                Method getConversionTimeBoost_m = ReflectionHelper.findMethod(
                    EntityZombie.class,
                    (EntityZombie) event.entity,
                    new String[] { "getConversionTimeBoost", "func_82233_q" }); // The MCP mapping for this method name
                ...
                try {
                    ...
                    // if (GeneralConfigHandler.debugMessages) {LogHelper.warn("EntityMonitorHandler > Could not reflect
                    // EntityZombie.getConversionTimeBoost");}
                }

                final int nextConversionTime = conversionTime - getConversionTimeBoost;// zombie.conversionTime -
                                                                                       // zombie.getConversionTimeBoost();
            ...
            if (!zombie.worldObj.isRemote) {
                ...
                if (ezv.getBiomeType() == -1) {
                    ezv.setBiomeType(FunctionsVN.returnBiomeTypeForEntityLocation(zombie));
                }
                if (ezv.getSkinTone() == -1) {
                    ezv.setSkinTone(FunctionsVN.returnSkinToneForEntityLocation(zombie));
                }

                if ((zombie.ticksExisted + zombie.getEntityId()) % 5 == 0) // Ticks intermittently, modulated so
                                                                           // villagers don't deliberately sync.
                {
                    ...
                    // if (ezv.getBiomeType() < 0)
                    // {ezv.setBiomeType(FunctionsVN.returnBiomeTypeForEntityLocation(zombie));}
                    // (ExtendedZombieVillager.get( zombie
        ...
        // New entity is a village guard. Check to see if it came into being via a player's recruitment.
            ...
            && (EventType.GUARD).getTracker()
                .size() > 0) {
                    ...
                    if (event.entity.worldObj.isRemote) {
                        // Looks for info sent by the server that should be applied to the zombie (e.g. villager
                        // profession)
                        ...
                        final EventTracker tracked = ServerInfoTracker.seek(
                            EventType.GUARD,
                            // Vec3.createVectorHelper(zombie.posX, zombie.posY, zombie.posZ)
                            new Vec3i(guard.posX, guard.posY + 0.5D, guard.posZ)); // Replaced because of mp server-side
                                                                                   // crash
                         ...
                         * if (GeneralConfigHandler.debugMessages) {
                         * LogHelper.
                         * info("EntityMonitorHandler > No villager info on the tracker--can't assign to guard!");
                         * }
                         * }
                         * if (GeneralConfigHandler.debugMessages) {
        ...
        // --- Initialize villager trades and sync skin with client --- //
            ...
            .toString()
            .substring(6)
            .equals(Reference.VILLAGER_CLASS) // Explicit vanilla villager class
        ...
        // Monitor the player for purposes of the village reputations achievements
            ...
            && event.entity.ticksExisted % (tickRate) == 0) { // Only check every few seconds so as not to bog down the
                                                              // server with Village.dat scans
                ...
                try {
                    ...
                    if (!villageTopTagPlayerIsIn.equals("none") || villageObjPlayerIsIn != null) { // Player is in a
                        ...
                        // --- Maximum Rep Achievement --- //
                            ...
                            && !player.func_147099_x()
                                .hasAchievementUnlocked(VillageNames.maxrep) // Copied over from EntityPlayerMP
                        ) {
