...
/**
 * This method rewards the player for completing all five achievements. It will be called each time an advancement is
 * triggered.
 * 
 * @author AstroTibs
 */
...
public class AchievementReward {
    ...
    public static void allFiveAchievements(EntityPlayerMP playerMP) {
        ...
        if (playerMP.func_147099_x()
            ...
            && playerMP.func_147099_x()
                .hasAchievementUnlocked(VillageNames.maxrep)
            && playerMP.func_147099_x()
                .hasAchievementUnlocked(VillageNames.minrep)) {

            // All five are done

            // Send a congratulation message as two parts
            String[] congratsStart = new String[] { "Nicely done!", "Congratulations!",
                "Thanks for using " + Reference.MOD_NAME + "!", "I'm glad you enjoy " + Reference.MOD_NAME + "!",
                "Great job!" };
            String[] congratsEnd = new String[] { "Here's a little something on me.", "Here's something as thanks.",
                "Take this as a memento.", "Have a keepsake.", "Here's a souvenir." };
            ...
            playerMP.addChatComponentMessage(
                new ChatComponentText(
                    EnumChatFormatting.GOLD + congratsStart[new Random().nextInt(congratsStart.length)]
                        + " "
                        + congratsEnd[new Random().nextInt(congratsEnd.length)]
                        + EnumChatFormatting.RESET));
            playerMP.addChatComponentMessage(
                new ChatComponentText(
                    EnumChatFormatting.GOLD + "It's a replica of a sword I used while testing "
                        + Reference.MOD_NAME
                        + " V3."
                        + EnumChatFormatting.RESET
                        + " --AstroTibs"));
