...
public class EntityInteractHandler {
    ...
    public void onPlayerInteractEvent(PlayerInteractEvent event) {
        if (!event.world.isRemote && event.action == Action.RIGHT_CLICK_BLOCK && GeneralConfig.debugMessages) {
            ...
            event.entityPlayer.addChatComponentMessage(
                    new ChatComponentText(
                            "Class path of this block: " + targetBlock.getClass().toString().substring(6)));
            event.entityPlayer
                    .addChatComponentMessage(new ChatComponentText("Unlocalized name: " + targetBlockUnlocName));
            ...
            // if (blockToSet!=null) {event.world.setBlock((int)event.entityPlayer.posX+2, (int)event.entityPlayer.posY,
            // (int)event.entityPlayer.posZ, blockToSet,
            // 0, // Meta
            // 2);}

            // Get the NBT list of marker data
            // MarkerToTileMap signposts_MarkerToTileMap = (MarkerToTileMap)
            // event.world.loadItemData(MarkerToTileMap.class, "signposts_MarkerToTileMap");
    ...
    public void onEntityInteract(EntityInteractEvent event) {
        ...
        if (Loader.isModLoaded("witchery") && event.entity instanceof EntityPlayer
                ...
                && event.entity.worldObj.villageCollectionObj != null
                && event.entity.worldObj.villageCollectionObj.findNearestVillage(
                        (int) event.entity.posX,
                        (int) event.entity.posY,
                        (int) event.entity.posZ,
                        villageRadiusBuffer) != null) {
            ...
            if (GeneralConfig.debugMessages) {
                LogHelper.info(
                        "EntityMonitorHandler > A player just attempted to recruit villager "
                                + (villager.getCustomNameTag().equals("") || villager.getCustomNameTag().equals(null)
                                        ? "(None)"
        ...
        else if (event.entity instanceof EntityPlayer // IF A PLAYER INITIALIZES THIS INTERACTION
            ...
            String targetClassPath = event.target.getClass().toString().substring(6); // The classpath string of the
                                                                                      // target
            ...
            if (target instanceof EntityGuardian) {
                if (!world.isRemote && ((EntityGuardian) target).isElder() // Reference "Elder" guardians using the
                                                                           // below string. Reference ordinary guardians
                                                                           // as EntityGuardian
                ) {
            ...
            int targetCareer = 0;
            boolean targetPlayerCreated = false; // Specifically to determine whether an iron golem was made by the
                                                 // player or not
            ...
            try {
                villageNearTarget = world.villageCollectionObj
                        .findNearestVillage((int) targetX, (int) targetY, (int) targetZ, villageRadiusBuffer);
            } catch (Exception e) {}

            // Player rep evaluated near the start
            int playerRep = 0;
            try {
                playerRep = ReputationHandler.getVNReputationForPlayer(
                        (EntityPlayerMP) player,
                        ReputationHandler.getVillageTagPlayerIsIn((EntityPlayerMP) player),
                        villageNearTarget);
            } catch (Exception e) {}
            ...
            // Wrapped this in a try/catch because non-standard entities (e.g. Robit, Pixelmon) cause a mysterious NPE
            // crash
            try {
            ...
            if (!world.isRemote && target instanceof EntityVillager && GeneralConfig.villagerCareers) {
                ...
                try {
                    if (isVanillaProfession(targetProfession)) {
                        ...
                        int indexofmodprof = GeneralConfig.modProfessionMapping_map.get("IDs")
                                .indexOf(targetProfession);
                        if (indexofmodprof > -1) {
                            villagerMappedProfession = !isVanillaProfession(targetProfession)
                    ...
                    LogHelper.error(
                            "Error evaluating mod profession ID mapping. Check the formatting of your Mod Professions config entry!");
                }
            }
            ...
            if (!world.isRemote && GeneralConfig.debugMessages) {
                ...
                if (target instanceof EntityVillager) {
                        ...
                        LogHelper.info(
                                "Profession: " + targetProfession
                                        + (GeneralConfig.villagerCareers
                                                ? ", Career: "
                                                        + (ExtendedVillager.get((EntityVillager) target)).getCareer()
                                                : "")
                                        + (GeneralConfig.modernVillagerSkins
                                                ? ", BiomeType: "
                                                        + (ExtendedVillager.get((EntityVillager) target)).getBiomeType()
                                                : "")
                                        + (GeneralConfig.modernVillagerSkins ? ", Profession Level: "
                                                + (ExtendedVillager.get((EntityVillager) target)).getProfessionLevel()
                                                : "")
                                        + ", Mapped profession: "
                                        + villagerMappedProfession
                                        + (GeneralConfig.villagerSkinTones
                                                ? ", Skin Tone: "
                                                        + (ExtendedVillager.get((EntityVillager) target)).getSkinTone()
                                                : ""));
                    ...
                    try {
                        LogHelper.info(
                                (GeneralConfig.modernZombieSkins
                                        ? "Zombie Profession: "
                                                + (ExtendedZombieVillager.get((EntityZombie) target)).getProfession()
                                        : "")
                                        + ((GeneralConfig.villagerCareers && GeneralConfig.modernZombieSkins)
                                                ? ", Career: " + (ExtendedZombieVillager.get((EntityZombie) target))
                                                        .getCareer()
                                                : "")
                                        + (GeneralConfig.modernZombieSkins
                                                ? ", BiomeType: " + (ExtendedZombieVillager.get((EntityZombie) target))
                                                        .getBiomeType()
                                                : "")
                                        + (GeneralConfig.modernZombieSkins
                                                ? ", Profession Level: "
                                                        + (ExtendedZombieVillager.get((EntityZombie) target))
                                                                .getProfessionLevel()
                                                : "")
                                        + (GeneralConfig.villagerSkinTones ? ", Skin Tone: "
                                                + (ExtendedZombieVillager.get((EntityZombie) target)).getSkinTone()
                                                : ""));
                    } catch (Exception e) {} // This is some kind of non-standard zombie
                }
                ...
                LogHelper.info(
                        "Entity CustomName: "
                                + (customName.equals("") || customName.equals(null) ? "(None)" : customName)
                                + ", Age: "
                                + targetAge
                                + (target instanceof EntityIronGolem ? ", PlayerCreated: " + targetPlayerCreated : ""));

                try {
                    LogHelper.info(player.getDisplayName() + " reputation in this village: " + playerRep);
                } catch (Exception e) {}
            }

            // If you're talking to a nitwit, cancel the trade gui
            if (GeneralConfig.enableNitwit && target instanceof EntityVillager && targetProfession == 5) {
                // summon Villager ~ ~ ~ {Profession:5}
                if (!world.isRemote) {
                    // Blank out the trade
                    MerchantRecipeList buyingList = ReflectionHelper.getPrivateValue(
                            EntityVillager.class,
                            (EntityVillager) target,
            ...
            if (itemstack != null && itemstack.getItem() == Items.name_tag) {
                ...
                if (itemstack.hasDisplayName() && !itemstack.getDisplayName().equals(customName)) {
                    if (!player.capabilities.isCreativeMode) {
                        ...
                        (target instanceof EntityVillager && GeneralConfig.nameEntities)
                                || (target instanceof EntityIronGolem && GeneralConfig.nameGolems
                                        && !targetPlayerCreated)
                                || (GeneralConfig.nameEntities && GeneralConfig.modNameMappingAutomatic_map
                                        .get("ClassPaths").contains(targetClassPath))
                                || (GeneralConfig.nameEntities && GeneralConfig.modNameMappingClickable_map
                                        .get("ClassPaths").contains(targetClassPath))) {
                            // If so, you should be prevented from naming the entity.
                            event.setCanceled(true);
                            if (!world.isRemote)
                                player.addChatComponentMessage(new ChatComponentText("That is not its name!"));
                            return;
                        }
                    } else {
                ...
                /*
                 * give AstroTibs minecraft:name_tag 64 0 {display:{Name:"Larongom"}} summon Wolf ~ ~ ~
                 * {Owner:"AstroTibs"}
                 */
                try {
                    if (!world.isRemote
                            && (GeneralConfig.entitiesNameableLikePets_set
                                    .contains(target.getClass().toString().substring(6))
                                    || (target instanceof EntityTameable && ((EntityTameable) target).isTamed()
                                            && ((EntityTameable) target).func_152114_e(player))
                                    || (target instanceof EntityHorse && ((EntityHorse) target).func_152119_ch() != null
                                            && ((EntityHorse) target).func_152119_ch()
                                                    .equals(player.getUniqueID().toString())))

                            && !target.hasCustomNameTag()
                            && (!itemstack.hasDisplayName()
                                    || (itemstack.hasDisplayName() && itemstack.getDisplayName().equals("")))) {
                        event.setCanceled(true);
                    ...
                    LogHelper.error("Caught exception when naming a pet: " + e);
                }
            }
            ...
            // -------------------------------//
            ...
            else if (!world.isRemote && event.entity.dimension == 0 // Only applies to the Overworld
                    && itemstack != null
                ...
                if ((population == 0 || (playerIsInVillage && population == -1)) // No Villagers in the village
                    ...
                    // Consume the poppy
                    if (!player.capabilities.isCreativeMode) {
                        player.inventory.consumeInventoryItem(Item.getItemFromBlock(Blocks.red_flower));
                    }
            ...
            // If you're holding an emerald or iron/gold ingot,
            else if (!world.isRemote && GeneralConfig.villagerMakesCodex
                    && itemstack != null
                    && (itemstack.getItem() == Items.emerald || itemstack.getItem() == Items.gold_ingot
                            || itemstack.getItem() == Items.iron_ingot)
                    && target instanceof EntityVillager
                    && !player.isSneaking() // I'm disallowing you from making a Codex when crouching because it doesn't
                                            // visually update your itemstacks correctly
                    && targetProfession == 1 // check to see if the target is a Villager with Profession 1 (Librarian).
                    && villagerMappedProfession == 1 // To prevent things like Traveling Merchant from selling you the
                                                     // Codex
            ) {
                event.setCanceled(true);
                ...
                if (targetAge < 0) {
                    ...
                    // Finds the nearest village to this target, but only if the villager's coordinates are within its
                    // bounding radius plus the buffer
                        ...
                        if (playerRep < 0) { // Your reputation is too low.
                            if (!world.isRemote) {
                                player.addChatComponentMessage(
                                        new ChatComponentText("The villager does not trust you."));
                            }
                        } else {
                            ...
                            for (int slot = 0; slot < player.inventory.getSizeInventory(); slot++) {
                                ...
                                if (Stack != null && Stack.getItem().equals(Items.emerald)) {
                                    emeralds += Stack.stackSize;
                                }
                                if (Stack != null && Stack.getItem().equals(Items.iron_ingot)) {
                                    ironIngots += Stack.stackSize;
                                }
                                if (Stack != null && Stack.getItem().equals(Items.gold_ingot)) {
                                    goldIngots += Stack.stackSize;
                                }
                            }
                            ...
                            if (GeneralConfig.debugMessages && !world.isRemote) {
                                LogHelper.info(
                                        "Your reputation: " + playerRep
                                                + ", Emeralds req: "
                                                + emeraldRequired
                                                + ", Iron req: "
                                                + ironRequired
                            ...
                            if (itemstack.getItem() == Items.emerald) {
                                ...
                                if (emeralds >= emeraldRequired + 1 || player.capabilities.isCreativeMode) { // Allow
                                                                                                             // Creative
                                                                                                             // players
                                                                                                             // to get
                                                                                                             // Codexes
                                    ...
                                    if (!player.capabilities.isCreativeMode) { // Consume the emeralds only if you're in
                                                                               // survival
                                        for (int i = 0; i < emeraldRequired + 1; i++) {
                                            player.inventory.consumeInventoryItem(Items.emerald);
                                        }
                                    }

                                    EntityItem eitem = (GeneralConfig.villagerDropBook ? target : player)
                                            .entityDropItem(new ItemStack(ModItems.codex), 1);
                                    eitem.delayBeforeCanPickup = 0; // No delay: directly into the inventory!
                                } else if (!world.isRemote) {
                                    player.addChatComponentMessage(
                                            new ChatComponentText("The Villager wants more emeralds from you."));
                                }
                            }
                            // If the item in your hand is a gold ingot, trade both gold ingots and emeralds.
                                ...
                                if ((emeralds >= emeraldRequired && goldIngots >= goldRequired)
                                        || player.capabilities.isCreativeMode) { // Allow Creative players to get
                                                                                 // Codexes regardless of emerald/gold
                                                                                 // count

                                    if (!player.capabilities.isCreativeMode) { // Consume the items only if you're in
                                                                               // survival
                                        for (int i = 0; i < emeraldRequired; i++) {
                                            player.inventory.consumeInventoryItem(Items.emerald);
                                        }
                                    ...
                                    if (emeralds < emeraldRequired && goldIngots < goldRequired) {
                                        player.addChatComponentMessage(
                                                new ChatComponentText(
                                                        "The Villager wants more emeralds and gold from you."));
                                    } else if (emeralds < emeraldRequired) {
                                        player.addChatComponentMessage(
                                                new ChatComponentText("The Villager wants more emeralds from you."));
                                    } else if (goldIngots < goldRequired) {
                                        player.addChatComponentMessage(
                                                new ChatComponentText("The Villager wants more gold from you."));
                                    }
                                }
                            ...
                            // If the item in your hand is an iron ingot, trade both iron ingots and emeralds.
                                ...
                                if ((emeralds >= emeraldRequired && ironIngots >= ironRequired)
                                        || player.capabilities.isCreativeMode) { // Allow Creative players to get
                                                                                 // Codexes regardless of emerald/iron
                                                                                 // count
                                    ...
                                    if (emeralds < emeraldRequired && ironIngots < ironRequired) {
                                        player.addChatComponentMessage(
                                                new ChatComponentText(
                                                        "The Villager wants more emeralds and iron from you."));
                                    } else if (emeralds < emeraldRequired) {
                                        player.addChatComponentMessage(
                                                new ChatComponentText("The Villager wants more emeralds from you."));
                                    } else if (ironIngots < ironRequired) {
                                        player.addChatComponentMessage(
                                                new ChatComponentText("The Villager wants more iron from you."));
                                    }
                                }
                            }
                        }
            ...
            // If you're holding a book...
            else if (!world.isRemote && itemstack != null
                    ...
                    && GeneralConfig.villagerMakesBook) {
                        ...
                        if (target instanceof EntityVillager) {
                            ...
                            if (targetAge >= 0) { // Villager is an adult.
                                ...
                                if (villageNearTarget == null) {
                                    ...
                                    if (playerRep < 0) { // Your reputation is too low.
                                        ...
                                        Set tagmapKeyset = tagCompound.func_150296_c(); // Gets the town key list:
                                                                                        // "coordinates"
                                        ...
                                        boolean signLocated = false; // Use this to record whether or not a sign was
                                                                     // found
                                        boolean isColony = false; // Use this to record whether or not the village was
                                                                  // naturally generated

                                        while (itr.hasNext()) { // Going through the list of VN villages
                                            Object element = itr.next();
                                            townSignEntry = element.toString(); // Text name of village header (e.g.
                                                                                // "x535y80z39")
                                            // The only index that has data is 0:
                                            NBTTagCompound tagList = tagCompound
                                                    .getTagList(townSignEntry, tagCompound.getId()).getCompoundTagAt(0);
                                            ...
                                            // Now find the nearest Village to that sign's coordinate, within
                                            // villageRadiusBuffer blocks outside the radius.
                                            Village villageNearSign = world.villageCollectionObj
                                                    .findNearestVillage(townX, townY, townZ, villageRadiusBuffer);

                                            // Player rep evaluated here before writing book
                                            int playerRepInVillageNearSign = 0;
                                            try {
                                                playerRepInVillageNearSign = ReputationHandler.getVNReputationForPlayer(
                                                        (EntityPlayerMP) player,
                                                        ReputationHandler
                                                                .getVillageTagPlayerIsIn((EntityPlayerMP) player),
                                                        villageNearSign);
                                            } catch (Exception e) {}

                                            isColony = tagList.getBoolean("isColony");

                                            if (villageNearSign == villageNearTarget) { // There is a match between the
                                                                                        // nearest village to this
                                                                                        // villager and the nearest
                                                                                        // village to the sign
                                                signLocated = true;
                                                if (!world.isRemote) {
                                                    ...
                                                    WriteBookHandler.targetWriteNewVillageBook(
                                                            ...
                                                            player.dimension == 0 ? ""
                                                                    : player.dimension == -1 ? "The Nether"
                                                                            : player.dimension == 1 ? "The End"
                                        ...
                                        if (!signLocated) {
                                            ...
                                            // We can assume this is a village WITHOUT a sign. So let's at least give it
                                            // a name!
                                            // Name type based on dimension
                                            Random deterministic = new Random();
                                            deterministic.setSeed(
                                                    world.getSeed() + FunctionsVN
                                                            .getUniqueLongForXYZ(centerX, centerY, centerZ));
                                            String[] newVillageName = NameGenerator
                                                    .newRandomName(
                                                            player.dimension == -1 ? "Village-Fortress"
                                                                    : player.dimension == 1 ? "Village-EndCity"
                                                                            : "Village",
                                                            deterministic);
                                            ...
                                            nbttagcompound1.setInteger("townColor", townColorMeta); // In case we want
                                                                                                    // to make clay,
                                                                                                    // carpet, wool,
                                                                                                    // glass, etc
                                            ...
                                            nbttagcompound1.setBoolean("fromEntity", true); // Record whether this name
                                                                                            // was generated from
                                                                                            // interaction with an
                                                                                            // entity

                                            if (player.dimension == 0 && // Added to avoid a crash by loading a null
                                                                         // list of villages
                                            !ReputationHandler.getVillageTagPlayerIsIn((EntityPlayerMP) player)
                                                    .equals("none")) {
                                                nbttagcompound1.setBoolean("preVN", true); // No Village Names entry was
                                                                                           // discovered, so presumably
                                                                                           // this village was generated
                                                                                           // before including VN
                                            } else {
                                                nbttagcompound1.setBoolean("isColony", true); // The village is
                                                                                              // player-created
                                                isColony = true;
                                            }

                                            // Form and append banner info
                                            // If you don't have a mod banner, this will not be added. It will be
                                            ...
                                            data.getData().setTag(
                                                    (namePrefix + " " + nameRoot + " " + nameSuffix)
                                                            .trim() + ", x" + centerX + " y" + centerY + " z" + centerZ,
                                                    nbttaglist);
                                ...
                                if (!world.isRemote) {
                                    player.addChatComponentMessage(new ChatComponentText(babyCantHelpString("book")));
                                }
                            }
                        }
                            ...
                            /*
                             * STRUCTURE NBTname key toptag Villages: villagenames Villages Temples: villagenames_te
                             * Temples Mineshafts: villagenames_mi Mineshafts Strongholds: villagenames_st Strongholds
                             * Monuments: villagenames_mo Monuments Mansions: villagenames_ma Mansions Fortress:
                             * villagenames_fo Fortresses End City: villagenames_ec EndCities Abandoned Base:
                             * villagenames_gcab AbandonedBases End Islands: villagenames_heei
                             * hardcoreenderdragon_EndIsland End Towers: villagenames_heet hardcoreenderdragon_EndTower
                             * Fronos Villages: FronosVillage villagenames_mpfv FronosVillages Koentus Vill:
                             * KoentusVillage villagenames_mpkv KoentusVillages Moon Villages: MoonVillage
                             * villagenames_gcmv MoonVillages Nibiru Villages: NibiruVillage villagenames_mpnv
                             * NibiruVillages
                             */
                            ...
                            try {
                                nameType = (String) (GeneralConfig.modStructureNames_map.get("NameTypes")).get(
                                        GeneralConfig.modStructureNames_map.get("ClassPaths").indexOf(targetClassPath));
                            } catch (Exception e) {}

                            // The name of the NBT structure file generated by the mod (e.g. "FronosVillage")
                            String structureType = "";
                            try {
                                structureType = (String) (GeneralConfig.modStructureNames_map.get("StructureTypes"))
                                        .get(
                                                GeneralConfig.modStructureNames_map.get("ClassPaths")
                                                        .indexOf(targetClassPath));
                            } catch (Exception e) {
                                structureType = "";
                            }
                            ...
                            try {
                                structureTitle = (String) (GeneralConfig.modStructureNames_map.get("StructureTitles"))
                                        .get(
                                                GeneralConfig.modStructureNames_map.get("ClassPaths")
                                                        .indexOf(targetClassPath));
                            } catch (Exception e) {}
                            ...
                            try {
                                dimensionName = (String) (GeneralConfig.modStructureNames_map.get("DimensionNames"))
                                        .get(
                                                GeneralConfig.modStructureNames_map.get("ClassPaths")
                                                        .indexOf(targetClassPath));
                            } catch (Exception e) {}
                            ...
                            try {
                                bookType = (String) (GeneralConfig.modStructureNames_map.get("BookTypes")).get(
                                        GeneralConfig.modStructureNames_map.get("ClassPaths").indexOf(targetClassPath));
                            } catch (Exception e) {}
                            ...
                            if (targetAge < 0) {
                                ...
                                try {
                                    structureData = (MapGenStructureData) world.perWorldStorage
                                            .loadData(MapGenStructureData.class, structureType);
                                    ...
                                    while (itr.hasNext()) {
                                        ...
                                        if (nbtbase.getId() == 10) { // 10 is "compound tag" I think?
                                            ...
                                            try {
                                                ...
                                                if (targetX >= boundingBox[0] && targetY >= boundingBox[1]
                                                        && targetZ >= boundingBox[2]
                                                        && targetX <= boundingBox[3]
                                                        && targetY <= boundingBox[4]
                                                        && targetZ <= boundingBox[5]) { // Target is inside bounding
                                                                                        // box.
                                                    ...
                                                    String[] structureInfoArray = WriteBookHandler
                                                            .tryGetStructureInfo(structureType, boundingBox, world);
                                                    ...
                                                    // If none is found, these strings are "null" which parseInt does
                                                    // not like very much
                                                    try {
                                                    ...
                                                    // If a name was NOT returned, then we need to generate a new one,
                                                    // as is done below:
                                                    ...
                                                    if (structureInfoArray[0] == null && structureInfoArray[1] == null
                                                        ...
                                                        deterministic.setSeed(
                                                                world.getSeed() + FunctionsVN
                                                                        .getUniqueLongForXYZ(signX, signY, signZ));
                                                        structureInfoArray = NameGenerator
                                                                .newRandomName(nameType, deterministic); // Generates
                                                                                                         // name based
                                                                                                         // on table
                                                                                                         // above
                                                        ...
                                                        nbttagcompound1.setInteger("townColor", townColorMeta); // In
                                                                                                                // case
                                                                                                                // we
                                                                                                                // want
                                                                                                                // to
                                                        ...
                                                        nbttagcompound1.setString("nameRoot", nameRoot);
                                                        nbttagcompound1.setString("nameSuffix", nameSuffix);
                                                        nbttagcompound1.setBoolean("fromEntity", true); // Record
                                                                                                        // whether this
                                                                                                        // name was
                                                                                                        // generated
                                                                                                        // from
                                                        ...
                                                        // .getTagList() will return all the entries under the specific
                                                        // village name.

                                                        tagCompound.setTag(
                                                                (namePrefix + " " + nameRoot + " " + nameSuffix).trim()
                                                                        + ", x"
                                                                        + signX
                                                                        + " y"
                                                                        + signY
                                                    ...
                                                    // Either the structure name already existed, or it was just
                                                    ...
                                                    // I don't care if the structure has a sign. We have to cut the name
                                                    // up into arbitrary sign strings for the book.
                                                    ...
                                                    if (!world.isRemote) {
                                                        WriteBookHandler.targetWriteNewVillageBook(
                                                                ...
                                                                nameRoot,
                                                                nameSuffix,
                                                                structureType.equals("Village"),
                                                                null,
                                                                event,
                                                                targetProfession,
                                                                targetCareer, // Also unused but there's no harm in
                                                                              // leaving them
                                                                -1,
                                                                -1, // Trade size and player rep are unused outside of
                                                                    // vanilla villages
                                                                player,
                                                                target);
                                                    }
                                                ...
                                                // There's a tag like [23,-3] (chunk location) but there's no bounding
                                                // box tag. Not a good sign, bruv.
                                                if (GeneralConfig.debugMessages && !world.isRemote) {
            ...
            // If you're holding a village book, right clicking a villager will return your village reputation.
                    ...
                    && target instanceof EntityVillager) {
                        ...
                        if (villageNearTarget != null && player.dimension == 0) // Villager is in a town, so get the rep
                                                                                // message
                        {
                            try {
                                player.addChatComponentMessage(
                                        new ChatComponentText(villagerAssessReputation(playerRep)));
                                return;
                            } catch (Exception e) {} // Could not assess reputation -- this might not be a valid village
                                                     // world.
                        }
                        villagerConfused(player);
            ...
            // If you're holding anything else (or nothing), check to see if the target is a Villager, Village Golem, or
            // entry from the config list.
            else if (!world.isRemote) {
                // Update villager trades on interaction
                if (event.target instanceof EntityVillager && GeneralConfig.modernVillagerTrades) {
                    FunctionsVN.monitorVillagerTrades((EntityVillager) event.target);
                }
                ...
                if (GeneralConfig.nameEntities
                    ...
                    // Unfortunately, I18n are client-side only and will crash the server. So we're going to just have
                    // to deal with the English versions.
                    if ((customName.trim()).equals("") || customName.equals(null)
                            || (targetClassPath
                                    .equals("net.daveyx0.primitivemobs.entity.passive.EntityTravelingMerchant")
                                    && (customName.equals(PMTMUnloc) || customName.equals(PMTMUnlocModern))// Contingency
                                                                                                           // in there
                                                                                                           // specifically
                                                                                                           // for PM's
                                                                                                           // Traveling
                            ...
                            || (targetClassPath.equals("net.daveyx0.primitivemobs.entity.passive.EntitySheepman")
                                    && (customName.equals(PMShUnloc) || customName.equals(PMShUnlocModern)) // Contingency
                                                                                                            // in there
                                                                                                            // specifically
                                                                                                            // for PM's
                                                                                                            // Sheepman
                            ...
                            || (targetClassPath.equals("net.daveyx0.primitivemobs.entity.passive.EntitySheepmanSmith")
                                    && (customName.equals(PMSSUnloc)) // Contingency in there specifically for PM's
                                                                      // Sheepman Smith
                        ...
                        try {
                            // Get the index number where targetClassPath is located, and then pull the NameTypes of
                            // that same index
                            mappedNameType = (String) ((GeneralConfig.modNameMappingClickable_map.get("NameTypes")).get(
                                    GeneralConfig.modNameMappingClickable_map.get("ClassPaths")
                                            .indexOf(targetClassPath)));
                        } catch (Exception e) { // Your config file was poorly formatted
                            if (!world.isRemote) LogHelper.error(
                                    "Your othermods.cfg > Clickable Names entries returned an error. Check to make sure they're formatted properly!");
                        }

                        String[] newNameList = NameGenerator.newRandomName(mappedNameType, new Random());
                        String newCustomName = (newNameList[1].trim() + " "
                                + newNameList[2].trim()
                                + " "
                                + newNameList[3].trim()).trim(); // Generate new name
                        // Generate profession tag
                        if (GeneralConfig.addJobToName && (!(target instanceof EntityVillager) || targetAge >= 0)) {
                            newCustomName += " "
                                    + NameGenerator.getCareerTag(targetClassPath, targetProfession, targetCareer);
                        }
                        // Apply the name
                        target.setCustomNameTag(newCustomName.trim());
                    }
                    // Clickable Entity already has a name. You may want to add (or remove) a career tag.
                    else if (customName.indexOf("(") == -1 && GeneralConfig.addJobToName
                            && (!(target instanceof EntityVillager) || targetAge >= 0)) { // Target is named but does
                                                                                          // not have job tag: add one!
                                String newCustomName = customName + " "
                                        + NameGenerator.getCareerTag(targetClassPath, targetProfession, targetCareer);
                                // Apply the name
                                target.setCustomNameTag(newCustomName.trim());
                            } else
                        if (customName.indexOf("(") != -1 && !GeneralConfig.addJobToName) { // Target has a job tag:
                                                                                            // remove it...
                            target.setCustomNameTag(customName.substring(0, customName.indexOf("(")).trim());
                        }
                }
                // Entity is a custom automatic config entry.
                else if (GeneralConfig.nameEntities
                        && GeneralConfig.modNameMappingAutomatic_map.get("ClassPaths").contains(targetClassPath)) {
                            if (((customName.trim()).equals("") || customName.equals(null))
                                    && !((String) ((GeneralConfig.modNameMappingAutomatic_map.get("AddOrRemove")).get(
                                            GeneralConfig.modNameMappingAutomatic_map.get("ClassPaths")
                                                    .indexOf(targetClassPath)))).trim().equals("remove")) {

                                // Generate a name type that's defined in the config entry

                                String mappedNameType = "villager";
                                try {
                                    // Get the index number where targetClassPath is located, and then pull the
                                    // NameTypes of that same index
                                    mappedNameType = (String) ((GeneralConfig.modNameMappingAutomatic_map
                                            .get("NameTypes")).get(
                                                    GeneralConfig.modNameMappingAutomatic_map.get("ClassPaths")
                                                            .indexOf(targetClassPath)));
                                } catch (Exception e) { // Your config file was poorly formatted
                                    if (!world.isRemote) LogHelper.error(
                                            "Your othermods.cfg > Automatic Names entries returned an error. Check to make sure they're formatted properly!");
                                }
                                ...
                                String newCustomName = (newNameList[1].trim() + " "
                                        ...
                                        + newNameList[3].trim()).trim(); // Generate new name
                                // Generate profession tag
                                if (GeneralConfig.addJobToName
                                        && (!(target instanceof EntityVillager) || targetAge >= 0)) {
                                    newCustomName += " " + NameGenerator
                                            .getCareerTag(targetClassPath, targetProfession, targetCareer);
                                }
                                // Apply the name
                                target.setCustomNameTag(newCustomName.trim());
                            }
                            // Automatic Entity already has a name. You may want to add (or remove) a career tag.
                            if (customName.indexOf("(") == -1 && GeneralConfig.addJobToName
                                    && (!(target instanceof EntityVillager) || targetAge >= 0)) { // Target is named but
                                                                                                  // does not have job
                                                                                                  // tag: add one!
                                String newCustomName = customName + " "
                                        + NameGenerator.getCareerTag(targetClassPath, targetProfession, targetCareer);
                                // Apply the name
                                target.setCustomNameTag(newCustomName.trim());
                            } else if (customName.indexOf("(") != -1 && !GeneralConfig.addJobToName) { // Target has a
                                                                                                       // job tag:
                                                                                                       // remove it...
                                target.setCustomNameTag(customName.substring(0, customName.indexOf("(")).trim());
                            }
                ...
                // Entity is a Villager
                    ...
                    if (GeneralConfig.nameEntities) {
                        ...
                        if ((customName.trim()).equals("") || customName.equals(null)) {
                            ...
                            // Generate profession tag
                            String careerTag = "";
                            if (GeneralConfig.addJobToName && (targetAge >= 0)) {
                                newCustomName += " "
                                        + NameGenerator.getCareerTag(targetClassPath, targetProfession, targetCareer);
                            }
                            // Apply the name
                        ...
                        // Villager already has a name. You may want to add (or remove) a career tag.
                        else if (customName.indexOf("(") == -1 && GeneralConfig.addJobToName && (targetAge >= 0)) { // Villager
                                                                                                                    // is
                                                                                                                    // named
                                                                                                                    // but
                                                                                                                    // does
                            ...
                            // Apply the name
                            target.setCustomNameTag(newCustomName.trim());
                        } else if (customName.indexOf("(") != -1 && !GeneralConfig.addJobToName) { // Villager has a job
                                                                                                   // tag: remove it...
                            target.setCustomNameTag(customName.substring(0, customName.indexOf("(")).trim());
                        }
                        ...
                        && ((customName.trim()).equals("") || customName.equals(null))) {
                            ...
                            if (targetPlayerCreated) {
                                ...
                                if (!world.isRemote) player.addChatComponentMessage(
                                        new ChatComponentText("You can give this golem a name tag!"));
                            } else {
                                // Determine whether it's in/near a populated village.
                                Village villageNearGolem = world.villageCollectionObj == null ? null
                                ...
                                if (villageNearGolem != null) { // The golem is in/near a village
                                    if ((customName.trim()).equals("") || customName.equals(null)) {
                                        if (villageNearGolem.getNumVillagers() > 0) {
                                            ...
                                            player.addChatComponentMessage(
                                                    new ChatComponentText(
                                                            "There is nobody left to tell you its name."));
                                        }
                                    }
                                    ...
                                    if (!world.isRemote) {
                                        player.addChatComponentMessage(
                                                new ChatComponentText("There is nobody around to tell you its name."));
                                    }
                                }
                            }
                        }
        ...
        // Execute healing procedure with Zombie Villager
            ...
            if (!zombie.worldObj.isRemote) {
                ...
                if (item != null && item.getItem() == Items.golden_apple && item.getItemDamage() == 0) {// item.getMetadata()
                                                                                                        // == 0) {
    ...
    public void onPlayerAttackEntity(AttackEntityEvent event) {
        ...
        if (event == null || event.target == null || event.target.worldObj == null) {
            return;
        }

        if (!event.target.worldObj.isRemote && event.entity.dimension == 0 // Only applies to the Overworld
                && event.target instanceof EntityLiving) {

            EntityLiving target = (EntityLiving) event.target;

            if (event.target instanceof EntityVillager || event.target instanceof EntityIronGolem
                ...
                try {
                    ...
                    int playerRep = ReputationHandler.getVNReputationForPlayer(
                            playerMP,
                            ReputationHandler.getVillageTagPlayerIsIn(playerMP),
                            villageNearTarget);

                    if (playerRep <= -30 // Town rep is minimum
                            && !playerMP.func_147099_x().hasAchievementUnlocked(VillageNames.minrep) // Copied over from
                                                                                                     // EntityPlayerMP
                    ) {
    ...
    /**
     * Generates a "baby villager can't help" string from a pool of possibilities. Setting forType to "book" or "codex"
     * will add possibilities unique to that type.
     */
    ...
    public static void villagerConfused(EntityPlayer player) {
        ...
        if (!player.worldObj.isRemote) player.addChatComponentMessage(
                new ChatComponentText(
                        messagetype
                                ? "The villager seems "
                                        + cantHelpAdjective.get(new Random().nextInt(cantHelpAdjective.size()))
                                        + ", and glances around."
                                : "The villager gives you a "
                                        + cantHelpAdjective.get(new Random().nextInt(cantHelpAdjective.size()))
                                        + " look."));
    }
    ...
    public static String villagerAssessReputation(int playerRep) {
        ...
        String[] villagerAssessmentPool = new String[] { "You are unable to gauge the villager's opinion of you.",
                "You are unsure how the village feels about you.", "This villager isn't interested in gossip.",
                "This villager doesn't know what the others think of you." };

        if (playerRep >= 7) {
            // Extremely positive reputation
            villagerAssessmentPool = new String[] { "You have brought great prosperity to this town.",
                    "You are very well regarded in this town!", "You are notably reputable in this village!",
                    "The town views you very favorably!" };
        } else if (playerRep >= 4) {
            // Very positive reputation
            villagerAssessmentPool = new String[] { "The villagers like having you around!",
                    "This villager has heard very good things about you!",
                    "The villagers here would like to get to know you!", "You are well regarded by this town." };
        } else if (playerRep >= 2) {
            // Moderately positive reputation
            villagerAssessmentPool = new String[] { "This villager has heard good things about you!",
                    "There is a general air of cautious trust regarding you.",
                    "You are well regarded by a couple villagers.", "Your presence gives the town some optimism!" };
        } else if (playerRep >= 1) {
            // Slightly positive reputation
            villagerAssessmentPool = new String[] { "The villagers don't mind you being around.",
            ...
            // Neutral reputation
            villagerAssessmentPool = new String[] { "The villagers are ambivalent to you.",
                    "The villagers don't know what to make of you.", "Nobody knows what to make of you.",
                    "The villagers are indifferent to you." };
        } else if (playerRep >= -3) {
            // Slightly negative reputation
            villagerAssessmentPool = new String[] { "There are unsavory rumors around town about you.",
                    "This villager seems unhappy with you.", "The village doesn't trust you.",
                    "This villager seems worried about you." };
        } else if (playerRep >= -9) {
            // Moderately negative reputation
            villagerAssessmentPool = new String[] { "You are viewed as a danger.",
                    "Your misdeeds are known throughout the town.", "This villager is visibly afraid of you.",
                    "The village sees you as a menace." };
        } else if (playerRep >= -18) {
            // Very negative reputation
            villagerAssessmentPool = new String[] { "The villagers are all afraid of you.",
                    "Your cruel acts are known to everyone.", "The villagers regard you as evil.",
                    "No villager will sleep soundly until you're gone." };
        } else {
            // Extremely negative reputation
            villagerAssessmentPool = new String[] { "Your vile acts have brought tragedy to this town.",
                    "You have brought devastation to this village.", "You are the stuff of nightmares.",
