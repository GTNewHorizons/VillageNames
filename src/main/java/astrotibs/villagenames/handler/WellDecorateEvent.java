...
public class WellDecorateEvent {
    ...
    public void onPopulating(PopulateChunkEvent.Post event) {
        ...
        if (event.hasVillageGenerated && event.world.provider.dimensionId == 0
            ...
            int inwardYaw = (GeneralConfig.villageBanners && testForBanner != null) ? GeneralConfig.signYaw : 4; // Value
                                                                                                                 // from
                                                                                                                 // 0 to
                                                                                                                 // 4
                                                                                                                 // indicating
                                ...
                                if (listWater.size() == 3 && listWater2.size() == 3
                                    ...
                                    if (1 - Math.abs(isWellCorner / 2 - 1) + ((isWellCorner - 1) % 2) * 2
                                        ...
                                        if (event.world.getBlock(x + signXOffset, y + 2, z + signZOffset)
                                            ...
                                            int villageArea = -1; // If a village area value is not ascertained, this
                                                                  // will remain as -1.
                                            ...
                                            // Assign the top line of the sign based on the area of the Village,
                                            // determined by bounding box

                                            int topLineRand = random.nextInt(4);

                                            String topLine = "Welcome to"; // The top line will be blank or "welcome"
                                                                           // for rand is 0 or 1

                                            if (topLineRand > 1) { // Half the time (rand is 2 or 3): actually make the
                                                                   // top line size-dependent!
                                                ...
                                                // If none of the above are true, then the size could not be
                                                // ascertained,
                                            ...
                                            if (GeneralConfig.wellDecorations && GeneralConfig.wellBoundary) {
                                                ...
                                                // Allow concrete well decoration even if Village Names concrete is
                                                // unavailable
                                                ...
                                                // Allow concrete well decoration even if Village Names concrete is
                                                // unavailable
                                                int metaSpin = random.nextInt(4) + 4; // I've got to add 4 because
                                                ...
                                                if (tryGlazedTerracotta != null && GeneralConfig.concreteWell) {
                                                    ...
                                                    int metaChirality = random.nextBoolean() ? 1 : -1; // Determines
                                                                                                       // "rotation
                                                                                                       // handedness" of
                                                                                                       // blocks as
                                                                                                       // they're placed
    ...
    /**
     * Roughly estimates if a center block is cornered by blocks of the given type. Actually searching for two unaligned
     * border blocks
     *
     * @param world The world containing the blocks
     * @param id    The block searched
    ...
    /**
     * Inputs PopulateChunkEvent.Post event, and determines whether the x, z in that event falls within the
     * buffer-expanded bounding box, of ANY generated Temple structure. It returns the coordBaseMode integer, or -1 if
     * no Temple was found.
     */
    public void searchTempleAndReplaceWool(PopulateChunkEvent.Post event, int buffer) {
        MapGenStructureData structureData = null;
        if (event.world.provider.dimensionId == 0 && !event.world.isRemote) { // Player is in the Overworld
            try {
