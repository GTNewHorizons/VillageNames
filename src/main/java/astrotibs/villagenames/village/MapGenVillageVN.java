...
public class MapGenVillageVN extends MapGenVillage {
    ...
    protected boolean canSpawnStructureAtCoords(int chunkXin, int chunkZin) {
        ...
        if (chunkXin < 0) {
            chunkXin -= this.field_82665_g - 1;
        }
        if (chunkZin < 0) {
            chunkZin -= this.field_82665_g - 1;
        }
        ...
        Random random = this.worldObj.setRandomSeed(chunkXModulated, chunkZModulated, 10387312); // Idk the significance
                                                                                                 // of this number. May
                                                                                                 // be unique to
                                                                                                 // "Village"
                                                                                                 // structures?
        ...
        // Return "true" if this chunk X & Z is flagged for village construction AND the biome is allowed as per the
        // config
        if (chunkX == chunkXModulated && chunkZ == chunkZModulated) {
            BiomeGenBase biome = this.worldObj.getBiomeGenForCoords(chunkX * 16 + 8, chunkZ * 16 + 8); // No need for
                                                                                                       // bit shift
                                                                                                       // here. Villages
                                                                                                       // rarely spawn.
    ...
    public static class Start extends StructureStart {
        ...
        public Start(World world, Random random, int chunkX, int chunkZ, float villageSize) {
            ...
            if (GeneralConfig.debugMessages) {
                ...
                int unmappedComponent = 0; // Counts how many structure components try to generate. If none, no text is
                                           // printed.
            ...
            if (totalWeight <= 0D) {
                start = plainsStarters[1];
            } // If total weight is 0, default to Plains well
            ...
            // start = new PlainsStructures.PlainsMeetingPoint1(world.getWorldChunkManager(), 0, random, (chunkX << 4) +
            // 2, (chunkZ << 4) + 2, list, villageSize);
            ...
            while (iterator.hasNext()) {
                ...
                if (!(structurecomponent_temp instanceof Road)) {
                    ++counter;
                }
            }
