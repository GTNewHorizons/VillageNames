...
public class StructureVillageVN {
    ...
    public static List getStructureVillageWeightedPieceList(Random random, float villageSize,
        ...
        VillagerRegistry.addExtraVillageComponents(
            arraylist,
            random,
            Math.floor(villageSize) + villageSize % 1 < random.nextFloat() ? 1 : 0); // Round to integer stochastically
     ...
     * x, int y, int z, int coordBaseMode, int componentType)
     * {
     * if (componentType > 50)
     * {
     * return null;
     ...
     * structureboundingbox = StructureBoundingBox.getComponentToAddBoundingBox(x, y, z, 0, 0, 0, 5, 12, 9,
     * coordBaseMode);
     * structurecomponent = (structureboundingbox != null && structureboundingbox.minY > 10 &&
     * StructureComponent.findIntersecting(components, structureboundingbox) == null) ? new
     * StructureVillagePieces.Church(start, componentType, random, structureboundingbox, coordBaseMode) : null;
     * if (structurecomponent != null)
     * {
    ...
    public static StructureBoundingBox findPieceBox(StructureVillagePieces.Start start,
        ...
        for (int i = 7 * MathHelper.getRandomIntegerInRange(random, 3, 5); i >= 7; i -= 7) {
            StructureBoundingBox structureboundingbox = StructureBoundingBox.getComponentToAddBoundingBox(
                ...
                0,
                0,
                0, // u, v, w offset. The u and w offsets are positive for horizIndex <=1 and negative for horizIndex
                   // >=2
                3,
                3,
     ...
     * A profession of -1 means return a random one. Whether this is vanilla only or from all registered will be
     * determined by the configs.
     ...
     * rightHandRule - whether the door opens counterclockwise when viewed from above. This is default state when placed
     * by a player
     */
     ...
     * The decor will be randomly selected based on the village type, except you can specify whether to allow troughs
     * for Taiga types
     */
