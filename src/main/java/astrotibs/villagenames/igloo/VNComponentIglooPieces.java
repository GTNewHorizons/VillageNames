...
public class VNComponentIglooPieces {
                ...
                for (int l = this.boundingBox.minZ; l <= this.boundingBox.maxZ; ++l) {
                    for (int i1 = this.boundingBox.minX; i1 <= this.boundingBox.maxX; ++i1) {
                        if (structurebb.isVecInside(i1, 64, l)) {
                            j += Math.max(
                                    worldIn.getTopSolidOrLiquidBlock(i1, l),
    ...
    public static class Igloo extends VNComponentIglooPieces.Feature {
         ...
         * at the end, it adds Fences... Author: Searge Translated to 1.7/1.8 by: AstroTibs
         */
        public boolean addComponentParts(World worldIn, Random randomIn, StructureBoundingBox structureBoundingBoxIn) {
            if (!this.func_74935_a(worldIn, structureBoundingBoxIn, -1)) // offsetToAverageGroundLevel()
            {
                 ...
                 * for (int x = minX; x <= maxX; x++) { for (int z = minZ; z <= maxZ; z++) { if (worldIn.getBlock(x, y,
                 * z) == Blocks.crafting_table) { // Detected a crafting table. Place the furnace as necessary. // Clear
                 * out blocks that would have surrounded the furnace for (int w=0; w <=3; w++) { worldIn.setBlock( x +
                 * (new int[]{0, 2, 0, -2})[this.coordBaseMode] + (new int[]{-1,0,1,0}[w]), y, z + (new int[]{-2, 0, 2,
                 * 0})[this.coordBaseMode] + (new int[]{0,1,0,-1}[w]), //Blocks.furnace, (new
                 * int[]{2,5,3,4})[this.coordBaseMode], 2); Blocks.air, (new int[]{3,4,2,5})[this.coordBaseMode], 2); }
                 * // Place the furnace worldIn.setBlock( x + (new int[]{0, 2, 0, -2})[this.coordBaseMode], y, z + (new
                 * int[]{-2, 0, 2, 0})[this.coordBaseMode], //Blocks.furnace, (new int[]{2,5,3,4})[this.coordBaseMode],
                 * 2); Blocks.furnace, (new int[]{3,4,2,5})[this.coordBaseMode], 2); } } } }
                 */
                ...
                /*
                 * //Block blocktofix; for (int i=minX; i<=maxX; i++) { for (int k=minZ; k<=maxZ; k++) { for (int j=48;
                 * j<=71; j++) { if (worldIn.getBlock(i, j, k) == Blocks.furnace) { worldIn.setBlock(i, j, k,
                 * Blocks.furnace, this.coordBaseMode==0?3:this.coordBaseMode==3?0:(this.coordBaseMode+2), 2);
        ...
        /**
         * Called to summon the villager and zombie in the igloo basement , to set the sign text, and to put the potion
