...
public class ClientInfoTracker {
    ...
    /*
     * This map holds packets sent from Server to Client indicating which mob has the extended properties, so the client
     * can load the correct skin / overlay / particles. The info comes from [MetworkHelper.Hanlder] methods and must be
     * loaded when the client executes [EntityJoinWorldEvent]. I can't use the info before that because when the client
     * receives the packet, the local world still haven't loaded all entities (at least on the first load of the world).
     */
    ...
    /**
     * Attempts to locate the entity by its ID and apply the extended properties.
     */
    @SideOnly(Side.CLIENT)
    ...
    public static void syncModernVillagerMessage(int entityID) {
        ...
        // If found the entity, attempt to sync with info sent by the server
        if (entity instanceof EntityVillager && ((EntityVillager) entity).getProfession() >= 0
