 ...
 * 1) Villager is killed and the source of damage is EntityZombie; 2) I intercept [onLivingDeath] and add some info of
 * the villager to this tracker - add(EntityVillager villager); 3) World spawns a zombie villager (firing the
 * [EntityJoinWorldEvent] event) at the exact position where the villager died, ignoring adult/child status (bug?); 4)
 * Players in that region starts tracking the new entity; Here I keep track of all villagers killed by zombies and add
 * them to a list for a certain time. When a zombie villager spawns, I check if their spawn coordinates matches any
 * villager that just died and, if so, copy the information I stored on the EventTracker (mainly profession and custom
 * name). After a zombie spawn, the [PlayerEvent.StartTracking] is fired and I use it to notify the clients about the
 * extended properties that were just copied. ZOMBIE VILLAGER -> VILLAGER 1) Zombie villager starts to be cured (player
 * used potion and golden apple); 2) EntityZombie fires the [startConversion] inner method, setting the conversion
 * counter to something between 3600~6000 ticks; 3) On every entity update, the counter is decreased by the amount
 * defined in [getConversionTimeBoost]; 4) When that counter reaches zero, EntityZombie fires the [convertToVillager]
 * method, creating a new villager on the same location, preserving adult/child status; 5) After creating the new
 * villager, the zombie is removed with [world.removeEntity] and the villager is added with [world.spawnEntityInWorld];
 * Here I listen to the [onLivingUpdateEvent], looking for zombie villagers about to convert. I try to predict the next
 * value of the counter, but since it has a rare random factor (iron bars and beds can give a boost), it's not 100%
 * reliable. When I detect the zombie is about to convert on the next tick update, I add it to the tracker list. I also
 * listen to the [onEntityJoinWorld] and every time a villager joins the world, I check if a zombie villager just
 * converted on that spot so I can copy the info.
 */
...
public class ServerInfoTracker {
    ...
    private static HashMap<Integer, Integer> CuredZombies = new HashMap<Integer, Integer>(); // Tracks which players
                                                                                             // cured zombies (used to
                                                                                             // reward achievements)
    private static int CuredZombiesListLastChange = 0; // Tracks when the list of cured zombies was last updated
    private static HashMap<Integer, Integer> CuredVillagers = new HashMap<Integer, Integer>(); // Tracks a villager just
                                                                                               // cured by a player
    private static int CuredVillagersListLastChange = 0; // Tracks when the list of cured villagers was last updated
    private static final int ListExpiration = 12000; // Number of ticks the list will stay alive after the last update,
                                                     // 12000 = 10 minutes

    private static boolean canStartTracking = false;

    /**
     * Adds villager information that should be tracked on the server. Used to track villager that were killed by
     * zombies.
     */
    ...
    /**
     * Adds zombie information that should be tracked on the server. Used to track zombies villagers that were about to
     * be cured.
     */
    ...
    private static void add(EventType type, EventTracker event) {
        ...
        /*
         * if (GeneralConfigHandler.debugMessages) { LogHelper.info("ServerInfoTracker > Tracking a new event of type ["
         * + type + "] - [" + event + "]"); }
         */
    ...
    {
        /*
         * if (GeneralConfigHandler.debugMessages) { LogHelper.info("ServerInfoTracker > Seeking one event of type [" +
         * type + "] at [" + position + "]"); }
         */
        ...
        switch (type) {
            ...
            case ZOMBIE:
                rangeLimit = 1; // Seek at the exact spot (maybe should be zero?)
                ageTolerance = 3; // Only 3 ticks tolerance to play safe, since the cure should happen on the next 1
                                  // tick
                break;
    ...
    public static void removeCuredZombiesFromTracker(World world, int zombieID) {
        ...
        // && zombieID > 0
            ...
            /*
             * // Attempt to locate the player by the entity ID final Entity entity = world.getEntityByID(playerID);
             * final Entity transformedZombie = world.getEntityByID(zombieID); if ( entity != null && entity instanceof
             * EntityPlayer && transformedZombie != null && transformedZombie instanceof EntityZombie ) { // summon
             * Zombie ~ ~ ~ {IsVillager:1} // If found the player, increment their village reputation EntityPlayer
             * player = (EntityPlayer)entity; EntityZombie zombie = (EntityZombie)transformedZombie; // Nearest village
             * to the player Village villageNearZombie = world.villageCollectionObj.findNearestVillage(
             * (int)zombie.posX, (int)zombie.posY, (int)zombie.posZ, EntityInteractHandler.villageRadiusBuffer); // If
             * player is in or near a village, if (villageNearZombie != null) { try { // Get their reputation... int
             * currentRep = villageNearZombie.getReputationForPlayer(player.getDisplayName()); // Update their
             * reputation villageNearZombie.setReputationForPlayer(player.getDisplayName(), Math.min(currentRep+5, 10));
             * } catch (Exception e) {} } }
             */
    ...
    /**
     * Looks if the infected villager was cured by a player recently to give him/her the achievement.
     * 
    ...
    public static void cleanExpired() {
        ...
        /*
         * // Debug if (GeneralConfigHandler.debugMessages) { LogHelper.info("-- ServerInfoTracker Cleanup [" + thisTick
         * + "] --"); }
         */
        ...
        if (CuredZombiesListLastChange + ListExpiration < thisTick && CuredZombies.size() > 0) {
            /*
             * if (GeneralConfigHandler.debugMessages) { LogHelper.info("resetting cured zombies list"); }
             */
        ...
        if (CuredVillagersListLastChange + ListExpiration < thisTick && CuredVillagers.size() > 0) {
            /*
             * if (GeneralConfigHandler.debugMessages) { LogHelper.info("resetting cured villagers list"); }
             */
    ...
    /*
     * NOTE: This exists to avoid using the tracker before it's actually needed. Not essential after the auto-cleaning
     * was finished, may be removed in the future. e.g. - Tracking player-created iron golem.
     */
    ...
    public static void startTracking() {
        /*
         * if (GeneralConfigHandler.debugMessages) { LogHelper.info("ServerInfoTracker > start tracking"); }
         */
