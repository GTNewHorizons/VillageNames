...
public class EventTracker {
    ...
    public void updateZombie(EntityJoinWorldEvent event, ExtendedZombieVillager ieep) {
        ...
        // Unfortunately, these are client-side only and will crash the server. So we're going to just have to deal with
        // the English versions.
        /*
         * try { PMTMUnloc = I18n.format(Reference.PMTravelingMerchantUnlocalized); PMTMUnlocModern =
         * I18n.format(Reference.PMTravelingMerchantUnlocalizedModern); PMShUnloc =
         * I18n.format(Reference.PMSheepmanUnlocalized); PMShUnlocModern =
         * I18n.format(Reference.PMSheepmanUnlocalizedModern); PMSSUnloc =
        ...
        } else if (profession >= 0) {// Allowing above 4 so that you can have modded professions //&& profession <= 4) {
                                     // // vanilla professions
        ...
        // SkinTone
        if (ieep.getSkinTone() == -99) {
            ieep.setSkinTone(FunctionsVN.returnSkinToneForEntityLocation(zombie));
        } else {
            ieep.setSkinTone(skinTone);
        }
    ...
    public void updateVillager(EntityVillager villager, ExtendedVillager ieep) {
        ...
        } else {// Allowing above 4 so that you can have modded professions //&& profession <= 4) { // vanilla
                // professions
        ...
        // SkinTone
        if (ieep.getSkinTone() == -99) {
            ieep.setSkinTone(FunctionsVN.returnSkinToneForEntityLocation(villager));
        } else {
            ieep.setSkinTone(skinTone);
        }
