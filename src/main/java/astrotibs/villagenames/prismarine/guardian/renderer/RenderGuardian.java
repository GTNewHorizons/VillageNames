...
public class RenderGuardian extends RenderLiving// Extender//<EntityGuardian>
{
     ...
     * Instead, I've slightly replicated the behavior with EntityGuardian.isInRangeToRender3d, however that disregards
     * the frustum completely when the beam is firing. C'EST LA VIE, MON FR�RE
     */

    /*
     * public boolean shouldRender(EntityGuardian livingEntity, ICamera camera, double camX, double camY, double camZ) {
     * LogHelper.info("shouldRender fired"); if (this.RLExtender.shouldRender(livingEntity, camera, camX, camY,
     * camZ))//(super.shouldRender(livingEntity, camera, camX, camY, camZ)) { return true; } else { if
     * (livingEntity.hasTargetedEntity()) { EntityLivingBase entitylivingbase = livingEntity.getTargetedEntity(); if
     * (entitylivingbase != null) { Vec3 vec3 = this.getPosition(entitylivingbase, (double)entitylivingbase.height *
     * 0.5D, 1.0F); Vec3 vec31 = this.getPosition(livingEntity, (double)livingEntity.getEyeHeight(), 1.0F); //if
     * (camera.isBoundingBoxInFrustum(AxisAlignedBB.fromBounds(vec31.xCoord, vec31.yCoord, vec31.zCoord, vec3.xCoord,
     * vec3.yCoord, vec3.zCoord))) double d0 = Math.min(vec31.xCoord, vec3.xCoord); double d1 = Math.min(vec31.yCoord,
     * vec3.yCoord); double d2 = Math.min(vec31.zCoord, vec3.zCoord); double d3 = Math.max(vec31.xCoord, vec3.xCoord);
     * double d4 = Math.max(vec31.yCoord, vec3.yCoord); double d5 = Math.max(vec31.zCoord, vec3.zCoord); if (
     * camera.isBoundingBoxInFrustum(AxisAlignedBB.getBoundingBox(d0, d1, d2, d3, d4, d5)) ) { return true; } } } return
     * false; } }
     */
     ...
     * LogHelper.info("getEntityTexture(EntityGuardian) fired"); return getEntityTexture((Entity)entity); }
     */
    // Fires
