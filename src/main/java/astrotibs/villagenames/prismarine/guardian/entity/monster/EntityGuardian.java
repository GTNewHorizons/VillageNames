...
public class EntityGuardian extends EntityMob // implements IMob
{
    ...
    /**
     * This version returns "true" if either the guardian OR its beam target are in range. It also similarly turns the
     * frustum check requirement on and off.
     */
    @SideOnly(Side.CLIENT)
    @Override
    ...
    /*
     * public boolean anyLiquidInBB() { double scaleDown = 0.2D; // How many blocks to contract the BB for liquid
     * detection return this.worldObj.isAnyLiquid( this.boundingBox.contract( 0.0D, scaleDown, 0.0D ) ); }
     */

    /*
     * Added this because the navigator, move helper, and look helper were all manually re-written. Ordinarily, these
     * equivalents would be run during updateAITasks, so I'm calling them here.
     */
    @Override
    ...
    /*
     * In 1.7 EntityLivingBase.onLivingUpdate() calls tasks based on isAIEnabled(): TRUE: calls this.updateAITasks() -
     * THIS calls updateAITick(), which allows the Guardian to sink and to beam-attack! FALSE: calls
     * this.updateEntityActionState(), which has no function within 1.8, or explicitly here. In 1.8, EntityLivingBase
     * always fires (the equivalent of) updateAITick() and doesn't check on isAIEnabled(). Thus, to get the behavior I
     * want, I might as well leave it "true" for all Guardians, prompting me to add it as the method below.
     */
    /**
    ...
    /*
     * In 1.8: This is called updateAITasks(), which is a blank method in EntityLiving. Both animals and monsters use
     * updateAITasks() to do various things. Iron Golems determine if they're in a village. Villagers determine if
     * they're in a village and get a trading list if they don't have one. Bats check whether they're hanging and if
     * there's a player nearby. All versions also call super.updateAITasks(). EntityLiving.updateAITasks() is called
     * during EntityLiving.updateEntityActionState(). EntityLiving.updateEntityActionState() is called during
     * EntityLivingBase.onLivingUpdate() 1.8 chain of command is: EntityLiving.onLivingUpdate() >
     * EntityLivingBase.onLivingUpdate() > EntityLiving.updateEntityActionState() > EntityGuardian.updateAITasks() In
     * 1.7: There is no equivalent method in EntityLiving, but EntityLiving.updateAITasks() is functionally identical to
     * 1.8's EntityLiving.updateEntityActionState(). HOWEVER, unlike 1.8, there actually is code in 1.7's
     * EntityLiving.updateEntityActionState(). HoweverEVER, 1.8's EntityLiving.updateEntityActionState() block calls
     * this.updateAITasks(), which in 1.7 is EntityLiving.updateAITasks() calling this.updateAITick(). 1.7 chain of
     * command is: EntityLiving.onLivingUpdate() > EntityLivingBase.onLivingUpdate() > EntityLiving.updateAITasks() >
     * below If isAIEnabled(), EntityLivingBase calls updateAITasks(). Otherwise, it runs updateEntityActionState().
     */
    protected void updateAITick() // Changed in 1.8 to updateEntityActionState()
    {
        /*
         * FLOATS WHEN: isAIEnabled()==false, this method is updateEntityActionState(), this calls
         * super.updateEntityActionState() isAIEnabled()==false, this method is updateAITick(), this calls
         * super.updateAITick() isAIEnabled()==false, this method is updateAITick(), this calls
         * super.updateEntityActionState() SINKS WHEN: isAIEnabled()==true, this method is updateEntityActionState(),
         * this calls super.updateEntityActionState() isAIEnabled()==true, this method is updateEntityActionState(),
         * this calls super.updateAITick() isAIEnabled()==false, this method is updateEntityActionState(), this calls
         * super.updateAITick() isAIEnabled()==true, this method is updateAITick(), this calls super.updateAITick()
         * <-----------------THIS ONE! isAIEnabled()==true, this method is updateAITick(), this calls
    ...
    /**
     * Beth helped me with this because idk wut me doing This mimicks the insertion of
     * this.gameController.getSoundHandler().playSound(new GuardianSound((EntityGuardian)entity)) into
     * NetPlayHandleClient.handleEntityStatus() in 1.8
     */
    @Override
     ...
     */
    /*
     * public boolean isNotColliding() //Crashes for some reason, so I commented it out. { return
     * this.worldObj.checkNoEntityCollision(this.getBoundingBox(), this) &&
     * this.worldObj.getCollidingBoundingBoxes(this, this.getBoundingBox()).isEmpty(); }
     */

    /*
     * In 1.8, the Guardian has both isNotColliding() AND getCanSpawnHere(). In 1.7, getCanSpawnHere() does exactly what
     * isNotColliding() does in 1.8. However, MobSpawnerBaseLogic now checks for BOTH, instead of just ONE in 1.7. My
     * solution to this is to rename isNotColliding() to getCanSpawnHere(), but also to call super AND manually do the
     * stuff in 1.8's getCanSpawnHere(), which adds an additional random element to the spawning.
     */
    /**
    ...
    /**
     * Manually porting World.canBlockSeeSky from 1.8 Returns true if the block is above sea level and this.canSeeSky,
     * which in 1.7 is canBlockSeeTheSky Returns false if the block is above sea level and !this.canSeeSky Returns false
     * if the block is below sea level, and the sea-level block above it !canSeeSky Returns false if the block is below
     * sea level, and the sea-level block above it canSeeSky, and any blocks between them both have opacity > 0 and
     * aren't liquid Returns true if the block is below sea level, and the sea-level block above it canSeeSky, and no
     * blocks between them both have opacity > 0 and aren't liquid
     */
    ...
    /*
     * Added by me, similar to GuardianLookHelper 1.8 EntityLookHelper is IDENTICAL to 1.7, except they added isLooking
     * and posX getters.
     */
