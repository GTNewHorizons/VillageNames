...
/**
 * Copied over practically wholesale from Et Futurum, with minimal changes:
 * https://github.com/ganymedes01/Et-Futurum/blob/master/src/main/java/ganymedes01/etfuturum/tileentities/TileEntityBanner.java
 * 
 * @author AstroTibs
 *
 */
public class TileEntityBanner extends TileEntity {
    ...
    public void writeToNBT(NBTTagCompound nbt) {
        ...
        if (patterns != null) nbt.setTag("Patterns", patterns);
    }
    ...
    public void onDataPacket(NetworkManager net, S35PacketUpdateTileEntity pkt) {
        if (pkt.func_148853_f() == 0) readFromNBT(pkt.func_148857_g());
    }
    ...
    private void initializeBannerData() {
        if (patternList == null || colorList == null || field_175121_j == null)
            if (!field_175119_g) field_175121_j = "";
            else {
    ...
    public ItemStack createStack() {
        ItemStack stack = ModObjects.chooseModBannerItem();// new ItemStack(ModBlocks.banner, 1, getBaseColor());
    ...
    public static enum EnumBannerPattern {
        ...
        public static EnumBannerPattern getPatternByID(String id) {
            for (EnumBannerPattern pattern : EnumBannerPattern.values())
                if (pattern.patternID.equals(id)) return pattern;
            return null;
        }
    }
}
