...
public class VillageGeneratorConfigHandler {
    ...
    // Town centers
    public static int componentModernPlainsFountain, componentModernPlainsWell, componentModernPlainsMarket,
            componentModernPlainsOakTree, componentModernDesertFountain, componentModernDesertWell,
            componentModernDesertMarket, componentModernTaigaSquare, componentModernTaigaWell,
            componentModernSavannaMarket, componentModernSavannaFountain, componentModernSavannaDoubleWell,
            componentModernSavannaWell, componentModernSnowyIceSpire, componentModernSnowyFountain,
            componentModernSnowyPavilion, componentModernJungleStatue, componentModernJungleCocoaTree,
            componentModernJungleGarden, componentModernJungleVilla, componentModernSwampWillow,
            componentModernSwampStatue, componentModernSwampPavilion, componentModernSwampMonolith;
    ...
    /*
     * public static boolean decorPlainsLamp1; // Four torches on elevated stripped wood public static boolean
     * decorDesertLamp1; // Torch on terracotta atop cut sandstone column public static boolean decorTaigaDecoration1;
     * // Trough public static boolean decorTaigaDecoration2; // Large Boulder public static boolean
     * decorTaigaDecoration3; // Small Boulder public static boolean decorTaigaDecoration4; // Medium Boulder public
     * static boolean decorTaigaDecoration5; // Campfire public static boolean decorTaigaDecoration6; // Campfire over
     * hay bin public static boolean decorTaigaDecoration7; // Torch on cobblestone wall public static boolean
     * decorSavannaLampPost01; // Torch on a fence public static boolean decorSnowyLampPost1; // Two lanterns public
     * static boolean decorSnowyLampPost02; // One lantern public static boolean decorSnowyLampPost03; // Four lanterns
     */
    ...
    public static void loadConfiguration() {
        ...
        /*
         * String decor = "Decor: "; String allowForThisDecorTypeIn = "Allow this decor type in ";
         */
        ...
        newVillageGenerator = config.getBoolean(
                "Activate New Village Generator",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                true,
                "Use replacement village generation system. You may need to deactivate village generation from other mods. All other settings in this section require this to be true.");

        // --- Village Size stuff --- //
        newVillageSizeMinimum = config.getFloat(
                "Village Size: Minimum",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                1.0F,
                1,
                5,
                "Lower limit for the randomly-selected size of a village. Vanilla is 1.");
        newVillageSizeMaximum = config.getFloat(
                "Village Size: Maximum",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                1.0F,
                1,
                5,
                "Upper limit for the randomly-selected size of a village. Vanilla is 1.");
        ...
        newVillageSizeNormalOrder = config.getInt(
                "Village Size: Normal Order",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                1,
                1,
                10,
                "Village size is drawn from a convolution of this many uniform distributions."
        ...
        newVillageSizeMode = config.getFloat(
                "Village Size: Mode",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                1.0F,
                1,
        ...
        if (newVillageSizeMode < newVillageSizeMinimum || newVillageSizeMode > newVillageSizeMaximum) {
            LogHelper.error(
                    "Village Size: Mode can't be outside of the range [minimum, maximum]. Setting to the midpoint between these.");
            newVillageSizeMode = (newVillageSizeMinimum + newVillageSizeMaximum) / 2F;
        }

        newVillageSpacingMedian = config.getInt(
                "Village Spacing: Median",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                20,
                1,
                100,
                "Median distance between villages. Vanilla is 20.");
        newVillageSpacingSpread = config.getInt(
                "Village Spacing: Range",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                12,
                1,
                100,
                "Variation in distances between villages. Must be lower than Median value. Vanilla is 12.");
        noVillagesRadius = config.getInt(
                "Village Spacing: Village-Free Radius",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                0,
                0,
                100000,
                "No villages will spawn less than this many chunks from world origin (0, 0).");
        // farmPumpkins = config.getBoolean("Pumpkin and Melon Crops", Reference.CATEGORY_VILLAGE_GENERATOR, true,
        // "Farms can have pumpkins and melons generate in them");
        ...
        componentLegacyHouse4Garden_string = config.getString(
                componentLegacy + "Small House",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(ald),
                generationStatsForL + "4.0");
        ...
        componentLegacyChurch_string = config.getString(
                componentLegacy + "Church",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(ald),
                generationStatsForL + "20.0");
        ...
        componentLegacyHouse1_string = config.getString(
                componentLegacy + "Library",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(ald),
                generationStatsForL + "20.0");
        ...
        componentLegacyWoodHut_string = config.getString(
                componentLegacy + "Hut",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(ald),
                generationStatsForL + "3.0");
        componentLegacyWoodHut_vals = parseDoubleArray(componentLegacyWoodHut_string, ald);

        ald = new ArrayList<Double>(Arrays.asList(0D, 1D, 0D, 1D, 2D));
        componentLegacyHall_string = config.getString(
                componentLegacy + "Butcher Shop",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(ald),
                generationStatsForL + "15.0");
        ...
        componentLegacyField1_string = config.getString(
                componentLegacy + "Large Farm",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(ald),
                generationStatsForL + "3.0");
        ...
        ald = new ArrayList<Double>(Arrays.asList(0D, 1D, 2D, 2D, 4D));
        componentLegacyField2_string = config.getString(
                componentLegacy + "Small Farm",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(ald),
                generationStatsForL + "3.0");
        ...
        componentLegacyHouse2_string = config.getString(
                componentLegacy + "Smithy",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(ald),
                generationStatsForL + "15.0");
        ...
        componentLegacyHouse3_string = config.getString(
                componentLegacy + "Large House",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(ald),
                generationStatsForL + "8.0");
        componentLegacyHouse3_vals = parseDoubleArray(componentLegacyHouse3_string, ald);

        // Modern Village components

        // Town centers
        componentModernPlainsFountain = config.getInt(
                componentModern + "Plains " + townCenter + "Fountain",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                1,
                0,
                10000,
        ...
        componentModernPlainsWell = config.getInt(
                componentModern + "Plains " + townCenter + "Well",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                1,
                0,
                10000,
        ...
        componentModernPlainsMarket = config.getInt(
                componentModern + "Plains " + townCenter + "Market",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                1,
                0,
                10000,
        ...
        componentModernPlainsOakTree = config.getInt(
                componentModern + "Plains " + townCenter + "Oak Tree",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                1,
                0,
                10000,
        ...
        componentModernDesertFountain = config.getInt(
                componentModern + "Desert " + townCenter + "Fountain",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                1,
                0,
                10000,
        ...
        componentModernDesertWell = config.getInt(
                componentModern + "Desert " + townCenter + "Well",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                1,
                0,
                10000,
        ...
        componentModernDesertMarket = config.getInt(
                componentModern + "Desert " + townCenter + "Market",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                1,
                0,
                10000,
        ...
        componentModernTaigaSquare = config.getInt(
                componentModern + "Taiga " + townCenter + "Square",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                1,
                0,
                10000,
        ...
        componentModernTaigaWell = config.getInt(
                componentModern + "Taiga " + townCenter + "Well",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                1,
                0,
                10000,
        ...
        componentModernSavannaMarket = config.getInt(
                componentModern + "Savanna " + townCenter + "Market",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                1,
                0,
                10000,
        ...
        componentModernSavannaFountain = config.getInt(
                componentModern + "Savanna " + townCenter + "Fountain",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                1,
                0,
                10000,
        ...
        componentModernSavannaDoubleWell = config.getInt(
                componentModern + "Savanna " + townCenter + "Double Well",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                1,
                0,
                10000,
        ...
        componentModernSavannaWell = config.getInt(
                componentModern + "Savanna " + townCenter + "Well",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                1,
                0,
                10000,
        ...
        componentModernSnowyIceSpire = config.getInt(
                componentModern + "Snowy " + townCenter + "Ice Spire",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                1,
                0,
                10000,
        ...
        componentModernSnowyFountain = config.getInt(
                componentModern + "Snowy " + townCenter + "Fountain",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                1,
                0,
                10000,
        ...
        componentModernSnowyPavilion = config.getInt(
                componentModern + "Snowy " + townCenter + "Pavilion",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                1,
                0,
                10000,
        ...
        componentModernJungleStatue = config.getInt(
                componentModern + "Jungle " + townCenter + "Statue",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                1,
                0,
                10000,
        ...
        componentModernJungleCocoaTree = config.getInt(
                componentModern + "Jungle " + townCenter + "Cocoa Tree",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                1,
                0,
                10000,
        ...
        componentModernJungleGarden = config.getInt(
                componentModern + "Jungle " + townCenter + "Garden",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                1,
                0,
                10000,
        ...
        componentModernJungleVilla = config.getInt(
                componentModern + "Jungle " + townCenter + "Villa",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                1,
                0,
                10000,
        ...
        componentModernSwampWillow = config.getInt(
                componentModern + "Swamp " + townCenter + "Willow",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                6,
                0,
                10000,
        ...
        componentModernSwampStatue = config.getInt(
                componentModern + "Swamp " + townCenter + "Statue",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                1,
                0,
                10000,
        ...
        componentModernSwampPavilion = config.getInt(
                componentModern + "Swamp " + townCenter + "Pavilion",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                6,
                0,
                10000,
        ...
        componentModernSwampMonolith = config.getInt(
                componentModern + "Swamp " + townCenter + "Monolith",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                3,
                0,
                10000,
        ...
        ArrayList<Double> modernDefaults = new ArrayList<Double>(
                Arrays.asList(
        ...
        // ArrayList<Double> modernDefaults = new ArrayList<Double>(Arrays.asList(10D, 1D, 1D, 2D, 3D)); // Placeholder
        // to ensure spawn
        ...
        double desertStreetToHouseRatio = 15D / 17D; // 17 house attachments and 28 street attachments across 11 streets
                                                     // and 2 street endcaps = (28-(11+2))/17 = 15/17

        double taigaDecorToHouseRatio = 30D / 18D;

        double savannaDecorToHouseRatio = 1D;
        double savannaStreetToHouseRatio = 31D / 17D; // 17 house attachments and 51 street attachments across 19
                                                      // streets and 1 street endcaps = (51-(19+1))/17 = 31/17
        ...
        componentModernPlainsAccessory1_string = config.getString(
                componentModern + "Plains Flower Planter",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + plainsVillages);
        componentModernPlainsAccessory1_vals = parseDoubleArray(componentModernPlainsAccessory1_string, modernDefaults);

        componentModernPlainsAnimalPen1_string = config.getString(
                componentModern + "Plains Small Animal Pen",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + plainsVillages);
        componentModernPlainsAnimalPen1_vals = parseDoubleArray(componentModernPlainsAnimalPen1_string, modernDefaults);

        componentModernPlainsAnimalPen2_string = config.getString(
                componentModern + "Plains Large Animal Pen",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + plainsVillages);
        componentModernPlainsAnimalPen2_vals = parseDoubleArray(componentModernPlainsAnimalPen2_string, modernDefaults);

        componentModernPlainsAnimalPen3_string = config.getString(
                componentModern + "Plains Decorated Animal Pen",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + plainsVillages);
        componentModernPlainsAnimalPen3_vals = parseDoubleArray(componentModernPlainsAnimalPen3_string, modernDefaults);

        componentModernPlainsArmorerHouse1_string = config.getString(
                componentModern + "Plains Armorer House",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + plainsVillages);
        componentModernPlainsArmorerHouse1_vals = parseDoubleArray(
                componentModernPlainsArmorerHouse1_string,
                modernDefaults);

        componentModernPlainsBigHouse1_string = config.getString(
                componentModern + "Plains Large House",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + plainsVillages);
        componentModernPlainsBigHouse1_vals = parseDoubleArray(componentModernPlainsBigHouse1_string, modernDefaults);

        componentModernPlainsButcherShop1_string = config.getString(
                componentModern + "Plains Small Butcher Shop",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + plainsVillages);
        componentModernPlainsButcherShop1_vals = parseDoubleArray(
                componentModernPlainsButcherShop1_string,
                modernDefaults);

        componentModernPlainsButcherShop2_string = config.getString(
                componentModern + "Plains Large Butcher Shop",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + plainsVillages);
        componentModernPlainsButcherShop2_vals = parseDoubleArray(
                componentModernPlainsButcherShop2_string,
                modernDefaults);

        componentModernPlainsCartographer1_string = config.getString(
                componentModern + "Plains Cartographer House",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + plainsVillages);
        componentModernPlainsCartographer1_vals = parseDoubleArray(
                componentModernPlainsCartographer1_string,
                modernDefaults);

        componentModernPlainsFisherCottage1_string = config.getString(
                componentModern + "Plains Fisher Cottage",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + plainsVillages);
        componentModernPlainsFisherCottage1_vals = parseDoubleArray(
                componentModernPlainsFisherCottage1_string,
                modernDefaults);

        componentModernPlainsFletcherHouse1_string = config.getString(
                componentModern + "Plains Fletcher House",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + plainsVillages);
        componentModernPlainsFletcherHouse1_vals = parseDoubleArray(
                componentModernPlainsFletcherHouse1_string,
                modernDefaults);

        componentModernPlainsLargeFarm1_string = config.getString(
                componentModern + "Plains Large Farm",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + plainsVillages);
        componentModernPlainsLargeFarm1_vals = parseDoubleArray(componentModernPlainsLargeFarm1_string, modernDefaults);

        componentModernPlainsLibrary1_string = config.getString(
                componentModern + "Plains Large Library",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + plainsVillages);
        componentModernPlainsLibrary1_vals = parseDoubleArray(componentModernPlainsLibrary1_string, modernDefaults);

        componentModernPlainsLibrary2_string = config.getString(
                componentModern + "Plains Small Library",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + plainsVillages);
        componentModernPlainsLibrary2_vals = parseDoubleArray(componentModernPlainsLibrary2_string, modernDefaults);

        componentModernPlainsMasonsHouse1_string = config.getString(
                componentModern + "Plains Mason House",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + plainsVillages);
        componentModernPlainsMasonsHouse1_vals = parseDoubleArray(
                componentModernPlainsMasonsHouse1_string,
                modernDefaults);

        componentModernPlainsMediumHouse1_string = config.getString(
                componentModern + "Plains Medium House 1",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + plainsVillages);
        componentModernPlainsMediumHouse1_vals = parseDoubleArray(
                componentModernPlainsMediumHouse1_string,
                modernDefaults);

        componentModernPlainsMediumHouse2_string = config.getString(
                componentModern + "Plains Medium House 2",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + plainsVillages);
        componentModernPlainsMediumHouse2_vals = parseDoubleArray(
                componentModernPlainsMediumHouse2_string,
                modernDefaults);

        componentModernPlainsMeetingPoint4_string = config.getString(
                componentModern + "Plains Large Market",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + plainsVillages);
        componentModernPlainsMeetingPoint4_vals = parseDoubleArray(
                componentModernPlainsMeetingPoint4_string,
                modernDefaults);

        componentModernPlainsMeetingPoint5_string = config.getString(
                componentModern + "Plains Small Market",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + plainsVillages);
        componentModernPlainsMeetingPoint5_vals = parseDoubleArray(
                componentModernPlainsMeetingPoint5_string,
                modernDefaults);

        componentModernPlainsShepherdsHouse1_string = config.getString(
                componentModern + "Plains Shepherd House",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + plainsVillages);
        componentModernPlainsShepherdsHouse1_vals = parseDoubleArray(
                componentModernPlainsShepherdsHouse1_string,
                modernDefaults);

        componentModernPlainsSmallFarm1_string = config.getString(
                componentModern + "Plains Small Farm",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + plainsVillages);
        componentModernPlainsSmallFarm1_vals = parseDoubleArray(componentModernPlainsSmallFarm1_string, modernDefaults);

        componentModernPlainsSmallHouse1_string = config.getString(
                componentModern + "Plains Small House 1",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + plainsVillages);
        componentModernPlainsSmallHouse1_vals = parseDoubleArray(
                componentModernPlainsSmallHouse1_string,
                modernDefaults);

        componentModernPlainsSmallHouse2_string = config.getString(
                componentModern + "Plains Small House 2",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + plainsVillages);
        componentModernPlainsSmallHouse2_vals = parseDoubleArray(
                componentModernPlainsSmallHouse2_string,
                modernDefaults);

        componentModernPlainsSmallHouse3_string = config.getString(
                componentModern + "Plains Small House 3",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + plainsVillages);
        componentModernPlainsSmallHouse3_vals = parseDoubleArray(
                componentModernPlainsSmallHouse3_string,
                modernDefaults);

        componentModernPlainsSmallHouse4_string = config.getString(
                componentModern + "Plains Small House 4",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + plainsVillages);
        componentModernPlainsSmallHouse4_vals = parseDoubleArray(
                componentModernPlainsSmallHouse4_string,
                modernDefaults);

        componentModernPlainsSmallHouse5_string = config.getString(
                componentModern + "Plains Small House 5",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + plainsVillages);
        componentModernPlainsSmallHouse5_vals = parseDoubleArray(
                componentModernPlainsSmallHouse5_string,
                modernDefaults);

        componentModernPlainsSmallHouse6_string = config.getString(
                componentModern + "Plains Small House 6",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + plainsVillages);
        componentModernPlainsSmallHouse6_vals = parseDoubleArray(
                componentModernPlainsSmallHouse6_string,
                modernDefaults);

        componentModernPlainsSmallHouse7_string = config.getString(
                componentModern + "Plains Small House 7",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + plainsVillages);
        componentModernPlainsSmallHouse7_vals = parseDoubleArray(
                componentModernPlainsSmallHouse7_string,
                modernDefaults);

        componentModernPlainsSmallHouse8_string = config.getString(
                componentModern + "Plains Small House 8",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + plainsVillages);
        componentModernPlainsSmallHouse8_vals = parseDoubleArray(
                componentModernPlainsSmallHouse8_string,
                modernDefaults);

        componentModernPlainsStable1_string = config.getString(
                componentModern + "Plains Cobblestone Stable",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + plainsVillages);
        componentModernPlainsStable1_vals = parseDoubleArray(componentModernPlainsStable1_string, modernDefaults);

        componentModernPlainsStable2_string = config.getString(
                componentModern + "Plains Terracotta Stable",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + plainsVillages);
        componentModernPlainsStable2_vals = parseDoubleArray(componentModernPlainsStable2_string, modernDefaults);

        componentModernPlainsTannery1_string = config.getString(
                componentModern + "Plains Tannery",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + plainsVillages);
        componentModernPlainsTannery1_vals = parseDoubleArray(componentModernPlainsTannery1_string, modernDefaults);

        componentModernPlainsTemple3_string = config.getString(
                componentModern + "Plains Terracotta Temple",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + plainsVillages);
        componentModernPlainsTemple3_vals = parseDoubleArray(componentModernPlainsTemple3_string, modernDefaults);

        componentModernPlainsTemple4_string = config.getString(
                componentModern + "Plains Cobblestone Temple",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + plainsVillages);
        componentModernPlainsTemple4_vals = parseDoubleArray(componentModernPlainsTemple4_string, modernDefaults);

        componentModernPlainsToolSmith1_string = config.getString(
                componentModern + "Plains Tool Smithy",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + plainsVillages);
        componentModernPlainsToolSmith1_vals = parseDoubleArray(componentModernPlainsToolSmith1_string, modernDefaults);

        componentModernPlainsWeaponsmith1_string = config.getString(
                componentModern + "Plains Weapon Smithy",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + plainsVillages);
        componentModernPlainsWeaponsmith1_vals = parseDoubleArray(
                componentModernPlainsWeaponsmith1_string,
                modernDefaults);

        modifiedDefaults.set(0, modernDefaults.get(0) * 9D * plainsDecorToHouseRatio);
        for (int i = 1; i < modernDefaults.size(); i++) {
            modifiedDefaults.set(i, modernDefaults.get(i) * plainsHouses * plainsDecorToHouseRatio);
        }
        componentModernPlainsStreetDecor1_string = config.getString(
                componentModern + "Plains Road Decor",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modifiedDefaults),
                generationStatsForM + plainsVillages);
        componentModernPlainsStreetDecor1_vals = parseDoubleArray(
                componentModernPlainsStreetDecor1_string,
                modifiedDefaults);

        // Desert components
        componentModernDesertAnimalPen1_string = config.getString(
                componentModern + "Desert Small Animal Pen",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + desertVillages);
        componentModernDesertAnimalPen1_vals = parseDoubleArray(componentModernDesertAnimalPen1_string, modernDefaults);

        componentModernDesertAnimalPen2_string = config.getString(
                componentModern + "Desert Covered Animal Pen",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + desertVillages);
        componentModernDesertAnimalPen2_vals = parseDoubleArray(componentModernDesertAnimalPen2_string, modernDefaults);

        componentModernDesertArmorer1_string = config.getString(
                componentModern + "Desert Armorer House",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + desertVillages);
        componentModernDesertArmorer1_vals = parseDoubleArray(componentModernDesertArmorer1_string, modernDefaults);

        componentModernDesertButcherShop1_string = config.getString(
                componentModern + "Desert Butcher Shop",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + desertVillages);
        componentModernDesertButcherShop1_vals = parseDoubleArray(
                componentModernDesertButcherShop1_string,
                modernDefaults);

        componentModernDesertCartographerHouse1_string = config.getString(
                componentModern + "Desert Cartographer House",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + desertVillages);
        componentModernDesertCartographerHouse1_vals = parseDoubleArray(
                componentModernDesertCartographerHouse1_string,
                modernDefaults);

        componentModernDesertFarm1_string = config.getString(
                componentModern + "Desert Small Farm",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + desertVillages);
        componentModernDesertFarm1_vals = parseDoubleArray(componentModernDesertFarm1_string, modernDefaults);

        componentModernDesertFarm2_string = config.getString(
                componentModern + "Desert Medium Farm",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + desertVillages);
        componentModernDesertFarm2_vals = parseDoubleArray(componentModernDesertFarm2_string, modernDefaults);

        componentModernDesertFisher1_string = config.getString(
                componentModern + "Desert Fisher Cottage",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + desertVillages);
        componentModernDesertFisher1_vals = parseDoubleArray(componentModernDesertFisher1_string, modernDefaults);

        componentModernDesertFletcherHouse1_string = config.getString(
                componentModern + "Desert Fletcher House",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + desertVillages);
        componentModernDesertFletcherHouse1_vals = parseDoubleArray(
                componentModernDesertFletcherHouse1_string,
                modernDefaults);

        componentModernDesertLargeFarm1_string = config.getString(
                componentModern + "Desert Large Farm",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + desertVillages);
        componentModernDesertLargeFarm1_vals = parseDoubleArray(componentModernDesertLargeFarm1_string, modernDefaults);

        componentModernDesertLibrary1_string = config.getString(
                componentModern + "Desert Library",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + desertVillages);
        componentModernDesertLibrary1_vals = parseDoubleArray(componentModernDesertLibrary1_string, modernDefaults);

        componentModernDesertMason1_string = config.getString(
                componentModern + "Desert Mason House",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + desertVillages);
        componentModernDesertMason1_vals = parseDoubleArray(componentModernDesertMason1_string, modernDefaults);

        componentModernDesertMediumHouse1_string = config.getString(
                componentModern + "Desert Medium House",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + desertVillages);
        componentModernDesertMediumHouse1_vals = parseDoubleArray(
                componentModernDesertMediumHouse1_string,
                modernDefaults);

        componentModernDesertMediumHouse2_string = config.getString(
                componentModern + "Desert Large House",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + desertVillages);
        componentModernDesertMediumHouse2_vals = parseDoubleArray(
                componentModernDesertMediumHouse2_string,
                modernDefaults);

        componentModernDesertShepherdHouse1_string = config.getString(
                componentModern + "Desert Shepherd House",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + desertVillages);
        componentModernDesertShepherdHouse1_vals = parseDoubleArray(
                componentModernDesertShepherdHouse1_string,
                modernDefaults);

        componentModernDesertSmallHouse1_string = config.getString(
                componentModern + "Desert Small House 1",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + desertVillages);
        componentModernDesertSmallHouse1_vals = parseDoubleArray(
                componentModernDesertSmallHouse1_string,
                modernDefaults);

        componentModernDesertSmallHouse2_string = config.getString(
                componentModern + "Desert Small House 2",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + desertVillages);
        componentModernDesertSmallHouse2_vals = parseDoubleArray(
                componentModernDesertSmallHouse2_string,
                modernDefaults);

        componentModernDesertSmallHouse3_string = config.getString(
                componentModern + "Desert Small House 3",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + desertVillages);
        componentModernDesertSmallHouse3_vals = parseDoubleArray(
                componentModernDesertSmallHouse3_string,
                modernDefaults);

        componentModernDesertSmallHouse4_string = config.getString(
                componentModern + "Desert Small House 4",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + desertVillages);
        componentModernDesertSmallHouse4_vals = parseDoubleArray(
                componentModernDesertSmallHouse4_string,
                modernDefaults);

        componentModernDesertSmallHouse5_string = config.getString(
                componentModern + "Desert Small House 5",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + desertVillages);
        componentModernDesertSmallHouse5_vals = parseDoubleArray(
                componentModernDesertSmallHouse5_string,
                modernDefaults);

        componentModernDesertSmallHouse6_string = config.getString(
                componentModern + "Desert Small House 6",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + desertVillages);
        componentModernDesertSmallHouse6_vals = parseDoubleArray(
                componentModernDesertSmallHouse6_string,
                modernDefaults);

        componentModernDesertSmallHouse7_string = config.getString(
                componentModern + "Desert Small House 7",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + desertVillages);
        componentModernDesertSmallHouse7_vals = parseDoubleArray(
                componentModernDesertSmallHouse7_string,
                modernDefaults);

        componentModernDesertSmallHouse8_string = config.getString(
                componentModern + "Desert Small House 8",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + desertVillages);
        componentModernDesertSmallHouse8_vals = parseDoubleArray(
                componentModernDesertSmallHouse8_string,
                modernDefaults);

        componentModernDesertTannery1_string = config.getString(
                componentModern + "Desert Tannery",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + desertVillages);
        componentModernDesertTannery1_vals = parseDoubleArray(componentModernDesertTannery1_string, modernDefaults);

        componentModernDesertTemple1_string = config.getString(
                componentModern + "Desert Temple 1",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + desertVillages);
        componentModernDesertTemple1_vals = parseDoubleArray(componentModernDesertTemple1_string, modernDefaults);

        componentModernDesertTemple2_string = config.getString(
                componentModern + "Desert Temple 2",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + desertVillages);
        componentModernDesertTemple2_vals = parseDoubleArray(componentModernDesertTemple2_string, modernDefaults);

        componentModernDesertToolSmith1_string = config.getString(
                componentModern + "Desert Tool Smithy",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + desertVillages);
        componentModernDesertToolSmith1_vals = parseDoubleArray(componentModernDesertToolSmith1_string, modernDefaults);

        componentModernDesertWeaponsmith1_string = config.getString(
                componentModern + "Desert Weapon Smithy",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + desertVillages);
        componentModernDesertWeaponsmith1_vals = parseDoubleArray(
                componentModernDesertWeaponsmith1_string,
                modernDefaults);

        modifiedDefaults.set(0, modernDefaults.get(0) * 9D * desertDecorToHouseRatio);
        for (int i = 1; i < modernDefaults.size(); i++) {
            modifiedDefaults.set(i, modernDefaults.get(i) * desertHouses * desertDecorToHouseRatio);
        }
        componentModernDesertStreetDecor1_string = config.getString(
                componentModern + "Desert Road Decor 1",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modifiedDefaults),
                generationStatsForM + desertVillages);
        componentModernDesertStreetDecor1_vals = parseDoubleArray(
                componentModernDesertStreetDecor1_string,
                modifiedDefaults);

        modifiedDefaults.set(0, modernDefaults.get(0) * 9D * desertStreetToHouseRatio / desertStreetsAndEndcaps);
        for (int i = 1; i < modernDefaults.size(); i++) {
            modifiedDefaults
                    .set(i, modernDefaults.get(i) * desertHouses * desertStreetToHouseRatio / desertStreetsAndEndcaps);
        }
        componentModernDesertStreetSubstitute1_string = config.getString(
                componentModern + "Desert Road Terracotta Accent 1",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modifiedDefaults),
                generationStatsForM + desertVillages);
        componentModernDesertStreetSubstitute1_vals = parseDoubleArray(
                componentModernDesertStreetSubstitute1_string,
                modifiedDefaults);
        ...
        componentModernDesertStreetSubstitute2_string = config.getString(
                componentModern + "Desert Road Terracotta Accent 2",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modifiedDefaults),
                generationStatsForM + desertVillages);
        componentModernDesertStreetSubstitute2_vals = parseDoubleArray(
                componentModernDesertStreetSubstitute2_string,
                modifiedDefaults);
        ...
        componentModernDesertStreetSubstitute3_string = config.getString(
                componentModern + "Desert Road Decor 2",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modifiedDefaults),
                generationStatsForM + desertVillages);
        componentModernDesertStreetSubstitute3_vals = parseDoubleArray(
                componentModernDesertStreetSubstitute3_string,
                modifiedDefaults);

        // Taiga components
        componentModernTaigaAnimalPen1_string = config.getString(
                componentModern + "Taiga Animal Pen",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + taigaVillages);
        componentModernTaigaAnimalPen1_vals = parseDoubleArray(componentModernTaigaAnimalPen1_string, modernDefaults);

        componentModernTaigaArmorer2_string = config.getString(
                componentModern + "Taiga Armorer Station",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + taigaVillages);
        componentModernTaigaArmorer2_vals = parseDoubleArray(componentModernTaigaArmorer2_string, modernDefaults);

        componentModernTaigaArmorerHouse1_string = config.getString(
                componentModern + "Taiga Armorer House",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + taigaVillages);
        componentModernTaigaArmorerHouse1_vals = parseDoubleArray(
                componentModernTaigaArmorerHouse1_string,
                modernDefaults);

        componentModernTaigaButcherShop1_string = config.getString(
                componentModern + "Taiga Butcher Shop",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + taigaVillages);
        componentModernTaigaButcherShop1_vals = parseDoubleArray(
                componentModernTaigaButcherShop1_string,
                modernDefaults);

        componentModernTaigaCartographerHouse1_string = config.getString(
                componentModern + "Taiga Cartographer House",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + taigaVillages);
        componentModernTaigaCartographerHouse1_vals = parseDoubleArray(
                componentModernTaigaCartographerHouse1_string,
                modernDefaults);

        componentModernTaigaFisherCottage1_string = config.getString(
                componentModern + "Taiga Fisher Cottage",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + taigaVillages);
        componentModernTaigaFisherCottage1_vals = parseDoubleArray(
                componentModernTaigaFisherCottage1_string,
                modernDefaults);

        componentModernTaigaFletcherHouse1_string = config.getString(
                componentModern + "Taiga Fletcher House",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + taigaVillages);
        componentModernTaigaFletcherHouse1_vals = parseDoubleArray(
                componentModernTaigaFletcherHouse1_string,
                modernDefaults);

        componentModernTaigaLargeFarm1_string = config.getString(
                componentModern + "Taiga Large Farm",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + taigaVillages);
        componentModernTaigaLargeFarm1_vals = parseDoubleArray(componentModernTaigaLargeFarm1_string, modernDefaults);

        componentModernTaigaLargeFarm2_string = config.getString(
                componentModern + "Taiga Medium Farm",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + taigaVillages);
        componentModernTaigaLargeFarm2_vals = parseDoubleArray(componentModernTaigaLargeFarm2_string, modernDefaults);

        componentModernTaigaLibrary1_string = config.getString(
                componentModern + "Taiga Library",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + taigaVillages);
        componentModernTaigaLibrary1_vals = parseDoubleArray(componentModernTaigaLibrary1_string, modernDefaults);

        componentModernTaigaMasonsHouse1_string = config.getString(
                componentModern + "Taiga Mason House",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + taigaVillages);
        componentModernTaigaMasonsHouse1_vals = parseDoubleArray(
                componentModernTaigaMasonsHouse1_string,
                modernDefaults);

        componentModernTaigaMediumHouse1_string = config.getString(
                componentModern + "Taiga Medium House 1",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + taigaVillages);
        componentModernTaigaMediumHouse1_vals = parseDoubleArray(
                componentModernTaigaMediumHouse1_string,
                modernDefaults);

        componentModernTaigaMediumHouse2_string = config.getString(
                componentModern + "Taiga Medium House 2",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + taigaVillages);
        componentModernTaigaMediumHouse2_vals = parseDoubleArray(
                componentModernTaigaMediumHouse2_string,
                modernDefaults);

        componentModernTaigaMediumHouse3_string = config.getString(
                componentModern + "Taiga Medium House 3",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + taigaVillages);
        componentModernTaigaMediumHouse3_vals = parseDoubleArray(
                componentModernTaigaMediumHouse3_string,
                modernDefaults);

        componentModernTaigaMediumHouse4_string = config.getString(
                componentModern + "Taiga Medium House 4",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + taigaVillages);
        componentModernTaigaMediumHouse4_vals = parseDoubleArray(
                componentModernTaigaMediumHouse4_string,
                modernDefaults);

        componentModernTaigaShepherdsHouse1_string = config.getString(
                componentModern + "Taiga Shepherd House",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + taigaVillages);
        componentModernTaigaShepherdsHouse1_vals = parseDoubleArray(
                componentModernTaigaShepherdsHouse1_string,
                modernDefaults);

        componentModernTaigaSmallFarm1_string = config.getString(
                componentModern + "Taiga Small Farm",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + taigaVillages);
        componentModernTaigaSmallFarm1_vals = parseDoubleArray(componentModernTaigaSmallFarm1_string, modernDefaults);

        componentModernTaigaSmallHouse1_string = config.getString(
                componentModern + "Taiga Small House 1",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + taigaVillages);
        componentModernTaigaSmallHouse1_vals = parseDoubleArray(componentModernTaigaSmallHouse1_string, modernDefaults);

        componentModernTaigaSmallHouse2_string = config.getString(
                componentModern + "Taiga Small House 2",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + taigaVillages);
        componentModernTaigaSmallHouse2_vals = parseDoubleArray(componentModernTaigaSmallHouse2_string, modernDefaults);

        componentModernTaigaSmallHouse3_string = config.getString(
                componentModern + "Taiga Small House 3",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + taigaVillages);
        componentModernTaigaSmallHouse3_vals = parseDoubleArray(componentModernTaigaSmallHouse3_string, modernDefaults);

        componentModernTaigaSmallHouse4_string = config.getString(
                componentModern + "Taiga Small House 4",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + taigaVillages);
        componentModernTaigaSmallHouse4_vals = parseDoubleArray(componentModernTaigaSmallHouse4_string, modernDefaults);

        componentModernTaigaSmallHouse5_string = config.getString(
                componentModern + "Taiga Small House 5",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + taigaVillages);
        componentModernTaigaSmallHouse5_vals = parseDoubleArray(componentModernTaigaSmallHouse5_string, modernDefaults);

        componentModernTaigaTannery1_string = config.getString(
                componentModern + "Taiga Tannery",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + taigaVillages);
        componentModernTaigaTannery1_vals = parseDoubleArray(componentModernTaigaTannery1_string, modernDefaults);

        componentModernTaigaTemple1_string = config.getString(
                componentModern + "Taiga Temple",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + taigaVillages);
        componentModernTaigaTemple1_vals = parseDoubleArray(componentModernTaigaTemple1_string, modernDefaults);

        componentModernTaigaToolSmith1_string = config.getString(
                componentModern + "Taiga Tool Smithy",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + taigaVillages);
        componentModernTaigaToolSmith1_vals = parseDoubleArray(componentModernTaigaToolSmith1_string, modernDefaults);

        componentModernTaigaWeaponsmith1_string = config.getString(
                componentModern + "Taiga Weapon Smith House",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + taigaVillages);
        componentModernTaigaWeaponsmith1_vals = parseDoubleArray(
                componentModernTaigaWeaponsmith1_string,
                modernDefaults);

        componentModernTaigaWeaponsmith2_string = config.getString(
                componentModern + "Taiga Weapon Smith Station",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + taigaVillages);
        componentModernTaigaWeaponsmith2_vals = parseDoubleArray(
                componentModernTaigaWeaponsmith2_string,
                modernDefaults);

        modifiedDefaults.set(0, modernDefaults.get(0) * 9D * taigaDecorToHouseRatio);
        for (int i = 1; i < modernDefaults.size(); i++) {
            modifiedDefaults.set(i, modernDefaults.get(i) * taigaHouses * taigaDecorToHouseRatio);
        }
        componentModernTaigaStreetDecor1_string = config.getString(
                componentModern + "Taiga Road Decor",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modifiedDefaults),
                generationStatsForM + taigaVillages);
        componentModernTaigaStreetDecor1_vals = parseDoubleArray(
                componentModernTaigaStreetDecor1_string,
                modifiedDefaults);

        // Savanna components
        componentModernSavannaAnimalPen1_string = config.getString(
                componentModern + "Savanna Covered Animal Pen",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + savannaVillages);
        componentModernSavannaAnimalPen1_vals = parseDoubleArray(
                componentModernSavannaAnimalPen1_string,
                modernDefaults);

        componentModernSavannaAnimalPen2_string = config.getString(
                componentModern + "Savanna Large Animal Pen",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + savannaVillages);
        componentModernSavannaAnimalPen2_vals = parseDoubleArray(
                componentModernSavannaAnimalPen2_string,
                modernDefaults);

        componentModernSavannaAnimalPen3_string = config.getString(
                componentModern + "Savanna Medium Animal Pen",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + savannaVillages);
        componentModernSavannaAnimalPen3_vals = parseDoubleArray(
                componentModernSavannaAnimalPen3_string,
                modernDefaults);

        componentModernSavannaArmorer1_string = config.getString(
                componentModern + "Savanna Armorer House",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + savannaVillages);
        componentModernSavannaArmorer1_vals = parseDoubleArray(componentModernSavannaArmorer1_string, modernDefaults);

        componentModernSavannaButchersShop1_string = config.getString(
                componentModern + "Savanna Butcher Shop 1",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + savannaVillages);
        componentModernSavannaButchersShop1_vals = parseDoubleArray(
                componentModernSavannaButchersShop1_string,
                modernDefaults);

        componentModernSavannaButchersShop2_string = config.getString(
                componentModern + "Savanna Butcher Shop 2",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + savannaVillages);
        componentModernSavannaButchersShop2_vals = parseDoubleArray(
                componentModernSavannaButchersShop2_string,
                modernDefaults);

        componentModernSavannaCartographer1_string = config.getString(
                componentModern + "Savanna Cartographer House",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + savannaVillages);
        componentModernSavannaCartographer1_vals = parseDoubleArray(
                componentModernSavannaCartographer1_string,
                modernDefaults);

        componentModernSavannaFisherCottage1_string = config.getString(
                componentModern + "Savanna Fisher Cottage",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + savannaVillages);
        componentModernSavannaFisherCottage1_vals = parseDoubleArray(
                componentModernSavannaFisherCottage1_string,
                modernDefaults);

        componentModernSavannaFletcherHouse1_string = config.getString(
                componentModern + "Savanna Fletcher House",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + savannaVillages);
        componentModernSavannaFletcherHouse1_vals = parseDoubleArray(
                componentModernSavannaFletcherHouse1_string,
                modernDefaults);

        componentModernSavannaLargeFarm1_string = config.getString(
                componentModern + "Savanna Methodical Farm",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + savannaVillages);
        componentModernSavannaLargeFarm1_vals = parseDoubleArray(
                componentModernSavannaLargeFarm1_string,
                modernDefaults);

        componentModernSavannaLargeFarm2_string = config.getString(
                componentModern + "Savanna Haphazard Farm",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + savannaVillages);
        componentModernSavannaLargeFarm2_vals = parseDoubleArray(
                componentModernSavannaLargeFarm2_string,
                modernDefaults);

        componentModernSavannaLibrary1_string = config.getString(
                componentModern + "Savanna Library",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + savannaVillages);
        componentModernSavannaLibrary1_vals = parseDoubleArray(componentModernSavannaLibrary1_string, modernDefaults);

        componentModernSavannaMason1_string = config.getString(
                componentModern + "Savanna Mason House",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + savannaVillages);
        componentModernSavannaMason1_vals = parseDoubleArray(componentModernSavannaMason1_string, modernDefaults);

        componentModernSavannaMediumHouse1_string = config.getString(
                componentModern + "Savanna Medium House 1",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + savannaVillages);
        componentModernSavannaMediumHouse1_vals = parseDoubleArray(
                componentModernSavannaMediumHouse1_string,
                modernDefaults);

        componentModernSavannaMediumHouse2_string = config.getString(
                componentModern + "Savanna Medium House 2",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + savannaVillages);
        componentModernSavannaMediumHouse2_vals = parseDoubleArray(
                componentModernSavannaMediumHouse2_string,
                modernDefaults);

        componentModernSavannaShepherd1_string = config.getString(
                componentModern + "Savanna Shepherd House",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + savannaVillages);
        componentModernSavannaShepherd1_vals = parseDoubleArray(componentModernSavannaShepherd1_string, modernDefaults);

        componentModernSavannaSmallFarm_string = config.getString(
                componentModern + "Savanna Small Farm",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + savannaVillages);
        componentModernSavannaSmallFarm_vals = parseDoubleArray(componentModernSavannaSmallFarm_string, modernDefaults);

        componentModernSavannaSmallHouse1_string = config.getString(
                componentModern + "Savanna Small House 1",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + savannaVillages);
        componentModernSavannaSmallHouse1_vals = parseDoubleArray(
                componentModernSavannaSmallHouse1_string,
                modernDefaults);

        componentModernSavannaSmallHouse2_string = config.getString(
                componentModern + "Savanna Small House 2",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + savannaVillages);
        componentModernSavannaSmallHouse2_vals = parseDoubleArray(
                componentModernSavannaSmallHouse2_string,
                modernDefaults);

        componentModernSavannaSmallHouse3_string = config.getString(
                componentModern + "Savanna Small House 3",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + savannaVillages);
        componentModernSavannaSmallHouse3_vals = parseDoubleArray(
                componentModernSavannaSmallHouse3_string,
                modernDefaults);

        componentModernSavannaSmallHouse4_string = config.getString(
                componentModern + "Savanna Small House 4",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + savannaVillages);
        componentModernSavannaSmallHouse4_vals = parseDoubleArray(
                componentModernSavannaSmallHouse4_string,
                modernDefaults);

        componentModernSavannaSmallHouse5_string = config.getString(
                componentModern + "Savanna Small House 5",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + savannaVillages);
        componentModernSavannaSmallHouse5_vals = parseDoubleArray(
                componentModernSavannaSmallHouse5_string,
                modernDefaults);

        componentModernSavannaSmallHouse6_string = config.getString(
                componentModern + "Savanna Small House 6",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + savannaVillages);
        componentModernSavannaSmallHouse6_vals = parseDoubleArray(
                componentModernSavannaSmallHouse6_string,
                modernDefaults);

        componentModernSavannaSmallHouse7_string = config.getString(
                componentModern + "Savanna Small House 7",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + savannaVillages);
        componentModernSavannaSmallHouse7_vals = parseDoubleArray(
                componentModernSavannaSmallHouse7_string,
                modernDefaults);

        componentModernSavannaSmallHouse8_string = config.getString(
                componentModern + "Savanna Small House 8",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + savannaVillages);
        componentModernSavannaSmallHouse8_vals = parseDoubleArray(
                componentModernSavannaSmallHouse8_string,
                modernDefaults);

        componentModernSavannaTannery1_string = config.getString(
                componentModern + "Savanna Tannery 1",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + savannaVillages);
        componentModernSavannaTannery1_vals = parseDoubleArray(componentModernSavannaTannery1_string, modernDefaults);

        componentModernSavannaTemple1_string = config.getString(
                componentModern + "Savanna Temple 1",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + savannaVillages);
        componentModernSavannaTemple1_vals = parseDoubleArray(componentModernSavannaTemple1_string, modernDefaults);

        componentModernSavannaTemple2_string = config.getString(
                componentModern + "Savanna Temple 2",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + savannaVillages);
        componentModernSavannaTemple2_vals = parseDoubleArray(componentModernSavannaTemple2_string, modernDefaults);

        componentModernSavannaToolSmith1_string = config.getString(
                componentModern + "Savanna Tool Smithy",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + savannaVillages);
        componentModernSavannaToolSmith1_vals = parseDoubleArray(
                componentModernSavannaToolSmith1_string,
                modernDefaults);

        componentModernSavannaWeaponsmith1_string = config.getString(
                componentModern + "Savanna Small Weapon Smithy",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + savannaVillages);
        componentModernSavannaWeaponsmith1_vals = parseDoubleArray(
                componentModernSavannaWeaponsmith1_string,
                modernDefaults);

        componentModernSavannaWeaponsmith2_string = config.getString(
                componentModern + "Savanna Large Weapon Smithy",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + savannaVillages);
        componentModernSavannaWeaponsmith2_vals = parseDoubleArray(
                componentModernSavannaWeaponsmith2_string,
                modernDefaults);

        modifiedDefaults.set(0, modernDefaults.get(0) * 9D * savannaDecorToHouseRatio);
        for (int i = 1; i < modernDefaults.size(); i++) {
            modifiedDefaults.set(i, modernDefaults.get(i) * savannaHouses * savannaDecorToHouseRatio);
        }
        componentModernSavannaStreetDecor1_string = config.getString(
                componentModern + "Savanna Road Decor",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modifiedDefaults),
                generationStatsForM + savannaVillages);
        componentModernSavannaStreetDecor1_vals = parseDoubleArray(
                componentModernSavannaStreetDecor1_string,
                modifiedDefaults);

        modifiedDefaults.set(0, modernDefaults.get(0) * 9D * savannaStreetToHouseRatio / savannaStreetsAndEndcaps);
        for (int i = 1; i < modernDefaults.size(); i++) {
            modifiedDefaults.set(
                    i,
                    modernDefaults.get(i) * savannaHouses * savannaStreetToHouseRatio / savannaStreetsAndEndcaps);
        }
        componentModernSavannaStreetSubstitute1_string = config.getString(
                componentModern + "Savanna Roadside Farm 1",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modifiedDefaults),
                generationStatsForM + savannaVillages);
        componentModernSavannaStreetSubstitute1_vals = parseDoubleArray(
                componentModernSavannaStreetSubstitute1_string,
                modifiedDefaults);
        ...
        componentModernSavannaStreetSubstitute2_string = config.getString(
                componentModern + "Savanna Roadside Farm 2",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modifiedDefaults),
                generationStatsForM + savannaVillages);
        componentModernSavannaStreetSubstitute2_vals = parseDoubleArray(
                componentModernSavannaStreetSubstitute2_string,
                modifiedDefaults);
        ...
        componentModernSavannaStreetSubstitute3_string = config.getString(
                componentModern + "Savanna Roadside Farm 3",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modifiedDefaults),
                generationStatsForM + savannaVillages);
        componentModernSavannaStreetSubstitute3_vals = parseDoubleArray(
                componentModernSavannaStreetSubstitute3_string,
                modifiedDefaults);
        ...
        componentModernSavannaStreetSubstitute4_string = config.getString(
                componentModern + "Savanna Roadside Farm 4",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modifiedDefaults),
                generationStatsForM + savannaVillages);
        componentModernSavannaStreetSubstitute4_vals = parseDoubleArray(
                componentModernSavannaStreetSubstitute4_string,
                modifiedDefaults);

        componentModernSnowyAnimalPen1_string = config.getString(
                componentModern + "Snowy Animal Pen 1",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + snowyVillages);
        componentModernSnowyAnimalPen1_vals = parseDoubleArray(componentModernSnowyAnimalPen1_string, modernDefaults);

        componentModernSnowyAnimalPen2_string = config.getString(
                componentModern + "Snowy Animal Pen 2",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + snowyVillages);
        componentModernSnowyAnimalPen2_vals = parseDoubleArray(componentModernSnowyAnimalPen2_string, modernDefaults);

        componentModernSnowyArmorerHouse1_string = config.getString(
                componentModern + "Snowy Armorer House 1",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + snowyVillages);
        componentModernSnowyArmorerHouse1_vals = parseDoubleArray(
                componentModernSnowyArmorerHouse1_string,
                modernDefaults);

        componentModernSnowyArmorerHouse2_string = config.getString(
                componentModern + "Snowy Armorer House 2",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + snowyVillages);
        componentModernSnowyArmorerHouse2_vals = parseDoubleArray(
                componentModernSnowyArmorerHouse2_string,
                modernDefaults);

        componentModernSnowyButchersShop1_string = config.getString(
                componentModern + "Snowy Butcher House",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + snowyVillages);
        componentModernSnowyButchersShop1_vals = parseDoubleArray(
                componentModernSnowyButchersShop1_string,
                modernDefaults);

        componentModernSnowyButchersShop2_string = config.getString(
                componentModern + "Snowy Butcher Igloo",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + snowyVillages);
        componentModernSnowyButchersShop2_vals = parseDoubleArray(
                componentModernSnowyButchersShop2_string,
                modernDefaults);

        componentModernSnowyCartographerHouse1_string = config.getString(
                componentModern + "Snowy Cartographer House",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + snowyVillages);
        componentModernSnowyCartographerHouse1_vals = parseDoubleArray(
                componentModernSnowyCartographerHouse1_string,
                modernDefaults);

        componentModernSnowyFarm1_string = config.getString(
                componentModern + "Snowy Square Farm",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + snowyVillages);
        componentModernSnowyFarm1_vals = parseDoubleArray(componentModernSnowyFarm1_string, modernDefaults);

        componentModernSnowyFarm2_string = config.getString(
                componentModern + "Snowy Patch Farm",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + snowyVillages);
        componentModernSnowyFarm2_vals = parseDoubleArray(componentModernSnowyFarm2_string, modernDefaults);

        componentModernSnowyFisherCottage_string = config.getString(
                componentModern + "Snowy Fisher Cottage",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + snowyVillages);
        componentModernSnowyFisherCottage_vals = parseDoubleArray(
                componentModernSnowyFisherCottage_string,
                modernDefaults);

        componentModernSnowyFletcherHouse1_string = config.getString(
                componentModern + "Snowy Fletcher House",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + snowyVillages);
        componentModernSnowyFletcherHouse1_vals = parseDoubleArray(
                componentModernSnowyFletcherHouse1_string,
                modernDefaults);

        componentModernSnowyLibrary1_string = config.getString(
                componentModern + "Snowy Library",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + snowyVillages);
        componentModernSnowyLibrary1_vals = parseDoubleArray(componentModernSnowyLibrary1_string, modernDefaults);

        componentModernSnowyMasonsHouse1_string = config.getString(
                componentModern + "Snowy Mason House 1",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + snowyVillages);
        componentModernSnowyMasonsHouse1_vals = parseDoubleArray(
                componentModernSnowyMasonsHouse1_string,
                modernDefaults);

        componentModernSnowyMasonsHouse2_string = config.getString(
                componentModern + "Snowy Mason House 2",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + snowyVillages);
        componentModernSnowyMasonsHouse2_vals = parseDoubleArray(
                componentModernSnowyMasonsHouse2_string,
                modernDefaults);

        componentModernSnowyMediumHouse1_string = config.getString(
                componentModern + "Snowy Medium House 1",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + snowyVillages);
        componentModernSnowyMediumHouse1_vals = parseDoubleArray(
                componentModernSnowyMediumHouse1_string,
                modernDefaults);

        componentModernSnowyMediumHouse2_string = config.getString(
                componentModern + "Snowy Medium House 2",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + snowyVillages);
        componentModernSnowyMediumHouse2_vals = parseDoubleArray(
                componentModernSnowyMediumHouse2_string,
                modernDefaults);

        componentModernSnowyMediumHouse3_string = config.getString(
                componentModern + "Snowy Medium House 3",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + snowyVillages);
        componentModernSnowyMediumHouse3_vals = parseDoubleArray(
                componentModernSnowyMediumHouse3_string,
                modernDefaults);

        componentModernSnowyShepherdsHouse1_string = config.getString(
                componentModern + "Snowy Shepherd House",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + snowyVillages);
        componentModernSnowyShepherdsHouse1_vals = parseDoubleArray(
                componentModernSnowyShepherdsHouse1_string,
                modernDefaults);

        componentModernSnowySmallHouse1_string = config.getString(
                componentModern + "Snowy Small House 1",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + snowyVillages);
        componentModernSnowySmallHouse1_vals = parseDoubleArray(componentModernSnowySmallHouse1_string, modernDefaults);

        componentModernSnowySmallHouse2_string = config.getString(
                componentModern + "Snowy Small House 2",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + snowyVillages);
        componentModernSnowySmallHouse2_vals = parseDoubleArray(componentModernSnowySmallHouse2_string, modernDefaults);

        componentModernSnowySmallHouse3_string = config.getString(
                componentModern + "Snowy Small House 3",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + snowyVillages);
        componentModernSnowySmallHouse3_vals = parseDoubleArray(componentModernSnowySmallHouse3_string, modernDefaults);

        componentModernSnowySmallHouse4_string = config.getString(
                componentModern + "Snowy Small House 4",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + snowyVillages);
        componentModernSnowySmallHouse4_vals = parseDoubleArray(componentModernSnowySmallHouse4_string, modernDefaults);

        componentModernSnowySmallHouse5_string = config.getString(
                componentModern + "Snowy Small House 5",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + snowyVillages);
        componentModernSnowySmallHouse5_vals = parseDoubleArray(componentModernSnowySmallHouse5_string, modernDefaults);

        componentModernSnowySmallHouse6_string = config.getString(
                componentModern + "Snowy Small House 6",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + snowyVillages);
        componentModernSnowySmallHouse6_vals = parseDoubleArray(componentModernSnowySmallHouse6_string, modernDefaults);

        componentModernSnowySmallHouse7_string = config.getString(
                componentModern + "Snowy Small House 7",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + snowyVillages);
        componentModernSnowySmallHouse7_vals = parseDoubleArray(componentModernSnowySmallHouse7_string, modernDefaults);

        componentModernSnowySmallHouse8_string = config.getString(
                componentModern + "Snowy Small House 8",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + snowyVillages);
        componentModernSnowySmallHouse8_vals = parseDoubleArray(componentModernSnowySmallHouse8_string, modernDefaults);

        componentModernSnowyTannery1_string = config.getString(
                componentModern + "Snowy Tannery",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + snowyVillages);
        componentModernSnowyTannery1_vals = parseDoubleArray(componentModernSnowyTannery1_string, modernDefaults);

        componentModernSnowyTemple1_string = config.getString(
                componentModern + "Snowy Temple",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + snowyVillages);
        componentModernSnowyTemple1_vals = parseDoubleArray(componentModernSnowyTemple1_string, modernDefaults);

        componentModernSnowyToolSmith1_string = config.getString(
                componentModern + "Snowy Tool Smithy",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + snowyVillages);
        componentModernSnowyToolSmith1_vals = parseDoubleArray(componentModernSnowyToolSmith1_string, modernDefaults);

        componentModernSnowyWeaponSmith1_string = config.getString(
                componentModern + "Snowy Weapon Smithy",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + snowyVillages);
        componentModernSnowyWeaponSmith1_vals = parseDoubleArray(
                componentModernSnowyWeaponSmith1_string,
                modernDefaults);

        modifiedDefaults.set(0, modernDefaults.get(0) * 9D * snowyDecorToHouseRatio);
        for (int i = 1; i < modernDefaults.size(); i++) {
            modifiedDefaults.set(i, modernDefaults.get(i) * snowyHouses * snowyDecorToHouseRatio);
        }
        componentModernSnowyStreetDecor1_string = config.getString(
                componentModern + "Snowy Road Decor",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modifiedDefaults),
                generationStatsForM + snowyVillages);
        componentModernSnowyStreetDecor1_vals = parseDoubleArray(
                componentModernSnowyStreetDecor1_string,
                modifiedDefaults);

        componentModernJungleArmorerHouse_string = config.getString(
                componentModern + "Jungle Armorer House",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + jungleVillages);
        componentModernJungleArmorerHouse_vals = parseDoubleArray(
                componentModernJungleArmorerHouse_string,
                modernDefaults);

        componentModernJungleButcherShop_string = config.getString(
                componentModern + "Jungle Butcher Shop",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + jungleVillages);
        componentModernJungleButcherShop_vals = parseDoubleArray(
                componentModernJungleButcherShop_string,
                modernDefaults);

        componentModernJungleCartographerHouse1_string = config.getString(
                componentModern + "Jungle Cartographer House 1",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + jungleVillages);
        componentModernJungleCartographerHouse1_vals = parseDoubleArray(
                componentModernJungleCartographerHouse1_string,
                modernDefaults);

        componentModernJungleCartographerHouse2_string = config.getString(
                componentModern + "Jungle Cartographer House 2",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + jungleVillages);
        componentModernJungleCartographerHouse2_vals = parseDoubleArray(
                componentModernJungleCartographerHouse2_string,
                modernDefaults);

        componentModernJungleFisherCottage_string = config.getString(
                componentModern + "Jungle Fisher Cottage",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + jungleVillages);
        componentModernJungleFisherCottage_vals = parseDoubleArray(
                componentModernJungleFisherCottage_string,
                modernDefaults);

        componentModernJungleFletcherHouse1_string = config.getString(
                componentModern + "Jungle Fletcher House 1",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + jungleVillages);
        componentModernJungleFletcherHouse1_vals = parseDoubleArray(
                componentModernJungleFletcherHouse1_string,
                modernDefaults);

        componentModernJungleFletcherHouse2_string = config.getString(
                componentModern + "Jungle Fletcher House 2",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + jungleVillages);
        componentModernJungleFletcherHouse2_vals = parseDoubleArray(
                componentModernJungleFletcherHouse2_string,
                modernDefaults);

        componentModernJungleLargeHouse_string = config.getString(
                componentModern + "Jungle Large House",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + jungleVillages);
        componentModernJungleLargeHouse_vals = parseDoubleArray(componentModernJungleLargeHouse_string, modernDefaults);

        componentModernJungleLibrary_string = config.getString(
                componentModern + "Jungle Library",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + jungleVillages);
        componentModernJungleLibrary_vals = parseDoubleArray(componentModernJungleLibrary_string, modernDefaults);

        componentModernJungleMasonHouse_string = config.getString(
                componentModern + "Jungle Mason House",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + jungleVillages);
        componentModernJungleMasonHouse_vals = parseDoubleArray(componentModernJungleMasonHouse_string, modernDefaults);

        componentModernJungleMediumHouse1_string = config.getString(
                componentModern + "Jungle Medium House 1",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + jungleVillages);
        componentModernJungleMediumHouse1_vals = parseDoubleArray(
                componentModernJungleMediumHouse1_string,
                modernDefaults);

        componentModernJungleMediumHouse2_string = config.getString(
                componentModern + "Jungle Medium House 2",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + jungleVillages);
        componentModernJungleMediumHouse2_vals = parseDoubleArray(
                componentModernJungleMediumHouse2_string,
                modernDefaults);

        componentModernJungleMediumHouse3_string = config.getString(
                componentModern + "Jungle Medium House 3",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + jungleVillages);
        componentModernJungleMediumHouse3_vals = parseDoubleArray(
                componentModernJungleMediumHouse3_string,
                modernDefaults);

        componentModernJungleMediumHouse4_string = config.getString(
                componentModern + "Jungle Medium House 4",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + jungleVillages);
        componentModernJungleMediumHouse4_vals = parseDoubleArray(
                componentModernJungleMediumHouse4_string,
                modernDefaults);

        componentModernJungleShepherdHouse_string = config.getString(
                componentModern + "Jungle Shepherd House",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + jungleVillages);
        componentModernJungleShepherdHouse_vals = parseDoubleArray(
                componentModernJungleShepherdHouse_string,
                modernDefaults);

        componentModernJungleSmallHouse1_string = config.getString(
                componentModern + "Jungle Small House 1",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + jungleVillages);
        componentModernJungleSmallHouse1_vals = parseDoubleArray(
                componentModernJungleSmallHouse1_string,
                modernDefaults);

        componentModernJungleSmallHouse2_string = config.getString(
                componentModern + "Jungle Small House 2",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + jungleVillages);
        componentModernJungleSmallHouse2_vals = parseDoubleArray(
                componentModernJungleSmallHouse2_string,
                modernDefaults);

        componentModernJungleSmallHouse3_string = config.getString(
                componentModern + "Jungle Small House 3",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + jungleVillages);
        componentModernJungleSmallHouse3_vals = parseDoubleArray(
                componentModernJungleSmallHouse3_string,
                modernDefaults);

        componentModernJungleSmallHouse4_string = config.getString(
                componentModern + "Jungle Small House 4",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + jungleVillages);
        componentModernJungleSmallHouse4_vals = parseDoubleArray(
                componentModernJungleSmallHouse4_string,
                modernDefaults);

        componentModernJungleSmallHouse5_string = config.getString(
                componentModern + "Jungle Small House 5",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + jungleVillages);
        componentModernJungleSmallHouse5_vals = parseDoubleArray(
                componentModernJungleSmallHouse5_string,
                modernDefaults);

        componentModernJungleSmallHouse6_string = config.getString(
                componentModern + "Jungle Small House 6",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + jungleVillages);
        componentModernJungleSmallHouse6_vals = parseDoubleArray(
                componentModernJungleSmallHouse6_string,
                modernDefaults);

        componentModernJungleSmallHouse7_string = config.getString(
                componentModern + "Jungle Small House 7",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + jungleVillages);
        componentModernJungleSmallHouse7_vals = parseDoubleArray(
                componentModernJungleSmallHouse7_string,
                modernDefaults);

        componentModernJungleSmallHouse8_string = config.getString(
                componentModern + "Jungle Small House 8",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + jungleVillages);
        componentModernJungleSmallHouse8_vals = parseDoubleArray(
                componentModernJungleSmallHouse8_string,
                modernDefaults);

        componentModernJungleStable_string = config.getString(
                componentModern + "Jungle Stone Animal Pen",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + jungleVillages);
        componentModernJungleStable_vals = parseDoubleArray(componentModernJungleStable_string, modernDefaults);

        componentModernJungleSteppedFarm_string = config.getString(
                componentModern + "Jungle Stepped Farm",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + jungleVillages);
        componentModernJungleSteppedFarm_vals = parseDoubleArray(
                componentModernJungleSteppedFarm_string,
                modernDefaults);

        componentModernJungleStoneAnimalPen_string = config.getString(
                componentModern + "Jungle Stone Animal Pen",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + jungleVillages);
        componentModernJungleStoneAnimalPen_vals = parseDoubleArray(
                componentModernJungleStoneAnimalPen_string,
                modernDefaults);

        componentModernJungleTamedFarm_string = config.getString(
                componentModern + "Jungle Tamed Farm",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + jungleVillages);
        componentModernJungleTamedFarm_vals = parseDoubleArray(componentModernJungleTamedFarm_string, modernDefaults);

        componentModernJungleTannery1_string = config.getString(
                componentModern + "Jungle Tannery 1",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + jungleVillages);
        componentModernJungleTannery1_vals = parseDoubleArray(componentModernJungleTannery1_string, modernDefaults);

        componentModernJungleTannery2_string = config.getString(
                componentModern + "Jungle Tannery 2",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + jungleVillages);
        componentModernJungleTannery2_vals = parseDoubleArray(componentModernJungleTannery2_string, modernDefaults);

        componentModernJungleTemple_string = config.getString(
                componentModern + "Jungle Temple",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + jungleVillages);
        componentModernJungleTemple_vals = parseDoubleArray(componentModernJungleTemple_string, modernDefaults);

        componentModernJungleToolSmithy1_string = config.getString(
                componentModern + "Jungle Tool Smithy 1",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + jungleVillages);
        componentModernJungleToolSmithy1_vals = parseDoubleArray(
                componentModernJungleToolSmithy1_string,
                modernDefaults);

        componentModernJungleToolSmithy2_string = config.getString(
                componentModern + "Jungle Tool Smithy 2",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + jungleVillages);
        componentModernJungleToolSmithy2_vals = parseDoubleArray(
                componentModernJungleToolSmithy2_string,
                modernDefaults);

        componentModernJungleWeaponSmithy_string = config.getString(
                componentModern + "Jungle Weapon Smithy",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + jungleVillages);
        componentModernJungleWeaponSmithy_vals = parseDoubleArray(
                componentModernJungleWeaponSmithy_string,
                modernDefaults);

        componentModernJungleWildFarm_string = config.getString(
                componentModern + "Jungle Wild Farm",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + jungleVillages);
        componentModernJungleWildFarm_vals = parseDoubleArray(componentModernJungleWildFarm_string, modernDefaults);

        componentModernJungleWoodAnimalPen_string = config.getString(
                componentModern + "Jungle Wood Animal Pen",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + jungleVillages);
        componentModernJungleWoodAnimalPen_vals = parseDoubleArray(
                componentModernJungleWoodAnimalPen_string,
                modernDefaults);

        modifiedDefaults.set(0, modernDefaults.get(0) * 9D * jungleDecorToHouseRatio);
        for (int i = 1; i < modernDefaults.size(); i++) {
            modifiedDefaults.set(i, modernDefaults.get(i) * jungleHouses * jungleDecorToHouseRatio);
        }
        componentModernJungleStreetDecor_string = config.getString(
                componentModern + "Jungle Road Decor",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modifiedDefaults),
                generationStatsForM + jungleVillages);
        componentModernJungleStreetDecor_vals = parseDoubleArray(
                componentModernJungleStreetDecor_string,
                modifiedDefaults);

        modifiedDefaults.set(0, modernDefaults.get(0) * 9D * jungleStreetToHouseRatio / jungleStreetsAndEndcaps);
        for (int i = 1; i < modernDefaults.size(); i++) {
            modifiedDefaults
                    .set(i, modernDefaults.get(i) * jungleHouses * jungleStreetToHouseRatio / jungleStreetsAndEndcaps);
        }
        componentModernJungleRoadAccent1_string = config.getString(
                componentModern + "Jungle Road Accent 1",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modifiedDefaults),
                generationStatsForM + jungleVillages);
        componentModernJungleRoadAccent1_vals = parseDoubleArray(
                componentModernJungleRoadAccent1_string,
                modifiedDefaults);
        ...
        componentModernJungleRoadAccent2_string = config.getString(
                componentModern + "Jungle Road Accent 2",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modifiedDefaults),
                generationStatsForM + jungleVillages);
        componentModernJungleRoadAccent2_vals = parseDoubleArray(
                componentModernJungleRoadAccent2_string,
                modifiedDefaults);

        componentModernSwampAnimalPen1_string = config.getString(
                componentModern + "Swamp Animal Pen 1",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + swampVillages);
        componentModernSwampAnimalPen1_vals = parseDoubleArray(componentModernSwampAnimalPen1_string, modernDefaults);

        componentModernSwampAnimalPen2_string = config.getString(
                componentModern + "Swamp Animal Pen 2",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + swampVillages);
        componentModernSwampAnimalPen2_vals = parseDoubleArray(componentModernSwampAnimalPen2_string, modernDefaults);

        componentModernSwampArmorerHouse_string = config.getString(
                componentModern + "Swamp Armorer House",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + swampVillages);
        componentModernSwampArmorerHouse_vals = parseDoubleArray(
                componentModernSwampArmorerHouse_string,
                modernDefaults);

        componentModernSwampButcherShop_string = config.getString(
                componentModern + "Swamp Butcher Shop",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + swampVillages);
        componentModernSwampButcherShop_vals = parseDoubleArray(componentModernSwampButcherShop_string, modernDefaults);

        componentModernSwampCartographerHouse_string = config.getString(
                componentModern + "Swamp Cartographer House",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + swampVillages);
        componentModernSwampCartographerHouse_vals = parseDoubleArray(
                componentModernSwampCartographerHouse_string,
                modernDefaults);

        componentModernSwampFisherCottage1_string = config.getString(
                componentModern + "Swamp Fisher Cottage 1",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + swampVillages);
        componentModernSwampFisherCottage1_vals = parseDoubleArray(
                componentModernSwampFisherCottage1_string,
                modernDefaults);

        componentModernSwampFisherCottage2_string = config.getString(
                componentModern + "Swamp Fisher Cottage 2",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + swampVillages);
        componentModernSwampFisherCottage2_vals = parseDoubleArray(
                componentModernSwampFisherCottage2_string,
                modernDefaults);

        componentModernSwampFletcherHouse_string = config.getString(
                componentModern + "Swamp Fletcher House",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + swampVillages);
        componentModernSwampFletcherHouse_vals = parseDoubleArray(
                componentModernSwampFletcherHouse_string,
                modernDefaults);

        componentModernSwampHutFarm_string = config.getString(
                componentModern + "Swamp Hut Farm",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + swampVillages);
        componentModernSwampHutFarm_vals = parseDoubleArray(componentModernSwampHutFarm_string, modernDefaults);

        componentModernSwampHorribleSecret_string = config.getString(
                componentModern + "Swamp Horrible Secret",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + swampVillages);
        componentModernSwampHorribleSecret_vals = parseDoubleArray(
                componentModernSwampHorribleSecret_string,
                modernDefaults);

        componentModernSwampLargeHouse_string = config.getString(
                componentModern + "Swamp Large House",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + swampVillages);
        componentModernSwampLargeHouse_vals = parseDoubleArray(componentModernSwampLargeHouse_string, modernDefaults);

        componentModernSwampLibrary_string = config.getString(
                componentModern + "Swamp Library",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + swampVillages);
        componentModernSwampLibrary_vals = parseDoubleArray(componentModernSwampLibrary_string, modernDefaults);

        componentModernSwampMasonHouse_string = config.getString(
                componentModern + "Swamp Mason House",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + swampVillages);
        componentModernSwampMasonHouse_vals = parseDoubleArray(componentModernSwampMasonHouse_string, modernDefaults);

        componentModernSwampMediumHouse1_string = config.getString(
                componentModern + "Swamp Medium House 1",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + swampVillages);
        componentModernSwampMediumHouse1_vals = parseDoubleArray(
                componentModernSwampMediumHouse1_string,
                modernDefaults);

        componentModernSwampMediumHouse2_string = config.getString(
                componentModern + "Swamp Medium House 2",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + swampVillages);
        componentModernSwampMediumHouse2_vals = parseDoubleArray(
                componentModernSwampMediumHouse2_string,
                modernDefaults);

        componentModernSwampShepherdHouse1_string = config.getString(
                componentModern + "Swamp Shepherd House 1",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + swampVillages);
        componentModernSwampShepherdHouse1_vals = parseDoubleArray(
                componentModernSwampShepherdHouse1_string,
                modernDefaults);

        componentModernSwampShepherdHouse2_string = config.getString(
                componentModern + "Swamp Shepherd House 2",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + swampVillages);
        componentModernSwampShepherdHouse2_vals = parseDoubleArray(
                componentModernSwampShepherdHouse2_string,
                modernDefaults);

        componentModernSwampSmallHouse1_string = config.getString(
                componentModern + "Swamp Small House 1",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + swampVillages);
        componentModernSwampSmallHouse1_vals = parseDoubleArray(componentModernSwampSmallHouse1_string, modernDefaults);

        componentModernSwampSmallHouse2_string = config.getString(
                componentModern + "Swamp Small House 2",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + swampVillages);
        componentModernSwampSmallHouse2_vals = parseDoubleArray(componentModernSwampSmallHouse2_string, modernDefaults);

        componentModernSwampSmallHouse3_string = config.getString(
                componentModern + "Swamp Small House 3",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + swampVillages);
        componentModernSwampSmallHouse3_vals = parseDoubleArray(componentModernSwampSmallHouse3_string, modernDefaults);

        componentModernSwampSmallHouse4_string = config.getString(
                componentModern + "Swamp Small House 4",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + swampVillages);
        componentModernSwampSmallHouse4_vals = parseDoubleArray(componentModernSwampSmallHouse4_string, modernDefaults);

        componentModernSwampSmallHouse5_string = config.getString(
                componentModern + "Swamp Small House 5",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + swampVillages);
        componentModernSwampSmallHouse5_vals = parseDoubleArray(componentModernSwampSmallHouse5_string, modernDefaults);

        componentModernSwampStable_string = config.getString(
                componentModern + "Swamp Stable",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + swampVillages);
        componentModernSwampStable_vals = parseDoubleArray(componentModernSwampStable_string, modernDefaults);

        componentModernSwampTannery_string = config.getString(
                componentModern + "Swamp Tannery",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + swampVillages);
        componentModernSwampTannery_vals = parseDoubleArray(componentModernSwampTannery_string, modernDefaults);

        componentModernSwampTemple_string = config.getString(
                componentModern + "Swamp Temple",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + swampVillages);
        componentModernSwampTemple_vals = parseDoubleArray(componentModernSwampTemple_string, modernDefaults);

        componentModernSwampToolSmithy_string = config.getString(
                componentModern + "Swamp Tool Smithy",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + swampVillages);
        componentModernSwampToolSmithy_vals = parseDoubleArray(componentModernSwampToolSmithy_string, modernDefaults);

        componentModernSwampWeaponSmithy_string = config.getString(
                componentModern + "Swamp Weapon Smithy",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + swampVillages);
        componentModernSwampWeaponSmithy_vals = parseDoubleArray(
                componentModernSwampWeaponSmithy_string,
                modernDefaults);

        componentModernSwampWildFarm_string = config.getString(
                componentModern + "Swamp Wild Farm",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modernDefaults),
                generationStatsForM + swampVillages);
        ...
        for (int i = 1; i < modernDefaults.size(); i++) {
            modifiedDefaults.set(i, modernDefaults.get(i) * swampHouses * swampDecorToHouseRatio);
        }
        componentModernSwampStreetDecor_string = config.getString(
                componentModern + "Swamp Road Decor",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modifiedDefaults),
                generationStatsForM + swampVillages);
        componentModernSwampStreetDecor_vals = parseDoubleArray(
                componentModernSwampStreetDecor_string,
                modifiedDefaults);

        modifiedDefaults.set(0, modernDefaults.get(0) * 9D * swampStreetToHouseRatio / swampStreetsAndEndcaps);
        for (int i = 1; i < modernDefaults.size(); i++) {
            modifiedDefaults
                    .set(i, modernDefaults.get(i) * swampHouses * swampStreetToHouseRatio / swampStreetsAndEndcaps);
        }
        componentModernSwampRoadAccent_string = config.getString(
                componentModern + "Swamp Road Accent",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                convertDoubleArrayToString(modifiedDefaults),
                generationStatsForM + swampVillages);
        ...
        // --- Decor --- //
        allowTaigaTroughs = config.getBoolean(
                "Decor: Allow Taiga Troughs",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                true,
                "Set to false to completely disallow the trough as decor in taiga villages");
        fillTroughsWithWater = config.getBoolean(
                "Decor: Fill Troughs With Water",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                true,
                "Whether generated decor troughs are filled with water");
        restrictTaigaTroughs = config.getBoolean(
                "Decor: Restrict Taiga Troughs",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                false,
                "Limit taiga troughs only to the well or as street decor. "
        ...
        componentVillageTypes = config.getStringListWithoutDefaultsInComment(
                "Component Village Types",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                FunctionsVN.joinTwoStringArrays(
        ...
        // Misc
        replaceAnvils = config.getBoolean(
                "Replace anvils with smooth stone slabs",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                false,
                "Set this to true to replace all instances of anvil generation in VN structures with smooth double stone slabs (like in classic vanilla blacksmiths).");
        useModdedWoodenDoors = config.getBoolean(
                "Use modded wooden doors in mod structures",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                true,
                "Set this to false to use the vanilla 1.7 wooden doors, even if supported mod doors are found. Some villagers have trouble opening some modded doors.");
        spawnModdedVillagers = config.getBoolean(
                "Spawn Extra Villagers with mod professions",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                false,
                "Villagers spawned in town centers or residential houses can have non-vanilla professions.");
        spawnVillagersInResidences = config.getBoolean(
                "Spawn Extra Villagers in Residences",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                false,
                "Spawn villagers with random professions and ages in non-job-specific residential houses.");
        spawnVillagersInTownCenters = config.getBoolean(
                "Spawn Extra Villagers in Town Centers",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                true,
                "Spawn villagers with random professions and ages in the town center.");
        nameVillageHorses = config.getBoolean(
                "Name Village Horses",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                false,
                "Domesticated horses spawn with names generated from the \"pet\" syllable pool.");
        cleanDroppedItems = config.getBoolean(
                "Clean Dropped Items",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                true,
                "Deletes most blocks and items accidentally broken and dropped onto the ground by village generation, such as torches, doors, beds, and chest contents.");
        villageAnimalRestrictionLevel = config.getInt(
                "Village Livestock Restriction Level",
                Reference.CATEGORY_VILLAGE_GENERATOR,
                0,
                0,
                3,
                "Limits whether animals are spawned in VN village components." + "\n0: No restriction"
        ...
        spawnBiomesNames = config.getStringListWithoutDefaultsInComment(
                ...
                new String[] {
                        // Vanilla
                        "Plains", "Desert", "Forest", "Taiga", "Swampland", "Ice Plains", "MushroomIsland", "Jungle",
                        "JungleEdge", "Birch Forest", "Roofed Forest", "Cold Taiga", "Mega Taiga", "Savanna", "Mesa",
                        "Sunflower Plains", "Desert M", "Flower Forest", "Jungle Edge", "Birch Forest M",
                        "Mega Spruce Taiga",
                        // Biomes o' Plenty
                        "Bamboo Forest", "Bayou", "Bog", "Boreal Forest", "Chaparral", "Cherry Blossom Grove",
                        "Coniferous Forest", "Snowy Coniferous Forest", "Deciduous Forest", "Dense Forest",
                        "Eucalyptus Forest", "Flower Field", "Flower Island", "Frost Forest", "Fungi Forest", "Garden",
                        "Grassland", "Grove", "Heathland|savanna|oak", "Highland", "Lavender Fields", "Lush Swamp",
                        "Maple Woods", "Meadow", "Mountain", "Mystic Grove", "Orchard", "Outback", "Prairie",
                        "Rainforest", "Redwood Forest", "RedwoodForest", "Sacred Springs", "Seasonal Forest", "Shield",
                        "Shrubland", "Steppe", "Temperate Rainforest", "Tropical Island", "Tropical Rainforest",
                        "Tundra", "Wetland", "Woodland", "Xeric Shrubland", "Meadow Forest", "Oasis", "Scrubland",
                        "Seasonal Forest Clearing", "Spruce Woods", "Mountain Foothills", "MountainFoothills",
                        // ATG
                        "Rocky Steppe",
                        // ExtrabiomeXL
                        "Autumn Woods", "Mini Jungle", "Mountain Taiga", "Pine Forest", "Redwood Lush", "Lush Redwoods",
                        "Snowy Forest", "Snow Forest", "Snowy Rainforest", "Woodlands",
                        // Highlands
                        "Autumn Forest", "Badlands", "Highlands", "Lowlands", "Outback", "Pinelands", "Sahel",
                        "Tall Pine Forest", "Tropics",
                        // ManaMetal
                        "BiomeAppleTree", "BiomeBarley", "BiomeCedarForest", "BiomeFertileForest", "BiomeFlower",
                        "BiomeFlowerPall", "BiomeForestFX", "BiomeLavender", "BiomeMagic", "BiomeOasis",
                        "BiomePineForest", "BiomeRichForest", "BiomeSnowmountain", "BiomeTallGrass", },
                "Names of biomes which can spawn villages. Only used with Village Generator, and only applies to Overworld. Note that this list is EXCLUSIVE: other mod configs won't override this. You have to paste all biome names here."
                        ...
                        + "disallowModSub: enter \"nosub\" in the last field if you want to disallow other mods from substituting blocks during village generation. Blank, or anything else, allows substitution. This only works for "
                        + Reference.MOD_NAME
                        + " buildings.\n"
    ...
    /**
     * Used to convert the comma-separated-integer string in the config value into an array of integers Returns the
     * given default array if the user screws up.
     */
    public static ArrayList<Double> parseDoubleArray(String configvalue, ArrayList<Double> defaultValues) {
        try {
            ...
            if (sMPA1_stringarray.length != 5) {
                LogHelper.error(
                        "Config entry " + configvalue
                                + " requires five values, not "
                                + sMPA1_stringarray.length
                                + ". Using default values "
                                + convertDoubleArrayToString(defaultValues)
                                + " until this is fixed.");
            ...
            if (doubleArrayListToReturn.get(0) < 0) {
                ...
                LogHelper.error(
                        "The first value of config entry " + configvalue
                                + " is a weight and must not be less than zero. It will be set to 0 until this is fixed.");
            }
            ...
            if ((doubleArrayListToReturn.get(1) * newVillageSizeMinimum + doubleArrayListToReturn.get(2)) < 0) {
                LogHelper.error(
                        "Values two and three of config entry " + configvalue
                                + " can result in fewer than zero of this structure component. Using default values "
                                + convertDoubleArrayToString(defaultValues)
                                + " until this is fixed.");
                return defaultValues;
            }
            ...
            if ((doubleArrayListToReturn.get(3) * newVillageSizeMinimum + doubleArrayListToReturn.get(4)) < 0) {
                LogHelper.error(
                        "Values four and five of config entry " + configvalue
                                + " will result in fewer than zero of this structure component. Using default values "
                                + convertDoubleArrayToString(defaultValues)
                                + " until this is fixed.");
                return defaultValues;
            }
            ...
            if ((doubleArrayListToReturn.get(1) * (newVillageSizeMinimum + newVillageSizeMaximum) / 2F
                    + doubleArrayListToReturn.get(2))
                    > (doubleArrayListToReturn.get(3) * (newVillageSizeMinimum + newVillageSizeMaximum) / 2F
                            + doubleArrayListToReturn.get(4))) {
                LogHelper.error(
                        "Values two through five of config entry " + configvalue
                                + " result in a higher upper bound than a lower bound for this structure component. Using default values "
                                + convertDoubleArrayToString(defaultValues)
                                + " until this is fixed.");
                return defaultValues;
            }
        ...
        } catch (Exception e) // Config entry was malformed
        {
            LogHelper.error(
                    "Config entry " + configvalue
                            + " was malformed. Check that it is five comma-separated integers. Using default values "
                            + convertDoubleArrayToString(defaultValues)
                            + " until this is fixed.");
            return defaultValues;
        }
    ...
    /**
     * Loads the (classPath|villageType) string lists from newvillages/village_gen.cfg and assigns them to this
    ...
    public static Map<String, ArrayList> unpackComponentVillageTypes(String[] inputList) {
        ...
        for (String entry : inputList) {
            ...
            try {
                componentClassPath = splitEntry[0].trim();
            } catch (Exception e) {
                componentClassPath = "FAILSAFE";
            }
            try {
                componentVillageType = splitEntry[1].trim();
            } catch (Exception e) {
                componentVillageType = "none";
            } // If the second field is blank, that implies "don't spawn"
    ...
    /**
     * Loads the (biomeName|villageType|materialType|modSubstitution) string lists from newvillages/village_gen.cfg and
     * assigns them to this instance's variables.
     */
