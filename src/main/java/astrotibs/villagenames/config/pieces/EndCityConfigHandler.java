...
public class EndCityConfigHandler
{
	...
	public static void loadConfiguration()
	{
		...
		syllable_count_weighting = config.get("Syllable Count Weighting", "End City Syllable Pool", NamePieces.endcity_syllable_count_weights,
		...
		prefix_chance = config.getFloat("Prefix Chance", "End City Syllable Pool",
				endcity_root_initial.length <= 0 ? 0 : ((float)endcity_prefix.length)/endcity_root_initial.length, 0.0F, 1.0F,
				"The fraction of names that include a prefix.");
		
		suffix_chance = config.getFloat("Suffix Chance", "End City Syllable Pool",
				endcity_root_initial.length <= 0 ? 0 : ((float)endcity_suffix.length)/endcity_root_initial.length, 0.0F, 1.0F,
