...
public class CustomConfigHandler
{
	...
	public static void loadConfiguration()
	{
		...
		syllable_count_weighting = config.get("Syllable Count Weighting", "Custom Syllable Pool", NamePieces.custom_syllable_count_weights,
		...
		prefix_chance = config.getFloat("Prefix Chance", "Custom Syllable Pool",
				custom_root_initial.length <= 0 ? 0 : ((float)custom_prefix.length)/custom_root_initial.length, 0.0F, 1.0F,
				"The fraction of names that include a prefix.");
		
		suffix_chance = config.getFloat("Suffix Chance", "Custom Syllable Pool",
				custom_root_initial.length <= 0 ? 0 : ((float)custom_suffix.length)/custom_root_initial.length, 0.0F, 1.0F,
