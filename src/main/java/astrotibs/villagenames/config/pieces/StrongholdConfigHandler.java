...
public class StrongholdConfigHandler {
    ...
    public static void loadConfiguration() {
        stronghold_prefix = config.getStringWithoutDefaultsInComment(
                Reference.NAME_PREFIXES,
                Reference.CATEGORY_STRONGHOLD_SYLLABLE_POOL,
                NamePieces.STRONGHOLD_PREFIX_DEFAULT,
                Reference.DESCRIPTION_PREFIXES).trim().split("\\s*,\\s*");

        stronghold_root_initial = config.getStringWithoutDefaultsInComment(
                Reference.NAME_ROOT_INITIAL,
                Reference.CATEGORY_STRONGHOLD_SYLLABLE_POOL,
                NamePieces.STRONGHOLD_ROOT_INITIAL_DEFAULT,
                Reference.DESCRIPTION_ROOT_INITIAL).trim().split("\\s*,\\s*");

        stronghold_root_syllables = config.getStringWithoutDefaultsInComment(
                Reference.NAME_ROOT_SYLLABLES,
                Reference.CATEGORY_STRONGHOLD_SYLLABLE_POOL,
                NamePieces.STRONGHOLD_ROOT_SYL_BEGIN_DEFAULT,
                Reference.DESCRIPTION_ROOT_SYLLABLES).trim().split("\\s*,\\s*");

        stronghold_root_terminal = config.getStringWithoutDefaultsInComment(
                Reference.NAME_ROOT_TERMINAL,
                Reference.CATEGORY_STRONGHOLD_SYLLABLE_POOL,
                NamePieces.STRONGHOLD_ROOT_TERMINAL_DEFAULT,
                Reference.DESCRIPTION_ROOT_TERMINAL).trim().split("\\s*,\\s*");

        stronghold_suffix = config.getStringWithoutDefaultsInComment(
                "Suffixes",
                Reference.CATEGORY_STRONGHOLD_SYLLABLE_POOL,
                NamePieces.STRONGHOLD_SUFFIX_DEFAULT,
                Reference.DESCRIPTION_SUFFIXES).trim().split("\\s*,\\s*");

        syllable_count_weighting = config.get(
                Reference.CATEGORY_STRONGHOLD_SYLLABLE_POOL,
                Reference.NAME_SYLLABLE_COUNT_WEIGHTING,
                NamePieces.STRONGHOLD_SYLLABLE_COUNT_WEIGHTS,
        ...
        terminal_blank_counts = config.get(
                ...
                NamePieces.STRONGHOLD_BLANK_TERMINAL_COUNTS,
                Reference.DESCRIPTION_TERMINAL_BLANK_COUNTS).getIntList();

        prefix_chance = config.getFloat(
                Reference.NAME_PREFIX_CHANCE,
                Reference.CATEGORY_STRONGHOLD_SYLLABLE_POOL,
                (stronghold_root_initial.length - 1) <= 0 ? 0
                        : ((float) stronghold_prefix.length - 1) / (stronghold_root_initial.length - 1),
                0.0F,
                1.0F,
        ...
        suffix_chance = config.getFloat(
                ...
                Reference.CATEGORY_STRONGHOLD_SYLLABLE_POOL,
                (stronghold_root_initial.length - 1) <= 0 ? 0
                        : ((float) stronghold_suffix.length - 1) / (stronghold_root_initial.length - 1),
                0.0F,
                1.0F,
