...
public class VillagerConfigHandler
{
	...
	{
		villager_prefix = config.getString("Prefixes", "Villager Syllable Pool", NamePiecesEntities.villager_prefix_default,
				"Prefixes that can occur before the core name.").trim().split("\\s*,\\s*");
		
		villager_root_initial = config.getString("Root: Initial", "Villager Syllable Pool", NamePiecesEntities.villager_root_initial_default,
				"Core names begin with one of these half-syllables. Use _ to represent a space.").trim().split("\\s*,\\s*");
		
		villager_root_syllables = config.getString("Root: Syllables", "Villager Syllable Pool", NamePiecesEntities.villager_root_sylBegin_default,
		...
		villager_root_terminal = config.getString("Root: Terminal", "Villager Syllable Pool", NamePiecesEntities.villager_root_terminal_default,
				"Core names end with one of these half-syllables. Use _ to represent a space, and ^ for a blank entry.").trim().split("\\s*,\\s*");
		
		villager_suffix = config.getString("Suffixes", "Villager Syllable Pool", NamePiecesEntities.villager_suffix_default,
				"Suffixes that can occur after the core name.").trim().split("\\s*,\\s*");

		
		
		syllable_count_weighting = config.get("Syllable Count Weighting", "Villager Syllable Pool", NamePiecesEntities.villager_syllable_count_weights,
		...
		prefix_chance = config.getFloat("Prefix Chance", "Villager Syllable Pool",
				villager_root_initial.length <= 0 ? 0 : ((float)villager_prefix.length)/villager_root_initial.length, 0.0F, 1.0F,
				"The fraction of names that include a prefix.");
		
		suffix_chance = config.getFloat("Suffix Chance", "Villager Syllable Pool",
				villager_root_initial.length <= 0 ? 0 : ((float)villager_suffix.length)/villager_root_initial.length, 0.0F, 1.0F,
