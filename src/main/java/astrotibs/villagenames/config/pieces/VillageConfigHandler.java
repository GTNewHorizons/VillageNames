...
public class VillageConfigHandler {
    ...
    public static void loadConfiguration() {
        village_prefix = config.getStringWithoutDefaultsInComment(
                Reference.NAME_PREFIXES,
                Reference.CATEGORY_VILLAGE_SYLLABLE_POOL,
                NamePieces.VILLAGE_PREFIX_DEFAULT,
                Reference.DESCRIPTION_PREFIXES).trim().split("\\s*,\\s*");

        village_root_initial = config.getStringWithoutDefaultsInComment(
                Reference.NAME_ROOT_INITIAL,
                Reference.CATEGORY_VILLAGE_SYLLABLE_POOL,
                NamePieces.VILLAGE_ROOT_INITIAL_DEFAULT,
                Reference.DESCRIPTION_ROOT_INITIAL).trim().split("\\s*,\\s*");

        village_root_syllables = config.getStringWithoutDefaultsInComment(
                Reference.NAME_ROOT_SYLLABLES,
                Reference.CATEGORY_VILLAGE_SYLLABLE_POOL,
                NamePieces.VILLAGE_ROOT_SYL_BEGIN_DEFAULT,
                Reference.DESCRIPTION_ROOT_SYLLABLES).trim().split("\\s*,\\s*");

        village_root_terminal = config.getStringWithoutDefaultsInComment(
                Reference.NAME_ROOT_TERMINAL,
                Reference.CATEGORY_VILLAGE_SYLLABLE_POOL,
                NamePieces.VILLAGE_ROOT_TERMINAL_DEFAULT,
                Reference.DESCRIPTION_ROOT_TERMINAL).trim().split("\\s*,\\s*");

        village_suffix = config.getStringWithoutDefaultsInComment(
                "Suffixes",
                Reference.CATEGORY_VILLAGE_SYLLABLE_POOL,
                NamePieces.VILLAGE_SUFFIX_DEFAULT,
                Reference.DESCRIPTION_SUFFIXES).trim().split("\\s*,\\s*");

        syllable_count_weighting = config.get(
                Reference.CATEGORY_VILLAGE_SYLLABLE_POOL,
                Reference.NAME_SYLLABLE_COUNT_WEIGHTING,
                NamePieces.VILLAGE_SYLLABLE_COUNT_WEIGHTS,
        ...
        terminal_blank_counts = config.get(
                ...
                NamePieces.VILLAGE_BLANK_TERMINAL_COUNTS,
                Reference.DESCRIPTION_TERMINAL_BLANK_COUNTS).getIntList();

        prefix_chance = config.getFloat(
                Reference.NAME_PREFIX_CHANCE,
                Reference.CATEGORY_VILLAGE_SYLLABLE_POOL,
                (village_root_initial.length - 1) <= 0 ? 0
                        : ((float) village_prefix.length - 1) / (village_root_initial.length - 1),
                0.0F,
                1.0F,
        ...
        suffix_chance = config.getFloat(
                ...
                Reference.CATEGORY_VILLAGE_SYLLABLE_POOL,
                (village_root_initial.length - 1) <= 0 ? 0
                        : ((float) village_suffix.length - 1) / (village_root_initial.length - 1),
                0.0F,
                1.0F,
