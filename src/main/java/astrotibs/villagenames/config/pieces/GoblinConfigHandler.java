...
public class GoblinConfigHandler
{
	...
	{
		goblin_prefix = config.getString("Prefixes", "Goblin Syllable Pool", NamePiecesEntities.goblin_prefix_default,
				"Prefixes that can occur before the core name.").trim().split("\\s*,\\s*");
		
		goblin_root_initial = config.getString("Root: Initial", "Goblin Syllable Pool", NamePiecesEntities.goblin_root_initial_default,
				"Core names begin with one of these half-syllables. Use _ to represent a space.").trim().split("\\s*,\\s*");
		
		goblin_root_syllables = config.getString("Root: Syllables", "Goblin Syllable Pool", NamePiecesEntities.goblin_root_sylBegin_default,
		...
		goblin_root_terminal = config.getString("Root: Terminal", "Goblin Syllable Pool", NamePiecesEntities.goblin_root_terminal_default,
				"Core names end with one of these half-syllables. Use _ to represent a space, and ^ for a blank entry.").trim().split("\\s*,\\s*");
		
		goblin_suffix = config.getString("Suffixes", "Goblin Syllable Pool", NamePiecesEntities.goblin_suffix_default,
				"Suffixes that can occur after the core name.").trim().split("\\s*,\\s*");

		
		
		syllable_count_weighting = config.get("Syllable Count Weighting", "Goblin Syllable Pool", NamePiecesEntities.goblin_syllable_count_weights,
		...
		prefix_chance = config.getFloat("Prefix Chance", "Goblin Syllable Pool",
				goblin_root_initial.length <= 0 ? 0 : ((float)goblin_prefix.length)/goblin_root_initial.length, 0.0F, 1.0F,
				"The fraction of names that include a prefix.");
		
		suffix_chance = config.getFloat("Suffix Chance", "Goblin Syllable Pool",
				goblin_root_initial.length <= 0 ? 0 : ((float)goblin_suffix.length)/goblin_root_initial.length, 0.0F, 1.0F,
