...
public class MineshaftConfigHandler
{
	...
	{
		mineshaft_prefix = config.getString("Prefixes", "Mineshaft Syllable Pool", NamePieces.mineshaft_prefix_default,
				"Prefixes that can occur before the core name.").trim().split("\\s*,\\s*");
		
		mineshaft_root_initial = config.getString("Root: Initial", "Mineshaft Syllable Pool", NamePieces.mineshaft_root_initial_default,
				"Core names begin with one of these half-syllables. Use _ to represent a space.").trim().split("\\s*,\\s*");
		
		mineshaft_root_syllables = config.getString("Root: Syllables", "Mineshaft Syllable Pool", NamePieces.mineshaft_root_sylBegin_default,
		...
		mineshaft_root_terminal = config.getString("Root: Terminal", "Mineshaft Syllable Pool", NamePieces.mineshaft_root_terminal_default,
				"Core names end with one of these half-syllables. Use _ to represent a space, and ^ for a blank entry.").trim().split("\\s*,\\s*");
		
		mineshaft_suffix = config.getString("Suffixes", "Mineshaft Syllable Pool", NamePieces.mineshaft_suffix_default,
