...
public class FortressConfigHandler
{
	...
	public static void loadConfiguration()
	{
		...
		syllable_count_weighting = config.get("Syllable Count Weighting", "Fortress Syllable Pool", NamePieces.fortress_syllable_count_weights,
		...
		prefix_chance = config.getFloat("Prefix Chance", "Fortress Syllable Pool",
				fortress_root_initial.length <= 0 ? 0 : ((float)fortress_prefix.length)/fortress_root_initial.length, 0.0F, 1.0F,
				"The fraction of names that include a prefix.");
		
		suffix_chance = config.getFloat("Suffix Chance", "Fortress Syllable Pool",
				fortress_root_initial.length <= 0 ? 0 : ((float)fortress_suffix.length)/fortress_root_initial.length, 0.0F, 1.0F,
