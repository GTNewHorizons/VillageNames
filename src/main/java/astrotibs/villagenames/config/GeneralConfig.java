...
public class GeneralConfig {
    ...
    protected static void loadConfiguration() {
        ...
        // --- General --- //
        nameSign = config.getBoolean(
            "Name Sign",
            Reference.CATEGORY_GENERAL,
            true,
            "Town centers display their name on one or more signs.");
        wellBoundary = config.getBoolean(
            "Well boundary",
            Reference.CATEGORY_GENERAL,
            true,
            "Whether to surround the well with colored blocks");
        ...
        villageBanners = config.getBoolean(
            "Village Banner",
            Reference.CATEGORY_GENERAL,
            true,
            "The town banner pattern is displayed at the town center. You must be using Et Futurum or Gany's Surface with banners enabled.");
        signYaw = config.getInt(
            "Sign Yaw",
            Reference.CATEGORY_GENERAL,
            3,
            0,
            4,
            "If Village Banner is enabled: Degree to which well signs and banners should face inward. At 0 they face directly outward away from the well; at 4 they face each other.");

        recordStructureCoords = config.getBoolean(
            "Record Structure Coords",
            Reference.CATEGORY_GENERAL,
            true,
            "Books generated by villagers or the Codex record the structure's coordinates.");
        villagerMakesBook = config.getBoolean(
            "Villager makes book",
            Reference.CATEGORY_GENERAL,
            true,
            "Villagers record the village name, and other info, when you right-click them with a book.");
        villagerDropBook = config.getBoolean(
            "Villager drops book/codex",
            Reference.CATEGORY_GENERAL,
            false,
            "Village books and Codices are dropped by the villager rather than going directly into your inventory.");
        villagersOfferCodexTrade = config.getBoolean(
            "Villagers offer Codex trade",
            Reference.CATEGORY_GENERAL,
            true,
            "Librarian and Cartographer villagers can offer to sell you the Codex item via an ordinary trade.");
        villagerMakesCodex = config.getBoolean(
            "Villager makes codex",
            Reference.CATEGORY_GENERAL,
            false,
            "Librarian villagers will give you a codex if you right-click them while holding emerald, iron ingots, and/or gold ingots.");
        useVillageColors = config.getBoolean(
            "Use village colors",
            Reference.CATEGORY_GENERAL,
            true,
            "Whether to apply the village's colors to concrete, terracotta, carpet, etc.");

        wellDecorations = config.getBoolean(
            "Allow well decorations",
            Reference.CATEGORY_WELL_KILL_SWITCH,
            true,
            "Set this to false to override-disable all well decoration: sign, slabs, terracotta, concrete.");

        addConcrete = config.getBoolean(
            "1.12 Blocks",
            Reference.CATEGORY_WORLD_OF_COLOR,
            true,
            "Whether to add 1.12 style Concrete, Concrete Powder, and Glazed Terracotta");
        concreteWell = config.getBoolean(
            "Concrete Well",
            Reference.CATEGORY_WORLD_OF_COLOR,
            true,
            "Whether to decorate wells with Concrete and Glazed Terracotta instead of stained clay. This only applies to old-school wells, i.e. if \"Activate New Village Generator\" is false.");

        // ---------------Professions-----------------//
        villagerCareers = config.getBoolean(
            "Villager Careers",
            Reference.CATEGORY_VILLAGER_PROFESSIONS,
            true,
        ...
        treasureTrades = config.getBoolean(
            "Treasure Trades",
            Reference.CATEGORY_VILLAGER_PROFESSIONS,
            true,
            "High-level Librarians and Cartographers will offer enchanted books and treasures in exchange for "
                + Reference.MOD_NAME
                + " items."
                + " This only applies if Villager Careers is true.");
        // villagerLegacyTrades = config.getBoolean("Villager Legacy Trades", Reference.CATEGORY_VILLAGER_PROFESSIONS,
        // true, "When using Villager Careers: some trades that were removed in 1.8 can still be unlocked.");

        modernVillagerSkins = config.getBoolean(
            "Modern Villager Profession Skins",
            Reference.CATEGORY_VILLAGER_PROFESSIONS,
            true,
            "Use the composite 1.14 Villager skins");
        modernZombieSkins = config.getBoolean(
            "Modern Zombie Profession Skins",
            Reference.CATEGORY_VILLAGER_PROFESSIONS,
            true,
            "Use the composite 1.14 Zombie skins");
        modernVillagerTrades = config.getBoolean(
            "Modern Villager Trades",
            Reference.CATEGORY_VILLAGER_PROFESSIONS,
            true,
            "Use JE 1.14 / BE 1.12 trade offerings and add the Mason villager");
        enableNitwit = config.getBoolean(
            "Nitwit Villager",
            Reference.CATEGORY_VILLAGER_PROFESSIONS,
            true,
            "Enable 1.11 NitWit Villagers");

        moddedVillagerHeadwear = config.getBoolean(
            "Modded Villager Headwear",
            Reference.CATEGORY_VILLAGER_PROFESSIONS,
            false,
            "If modern skins are enabled: renders the headwear layer for non-vanilla villager professions, if one exists.");

        removeMobArmor = config.getBoolean(
            "Remove Armor for Modern Skins",
            Reference.CATEGORY_VILLAGER_PROFESSIONS,
            true,
            "If modern skins are enabled: automatically removes armor from villagers and zombies to avoid a rendering bug.");

        moddedVillagerHeadwearGraylist = config.getStringListWithoutDefaultsInComment(
            "Modded Villager Headwear Graylist",
            Reference.CATEGORY_VILLAGER_PROFESSIONS,
            new String[] { "14", // Growthcraft Apiarist
                "-15", // Apple & Milk & Tea's Cafe Master -- turned off because I enjoy his sexy hair
                "44", // Village Taverns's Shepherdess -- turned on to better differentiate from the vanilla
                      // Shepherd
        ...
        for (String prof_s : moddedVillagerHeadwearGraylist) {
            try {
                ...
                if (prof_i > 0) {
                    moddedVillagerHeadwearWhitelist.add(prof_i);
                } else if (prof_i < 0) {
                    moddedVillagerHeadwearBlacklist.add(Math.abs(prof_i));
                }
        ...
        moddedVillagerModularSkins = config.getStringListWithoutDefaultsInComment(
            "Modded Villager Modular Skins",
            Reference.CATEGORY_VILLAGER_PROFESSIONS,
            new String[] {
                // Actually Additions
                "aa_jam|aa_jam|493827",
                // Apple & Milk & Tea
                "amt_cafemaster||15", "amt_warehousemanager||16",
                // ChocoCraft Plus
                "ccp_stablehand||19940402",
                // Enchanting Plus v4 Enchanter
                "ep_enchanter||935153",
                // Forestry
                "for_apiarist|for_apiarist|80", "for_arborist|for_arborist|81",
                // Fossils and Archaeology
                "fa_archaeologist||303",
                // GraveStone Mod
                "gs_undertaker|gs_undertaker|385",
                // Growthcraft
                "gc_brewer||10", "gc_apiarist||14",
                // Growthcraft Community Edition
                "gc_brewer||6677", "gc_apiarist||7766",
                // Immersive Engineering
                "ie_engineer||512",
                // Mine Trading Cards
                "mtc_cardmaster||7117", "mtc_cardtrader||7118",
                ...
                // Thaumcraft
                "thc_wizard||190", "thc_banker||191",
                // Tinkers Construct
                "tc_tinkerer||78943",
                // Traincraft
                "tc_stationchief||86",
                // Village Taverns
                "vt_barwench||42", "vt_hostler||43", "vt_shepherdess||44", "vt_baker||45",
                // Witchery
                "wit_apothecary||2435", },
            "(If modern skins are enabled) List of profession IDs for other mods' villagers to render in the modular skin style. Format is: careerAsset|zombieCareerAsset|professionID\n"
                + "careerAsset: career skin png to be overlaid onto the villager, located in assets\\"
                + Reference.MOD_ID.toLowerCase()
                + "\\textures\\entity\\villager\\profession\n"
                + "The default values are all available in "
                + Reference.MOD_NAME
                + ". You can access custom values with a resourcepack.\n"
        ...
        villagerSkinTones = config.getBoolean(
            "Display Skin Tones",
            Reference.CATEGORY_VILLAGER_SKIN_TONES,
            true,
            "Display Gaussian-distributed random skin tones assigned to villagers");
        villagerSkinToneVarianceAnnealing = config.getFloat(
            "Skin Tone Variance Annealing",
            Reference.CATEGORY_VILLAGER_SKIN_TONES,
            8F / 3,
            0,
            Float.MAX_VALUE,
        ...
        villagerSkinToneVarianceScale = config.getFloat(
            "Skin Tone Variance Scale",
            Reference.CATEGORY_VILLAGER_SKIN_TONES,
            1F,
            0,
            Float.MAX_VALUE,
        ...
        zombieCureCatalysts = config.getStringListWithoutDefaultsInComment(
            "Zombie Cure Catalysts",
            Reference.CATEGORY_GENERAL,
            new String[] { "vanilla|net.minecraft.block.BlockBed|tile.bed|-1",
        ...
        zombieCureGroups = config.getStringListWithoutDefaultsInComment(
            "Zombie Cure Groups",
            Reference.CATEGORY_GENERAL,
            new String[] { "vanilla|0.3|14" },
        ...
        versionChecker = config.getBoolean(
            "Version Checker",
            Reference.CATEGORY_MISCELLANEOUS,
            true,
            "Displays a client-side chat message on login if there's an update available.");
        codexChestLoot = config.getBoolean(
            "Codex Chest Loot",
            Reference.CATEGORY_MISCELLANEOUS,
            true,
            "The Codex can appear as rare chest loot.");
        allowInGameConfig = config.getBoolean(
            "Allow in-game config access",
            Reference.CATEGORY_MISCELLANEOUS,
            true,
            "Set this to false to deacactivate the in-game config GUI, in case it conflicts with other mods.");
        debugMessages = config.getBoolean(
            "Debug messages",
            Reference.CATEGORY_MISCELLANEOUS,
            false,
            "Print debug messages to the console, print the class paths of entities and blocks you right-click.");
        pyramidTerracotta = config.getBoolean(
            "Pyramid Terracotta",
            Reference.CATEGORY_MISCELLANEOUS,
            true,
            "Replace the wool blocks in desert pyramids with terracotta as in 1.8+");
        addLunarinBlocks = config.getBoolean(
            "Add Lunarin Blocks",
            Reference.CATEGORY_MISCELLANEOUS,
            true,
            "Adds an original brick-style block for iron and gold");
        ...
        addIgloos = config
            .getBoolean("Add Igloos", Reference.CATEGORY_MISCELLANEOUS, true, "Generate Igloos from 1.9+");
        ...
        nameEntities = config.getBoolean(
            "Entity names",
            Reference.CATEGORY_NAMING,
            true,
            "Entities reveal their names when you right-click them, or automatically if so assigned.");
        addJobToName = config.getBoolean(
            "Entity professions",
            Reference.CATEGORY_NAMING,
            false,
            "An entity's name also includes its profession/title. You may need to right-click the entity to update its name plate.");
        nameGolems = config.getBoolean(
            "Golem names",
            Reference.CATEGORY_NAMING,
            true,
            "Right-click village Golems to learn their name.");

        nitwitProfession = config
            .getString("Nitwit Profession", Reference.CATEGORY_NAMING, "", "The career displayed for a Nitwit");
        ...
        modNameMappingAutomatic = config.getStringListWithoutDefaultsInComment(
            ...
            new String[] {
                ...
                // Village Names
                "alien-golem|Elder Guardian|" + Reference.ELDER_GUARDIAN_CLASS + "|add", // NOT AN ACTUAL
                                                                                         // CLASSPATH:
                                                                                         // hard-coded in
                                                                                         // EntityInteractHandler.java

                // Galacticraft
                "alien-demon|Evolved Skeleton Boss|micdoodle8.mods.galacticraft.core.entities.EntitySkeletonBoss|add", // 1.7
                                                                                                                       // and
                                                                                                                       // 1.10
                "alien-golem|Evolved Creeper Boss|micdoodle8.mods.galacticraft.planets.mars.entities.EntityCreeperBoss|add", // 1.7
                                                                                                                             // and
                                                                                                                             // 1.10
        ...
        modNameMappingClickable = config.getStringListWithoutDefaultsInComment(
            ...
            new String[] {
                ...
                "goblin-demon||com.emoniph.witchery.entity.EntityImp", "demon||com.emoniph.witchery.entity.EntityDemon",
        ...
        // Forced pet names
        entitiesNameableLikePets = config.getStringListWithoutDefaultsInComment(
            "Entities Nameable Like Pets",
            Reference.CATEGORY_NAMING,
            new String[] {},
        ...
        harvestcraftCropFarmRate = config.getFloat(
            "Crop rate: Harvestcraft",
            Reference.CATEGORY_MOD_INTEGRATION,
            0.25F,
            0F,
            1F,
            "Generate Harvestcraft crops in farms. Only used with Village Generator. Set to 0 for no HC crops.");
        dragonQuestCropFarmRate = config.getFloat(
            "Crop rate: DQ Respect",
            Reference.CATEGORY_MOD_INTEGRATION,
            0.25F,
            0F,
            1F,
            "Generate Dragon Quest Respect crops in farms. Only used with Village Generator. Set to 0 for no DQR crops.");
        antiqueAtlasMarkerNames = config.getBoolean(
            "Antique Atlas: Village Marker Names",
            Reference.CATEGORY_MOD_INTEGRATION,
            true,
            "Label a new village marker with the village's name in your Antique Atlases.");
        antiqueAtlasPlaceSignPosts = config.getBoolean(
            "Antique Atlas: Place Sign-Posts",
            Reference.CATEGORY_MOD_INTEGRATION,
            true,
            "If using harceroi's SignPosts mod, new villages will generate with a registered Sign-Post somewhere");
        antiqueAtlasSignPostLabel = config.getString(
            "Antique Atlas: Sign-Post Label",
            Reference.CATEGORY_MOD_INTEGRATION,
            "",
            "Text that will be displayed on every Antique Atlas village SignPost marker. Leave blank for the village name.");
        manaMetalCartographersSellMapFragments = config.getBoolean(
            "ManaMetal: Cartographers Sell Map Fragments",
            Reference.CATEGORY_MOD_INTEGRATION,
            true,
            "Cartographer villagers can sell Map Fragments");

        modBamboo = config.getStringListWithoutDefaultsInComment(
            "Mod Priority: Bamboo",
            Reference.CATEGORY_MOD_INTEGRATION,
            new String[] { "etfuturum", "manametal", "growthcraft", "biomesoplenty", },
        ...
        modBanner = config.getStringListWithoutDefaultsInComment(
            "Mod Priority: Banner",
            Reference.CATEGORY_MOD_INTEGRATION,
            new String[] { "etfuturum", "ganyssurface", },
        ...
        modBarrel = config.getStringListWithoutDefaultsInComment(
            "Mod Priority: Barrel",
            Reference.CATEGORY_MOD_INTEGRATION,
            new String[] { "etfuturum", "uptodate", },
        ...
        modBed = config.getStringListWithoutDefaultsInComment(
            "Mod Priority: Bed",
            Reference.CATEGORY_MOD_INTEGRATION,
            new String[] { "etfuturum", "manametal", "bettervanilla", "carpentersblocks", },
        ...
        modBeetroot = config.getStringListWithoutDefaultsInComment(
            "Mod Priority: Beetroot",
            Reference.CATEGORY_MOD_INTEGRATION,
            new String[] { "etfuturum", "ganyssurface", },
        ...
        modBookshelf = config.getStringListWithoutDefaultsInComment(
            "Mod Priority: Bookshelf",
            Reference.CATEGORY_MOD_INTEGRATION,
            new String[] { "woodstuff", "ganyssurface", "cement", },
        ...
        modBountifulStone = config.getStringListWithoutDefaultsInComment(
            "Mod Priority: Bountiful Stone",
            Reference.CATEGORY_MOD_INTEGRATION,
            new String[] { "chisel", "etfuturum", "uptodate", "ganyssurface", "botania", "manametal", },
        ...
        modButton = config.getStringListWithoutDefaultsInComment(
            "Mod Priority: Button",
            Reference.CATEGORY_MOD_INTEGRATION,
            new String[] { "etfuturum", "uptodate", "woodstuff", "ganyssurface", },
        ...
        modCampfire = config.getStringListWithoutDefaultsInComment(
            "Mod Priority: Campfire",
            Reference.CATEGORY_MOD_INTEGRATION,
            new String[] { "campfirebackport", "manametal", },
        ...
        modChest = config.getStringListWithoutDefaultsInComment(
            "Mod Priority: Chest",
            Reference.CATEGORY_MOD_INTEGRATION,
            new String[] { "woodstuff", "ganyssurface", },
        ...
        modComposter = config.getStringListWithoutDefaultsInComment(
            "Mod Priority: Composter",
            Reference.CATEGORY_MOD_INTEGRATION,
            new String[] { "etfuturum", "gardenstuff", },
        ...
        modConcrete = config.getStringListWithoutDefaultsInComment(
            "Mod Priority: Concrete",
            Reference.CATEGORY_MOD_INTEGRATION,
            new String[] { "villagenames", "etfuturum", "uptodate", "manametal", },
            "Priority order for referencing concrete for well decorations; essentially, if you still want these features but want to disable "
                + Reference.MOD_NAME
                + "\'s versions. The version highest on the list and registered in your game will be used.");

        modCraftingTable = config.getStringListWithoutDefaultsInComment(
            "Mod Priority: Crafting Table",
            Reference.CATEGORY_MOD_INTEGRATION,
            new String[] { "woodstuff", "cement", },
        ...
        modDoor = config.getStringListWithoutDefaultsInComment(
            "Mod Priority: Door",
            Reference.CATEGORY_MOD_INTEGRATION,
            new String[] { "etfuturum", "uptodate", "ganyssurface", "malisisdoors", "manametal", },
        ...
        modDye = config.getStringListWithoutDefaultsInComment(
            "Mod Priority: Dye",
            Reference.CATEGORY_MOD_INTEGRATION,
            new String[] { "etfuturum", "uptodate", "biomesoplenty", "mariculture", },
        ...
        modFence = config.getStringListWithoutDefaultsInComment(
            "Mod Priority: Fence",
            Reference.CATEGORY_MOD_INTEGRATION,
            new String[] { "etfuturum", "uptodate", "woodstuff", "ganyssurface", "manametal", },
        ...
        modFenceGate = config.getStringListWithoutDefaultsInComment(
            "Mod Priority: Fence Gate",
            Reference.CATEGORY_MOD_INTEGRATION,
            new String[] { "etfuturum", "ganyssurface", "uptodate", "woodstuff", "manametal", "malisisdoors", },
        ...
        modFlower = config.getStringListWithoutDefaultsInComment(
            "Mod Priority: Flower",
            Reference.CATEGORY_MOD_INTEGRATION,
            new String[] { "etfuturum", "uptodate", },
        ...
        modGlazedTerracotta = config.getStringListWithoutDefaultsInComment(
            "Mod Priority: Glazed Terracotta",
            Reference.CATEGORY_MOD_INTEGRATION,
            new String[] { "villagenames", "etfuturum", "uptodate", "manametal", },
            "Priority order for referencing Glazed Terracotta for villager trade offers and well decorations; essentially, if you still want these features but want to disable "
                + Reference.MOD_NAME
                + "\'s versions. The version highest on the list and registered in your game will be used.");

        modGrassPath = config.getStringListWithoutDefaultsInComment(
            "Mod Priority: Grass Path",
            Reference.CATEGORY_MOD_INTEGRATION,
            new String[] { "etfuturum", "gregtech", "uptodate", "manametal", },
        ...
        modIronNugget = config.getStringListWithoutDefaultsInComment(
            "Mod Priority: Iron Nugget",
            Reference.CATEGORY_MOD_INTEGRATION,
            new String[] { "etfuturum", "uptodate", "tinkersconstruct", "thaumcraft", "immersiveengineering",
                "thermalfoundation", "railcraft", "mariculture", "netherlicious", "ganysnether", "manametal", },
        ...
        modKelp = config.getStringListWithoutDefaultsInComment(
            "Mod Priority: Kelp",
            Reference.CATEGORY_MOD_INTEGRATION,
            new String[] { "mariculture", "biomesoplenty", },
        ...
        modLantern = config.getStringListWithoutDefaultsInComment(
            "Mod Priority: Lantern",
            Reference.CATEGORY_MOD_INTEGRATION,
            new String[] { "etfuturum", "netherlicious", "uptodate", "enviromine", "manametal", },
        ...
        modLectern = config.getStringListWithoutDefaultsInComment(
            "Mod Priority: Lectern",
            Reference.CATEGORY_MOD_INTEGRATION,
            new String[] { "manametal", "bibliocraft", },
        ...
        modMossyStone = config.getStringListWithoutDefaultsInComment(
            "Mod Priority: Mossy Stone",
            Reference.CATEGORY_MOD_INTEGRATION,
            new String[] { "etfuturum", "uptodate", },
        ...
        modMutton = config.getStringListWithoutDefaultsInComment(
            "Mod Priority: Mutton",
            Reference.CATEGORY_MOD_INTEGRATION,
            new String[] { "etfuturum", "uptodate", "ganyssurface", "harvestcraft", },
        ...
        modPressurePlate = config.getStringListWithoutDefaultsInComment(
            "Mod Priority: Pressure Plate",
            Reference.CATEGORY_MOD_INTEGRATION,
            new String[] { "etfuturum", "uptodate", "woodstuff", "ganyssurface", },
            "Priority order for referencing Fence blocks for village generation. The version highest on the list and registered in your game will be used.");

        modPrismarine = config.getStringListWithoutDefaultsInComment(
            "Mod Priority: Prismarine",
            Reference.CATEGORY_MOD_INTEGRATION,
            new String[] { "villagenames", "etfuturum", "botania", "uptodate", "manametal", },
        ...
        modRedSandstone = config.getStringListWithoutDefaultsInComment(
            "Mod Priority: Red Sandstone",
            Reference.CATEGORY_MOD_INTEGRATION,
            new String[] { "etfuturum", "uptodate", "ganyssurface", },
        ...
        modSign = config.getStringListWithoutDefaultsInComment(
            "Mod Priority: Sign",
            Reference.CATEGORY_MOD_INTEGRATION,
            new String[] { "etfuturum", "ganyssurface", },
        ...
        modSmoothStone = config.getStringListWithoutDefaultsInComment(
            "Mod Priority: Smooth Stone",
            Reference.CATEGORY_MOD_INTEGRATION,
            new String[] { "etfuturum", "uptodate", "manametal", },
        ...
        modSmithingTable = config.getStringListWithoutDefaultsInComment(
            "Mod Priority: Smithing Table",
            Reference.CATEGORY_MOD_INTEGRATION,
            new String[] { "etfuturum", "smithinginthe90s", "netheriteplus", "manametal", },
        ...
        modSmoothSandstone = config.getStringListWithoutDefaultsInComment(
            "Mod Priority: Smooth Sandstone",
            Reference.CATEGORY_MOD_INTEGRATION,
            new String[] { "etfuturum", "uptodate", },
        ...
        modSponge = config.getStringListWithoutDefaultsInComment(
            "Mod Priority: Sponge",
            Reference.CATEGORY_MOD_INTEGRATION,
            new String[] { "villagenames", "etfuturum", "uptodate", },
        ...
        modStrippedLog = config.getStringListWithoutDefaultsInComment(
            "Mod Priority: Stripped Log",
            Reference.CATEGORY_MOD_INTEGRATION,
            new String[] { "etfuturum", "uptodate", "manametal", },
        ...
        modSuspiciousStew = config.getStringListWithoutDefaultsInComment(
            "Mod Priority: Suspicious Stew",
            Reference.CATEGORY_MOD_INTEGRATION,
            new String[] { "etfuturum", "uptodate", },
        ...
        modSweetBerries = config.getStringListWithoutDefaultsInComment(
            "Mod Priority: Sweet Berries",
            Reference.CATEGORY_MOD_INTEGRATION,
            new String[] { "etfuturum", "uptodate", "manametal", },
        ...
        modWall = config.getStringListWithoutDefaultsInComment(
            "Mod Priority: Wall",
            Reference.CATEGORY_MOD_INTEGRATION,
            new String[] { "etfuturum", "uptodate", "railcraft", },
        ...
        modWoodenTable = config.getStringListWithoutDefaultsInComment(
            "Mod Priority: Table",
            Reference.CATEGORY_MOD_INTEGRATION,
            new String[] { "minecraft", "bibliocraft", "mrcrayfishsfurnituremod", },
        ...
        modWoodenTrapdoor = config.getStringListWithoutDefaultsInComment(
            "Mod Priority: Trapdoor",
            Reference.CATEGORY_MOD_INTEGRATION,
            new String[] { "etfuturum", "uptodate", "ganyssurface", },
        ...
        modProfessionMapping = config.getStringListWithoutDefaultsInComment(
            ...
            new String[] {
                ...
                "Jam Guy|493827|0",
                // Apple & Milk & Tea
                "Cafe Master|15|0", "Warehouse Manager|16|0",
                // ChocoCraft Plus
                "Stablehand|19940402|0",
                // Enchanting Plus v4
                "Enchanter|935153|2",
                // Forestry
                "Apiarist|80|4", "Arborist|81|0",
                // Fossils and Archaeology
                "Archaeologist|303|2",
                // GraveStone Mod
                "Undertaker|385|4",
                // Growthcraft
                "Brewer|10|0", "Apiarist|14|4",
                // Growthcraft Community Edition
                "Brewer|6677|0", "Apiarist|7766|4",
                // Immersive Engineering
                "Engineer|512|0",
                // Mine Trading Cards
                "Card Master|7117|2", "Card Trader|7118|0",
                // More Swords mod version 2
                "Swordsmith|66|5",
                // Musica
                "Clerk|52798|0",
                ...
                "Engineer|456|3",
                // Thaumcraft
                "Wizard|190|2", "Banker|191|0",
                // Tinkers Construct
                "Tinkerer|78943|5",
                // Traincraft
                "Station Chief|86|0",
                // Village Taverns
                "Bar Wench|42|0", "Hostler|43|0", "Shepherd|44|0", "Baker|45|0",
                // Witchery
                "Apothecary|2435|2", },
        ...
        // PMMerchantProfession = config.getString("PMMerchantProfession", "Mapping Professions", "Merchant", "The
        // career displayed for Primitive Mobs's Traveling Merchant. Blank this out to display no profession regardless
        // of addJobToName.");
        PMMerchantProfessionMap = config.getInt(
            "PM Traveling Merchant Profession ID",
            Reference.CATEGORY_MOD_INTEGRATION,
            0,
            0,
            5,
        ...
        // PMLostMinerProfession = config.getString("PMLostMinerProfession", "Mapping Professions", "Miner", "The career
        // displayed for Primitive Mobs's Lost Miner. Blank this out to display no profession regardless of
        // addJobToName.");
        PMLostMinerProfessionMap = config.getInt(
            "PM Lost Miner Profession ID",
            Reference.CATEGORY_MOD_INTEGRATION,
            3,
            0,
            5,
     ...
     * othermods.cfg > Mod Structures and assigns them to this instance's variables.
     */
     ...
     * othermods.cfg > Clickable Names and assigns them to this instance's variables.
     */
