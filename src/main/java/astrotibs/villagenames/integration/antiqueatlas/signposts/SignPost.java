...
public class SignPost extends StructureVillageVN.VNComponent
{
    ...
    private static String[] foundationPattern = new String[]{
        	"FFFFF",
        	"FFFFF",
        	"FFFFF",
        	"FFFFF",
    ...
    public SignPost(StartVN start, int componentType, Random random, StructureBoundingBox boundingBox, int coordBaseMode)
    {
        super();
    ...
    public static SignPost buildComponent(StartVN villagePiece, List pieces, Random random, int x, int y, int z, int coordBaseMode, int componentType)
    {
        ...
        return canVillageGoDeeper(structureboundingbox) && StructureComponent.findIntersecting(pieces, structureboundingbox) == null ? new SignPost(villagePiece, componentType, random, structureboundingbox, coordBaseMode) : null;
    }
    ...
    public boolean addComponentParts(World world, Random random, StructureBoundingBox structureBB)
    {
    	...
    	blockObject = ModObjects.chooseModStrippedLog(biomeLogVertMeta, 0); Block biomeStrippedLogVerticBlock = (Block)blockObject[0]; int biomeStrippedLogVerticMeta = (Integer)blockObject[1];
    	// Fence
    	blockObject = StructureVillageVN.getBiomeSpecificBlockObject(Blocks.fence, 0, this.materialType, this.biome, this.disallowModSubs); Block biomeFenceBlock = (Block)blockObject[0]; int biomeFenceMeta = (Integer)blockObject[1];
    	...
    	this.establishFoundation(world, structureBB, this.foundationPattern, this.GROUND_LEVEL, this.materialType, this.disallowModSubs, this.biome, biomeTopBlock, biomeTopMeta, biomeFillerBlock, biomeFillerMeta, false);
		...
		case JUNGLE: // 6 2 1 1
    		signIsOn = 
			random_pick < 6 ? NOTHING :
			random_pick < 6+2 ? COBBLESTONE_BLOCK :
			random_pick < 6+2+1 ? COBBLESTONE_WALL :
			STRIPPED_LOG;
    		break;
    	case DESERT: // 2 8 0 0
    		signIsOn = 
			random_pick < 2 ? NOTHING :
			COBBLESTONE_BLOCK;
			break;
		case TAIGA: // 2 3 2 3
    		signIsOn = 
			...
			random_pick < 2+3+2 ? COBBLESTONE_WALL :
			STRIPPED_LOG;
    		break;
		case SAVANNA: // 9 0 0 1
    		signIsOn = 
			random_pick < 9 ? NOTHING :
			STRIPPED_LOG;
    		break;
		case SWAMP: // 6 2 0 2
    		signIsOn = 
    	...
    	if (!signIsOn.equals(NOTHING)) {
    		...
    		this.placeBlockAtCurrentPosition(world,
    				signIsOn.equals(COBBLESTONE_BLOCK) ? biomeCobblestoneBlock : signIsOn.equals(COBBLESTONE_WALL) ? biomeCobblestoneWallBlock : biomeStrippedLogVerticBlock,
					signIsOn.equals(COBBLESTONE_BLOCK) ? biomeCobblestoneMeta : signIsOn.equals(COBBLESTONE_WALL) ? biomeCobblestoneWallMeta : biomeStrippedLogVerticMeta,
    				2, GROUND_LEVEL, signPostZBB, structureBB);
    	}
    	...
    	case PLAINS: // 4 4 2 / 1
    		underTheGround = 
			random_pick < 4 ? NOTHING :
			random_pick < 4+4 ? COBBLESTONE_BLOCK :
			STRIPPED_LOG;
    		break;
    	case DESERT: // 5 5 0 / 1
		case JUNGLE: // 5 5 0 / 1
    		underTheGround = 
			random_pick < 5 ? NOTHING :
			COBBLESTONE_BLOCK;
    		break;
		case TAIGA: // 2 4 4 / 1-2
    		underTheGround = 
			random_pick < 2 ? NOTHING :
			random_pick < 2+4 ? COBBLESTONE_BLOCK :
			STRIPPED_LOG;
		...
		case SNOWY: // 4 2 4 / 1
    		underTheGround = 
			random_pick < 4 ? NOTHING :
			random_pick < 4+2 ? COBBLESTONE_BLOCK :
			STRIPPED_LOG;
    		break;
		case SWAMP: // 0 5 5 / 1-2
    		underTheGround = 
			random_pick < 5 ? COBBLESTONE_BLOCK :
			STRIPPED_LOG;
    		signFoundationDepth = random.nextInt(2)+1;
    		break;
    	}
    	...
    	if (signFoundationDepth != 0) {
    		this.fillWithMetadataBlocks(world, structureBB,
    				2, GROUND_LEVEL-signFoundationDepth, signPostZBB,
    				2, GROUND_LEVEL-1, signPostZBB,
    	...
    	case PLAINS: // 3 7 0
			enclosingMaterial = 
			random_pick < 3 ? NOTHING :
			FENCE;
			connectedWalls = true;
    		break;
    	case DESERT: // 7 0 3
			enclosingMaterial = 
			random_pick < 7 ? NOTHING :
			COBBLESTONE_WALL;
		...
		case TAIGA: // 2 4 4
			enclosingMaterial = 
			random_pick < 2 ? NOTHING :
			random_pick < 2+4 ? FENCE :
			COBBLESTONE_WALL;
			connectedWalls = random.nextBoolean();
    		break;
		case SAVANNA: // 8 2 0
			enclosingMaterial = 
			random_pick < 8 ? NOTHING :
			FENCE;
    	...
    	if (!enclosingMaterial.equals(NOTHING)) {
            for(int[] uuvvww : new int[][]{
            	{0,GROUND_LEVEL,3, 4,GROUND_LEVEL,3}, 
            	{0,GROUND_LEVEL,0, 0,GROUND_LEVEL,3}, 
            	{4,GROUND_LEVEL,0, 4,GROUND_LEVEL,3}, 
            	})
            {
            	Block fenceMaterialBlock = enclosingMaterial.equals(FENCE) ? biomeFenceBlock : biomeCobblestoneWallBlock;
            	int fenceMaterialMeta = enclosingMaterial.equals(FENCE) ? biomeFenceMeta : biomeCobblestoneWallMeta;
            	
            	if (connectedWalls) {
            		this.fillWithMetadataBlocks(world, structureBB,
                			uuvvww[0], uuvvww[1], uuvvww[2], uuvvww[3], uuvvww[4], uuvvww[5],
                			fenceMaterialBlock, fenceMaterialMeta,
							fenceMaterialBlock, fenceMaterialMeta,
                			false);	
            	}
            	else
            	{
            		this.placeBlockAtCurrentPosition(world,
            				fenceMaterialBlock, fenceMaterialMeta,
            				uuvvww[0], uuvvww[1], uuvvww[2],
            				structureBB);
            		this.placeBlockAtCurrentPosition(world,
            				fenceMaterialBlock, fenceMaterialMeta,
            				uuvvww[3], uuvvww[4], uuvvww[5],
            				structureBB);
            	}
            	
            	if (this.villageType==VillageType.SWAMP)
            	{
        			this.func_151554_b(world, biomeStrippedLogVerticBlock, biomeStrippedLogVerticMeta, uuvvww[0], uuvvww[1]-1, uuvvww[2], structureBB);
        			this.func_151554_b(world, biomeStrippedLogVerticBlock, biomeStrippedLogVerticMeta, uuvvww[3], uuvvww[4]-1, uuvvww[5], structureBB);
            	}
            }
    	}
    	...
    	{
    		for (int[] uvw : new int[][]{
            	{0,torchHeight,0}, {4,torchHeight,0},
            	}) {
            	this.placeBlockAtCurrentPosition(world, torchlikeBlock, torchlikeMeta, uvw[0], uvw[1], uvw[2], structureBB);
            	// Add support post
            	if (this.villageType==VillageType.SWAMP)
            	{
        			this.func_151554_b(world, biomeStrippedLogVerticBlock, biomeStrippedLogVerticMeta, uvw[0], GROUND_LEVEL-1, uvw[2], structureBB);
            	}
            }
    	}
    	...
    	{
    		for (int[] uvw : new int[][]{
            	{0,torchHeight,3}, {4,torchHeight,3},
            	}) {
            	this.placeBlockAtCurrentPosition(world, torchlikeBlock, torchlikeMeta, uvw[0], uvw[1], uvw[2], structureBB);
            	// Add support post
            	if (this.villageType==VillageType.SWAMP)
		...
		if (signpostsSignBlock!=null && true)
		{
        	...
        	for (int i=0; i<2; i++) {
        		int signPostMeta = new int[]{StructureVillageVN.getSignRotationMeta(8, this.coordBaseMode, false)/4, 4}[i];
        		boolean isBottom = signPostMeta!=4;
        		int signPostYMod = new int[]{0, 1}[i];
        		// Block
        		world.setBlock(signPostX, signPostY+signPostYMod, signPostZ, signpostsSignBlock,
        				signPostMeta, // Meta
        				2);
...
//                    		String label = this.nameRoot.trim() + " WP";
            				...
            				int markerId = harceroi.mc.signposts.SignPostsMod.addGlobalMarker(signX, signZ, markerName, world);
            				signPost.setAsJumpLocation(jumpX, jumpY, jumpZ, markerId);
            				harceroi.mc.signposts.data.MarkerToTileMap.get(world).setTileForMarker(signX, signY, signZ, markerId);
            				
            				signPostTagCompound.setDouble("jumpX", signPostX + (signPostMeta==1 ? -1 : signPostMeta==3 ? 1 : 0) + 0.5);
                        	signPostTagCompound.setDouble("jumpY", signPostY);
                        	signPostTagCompound.setDouble("jumpZ", signPostZ + (signPostMeta==0 ? 1 : signPostMeta==2 ? -1 : 0) + 0.5);
            		...
            		LogHelper.info("Error while assigning TileEntity to SignPost at "+signPostX+" "+(signPostY+signPostYMod)+" "+signPostZ+":");
            		LogHelper.info(e);
            	}
			}
		}
