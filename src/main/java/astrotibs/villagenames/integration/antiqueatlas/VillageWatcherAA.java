...
public class VillageWatcherAA {
    ...
    public static final String TILE_VILLAGE_FENCE = "npcVillageFence", TILE_VILLAGE_TREE = "npcVillageTree",
    ...
    public static void setCustomTileTexture(String tileName, String printedName, String aaTileFileRoot) {
        ...
        tileAPI.setCustomTileTexture(
                tileName,
                tileAPI.registerTextureSet(
                        printedName,
                        new ResourceLocation(
                                Reference.ANTIQUE_ATLAS_MODID,
                                "textures/gui/tiles/" + aaTileFileRoot + ".png")));
    }
    ...
    // === ANTIQUE ATLAS ICONS === //
    public static final String TILE_VILLAGE_LIBRARY = "npcVillageLibrary", TILE_VILLAGE_SMITHY = "npcVillageSmithy",
    ...
    // private static final String HOUSE_SMALL = "ViSH"; // Slightly larger than huts, sometimes with a fenced balcony
    // on the roof and a ladder.
    ...
    VN_PATH = "VNPath", // VN Path
            ...
            SWAMP_HORRIBLE_SECRET = "VNSwHoSe", // Swamp Horrible Secret - NO MAPPING for this because it's supposed to
                                                // be a secret!
    ...
    private void visitVillage(World world, NBTTagCompound tag) {
        if (!VillageGeneratorConfigHandler.newVillageGenerator) {
            return;
        } // Don't do any of this if you're not using the new village generator
        ...
        if (GeneralConfig.debugMessages) {
            LogHelper.info(
                    "Visiting NPC Village in dimension #" + world.provider.dimensionId
                            + " \""
        ...
        for (int i = 0; i < children.tagCount(); i++) {
            ...
            if (isTownCenter(childID)) {
                ...
                for (int j = -1; j <= 1; j++) {
                    for (int k = -1; k <= 1; k++) {
                        List<Marker> markers = AntiqueAtlasMod.globalMarkersData.getData().getMarkersAtChunk(
                ...
                if (markers != null) {
                    for (Marker marker : markers) {
                        if (marker.getType().equals(VILLAGE_MARKER)) {
                            ...
                            if (GeneralConfig.debugMessages) {
                                LogHelper.info(
                                        "Removed faux village marker at x=" + (chunkX << 4) + " z=" + (chunkZ << 4));
                            }
                            break;
                        }
                    }
    ...
    public static boolean isTownCenter(String childID) {
        return PLAINS_FOUNTAIN.equals(childID) || PLAINS_WELL.equals(childID)
