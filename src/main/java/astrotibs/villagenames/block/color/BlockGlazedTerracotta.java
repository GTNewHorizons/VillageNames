...
public class BlockGlazedTerracotta extends Block {
    ...
    public void registerBlockIcons(IIconRegister iicon) {
        ...
        for (int subs = 0; subs < 4; subs++) {
            /*
             * VN addresses are kept here because there needs to be two icons per block, which is different from the
             * vanilla-style one per block. If someone were to use a resource pack to reference the vanilla addresses,
    ...
    public IIcon getIcon(int side, int metadata) {
        ...
        if (side == 0) {
            sideIcon = this.blockIcons[metadata % 4 + 16];
        } // Bottom
        if (side == 2 && ((metadata / 4) % 2 == 0)) {
            sideIcon = this.blockIcons[metadata % 4 + 16];
        } // North
        if (side == 5 && ((metadata / 4) % 2 == 1)) {
            sideIcon = this.blockIcons[metadata % 4 + 16];
        } // East
        return sideIcon;
    }
